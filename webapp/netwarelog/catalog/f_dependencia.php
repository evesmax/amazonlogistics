<?php
	...
	if($regd{'tipodependencia'}=="S"){
		...
		$objeto="
			...
			$sql="select ".$campovalor.", ".$campodesc." from ".$dependenciatabla." ".$sqlw." order by catalog_campodesc "; // ".$campodesc;
			//error_log($sql);
		...
		// SecundarioLog
		if($reg{'formato'}!="-1"){
			$objeto.="</div></td><td style='padding-left:10px;'><input class='btn btn-default btn-sm' type='button' value='...' onclick='btn_i".$reg{'idcampo'}."_click();' /></td></tr></table>";
