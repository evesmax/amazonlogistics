...
class estructura{
			...
			if($conexion->existe($sql)){
				?>
					<script type='text/javascript'> 
						alert("El nombre de la estructura ya se encuentra seleccionado.\n\nEs posible que por otro lado, este utilizando parte de los nombres reservados: organizaciones, empleados, accelog, etc.");
						window.history.go(-1);
					</script>
				<?php
			}else{						
    ...
    $conexion->transaccion("CATALOG -  DESHABILITAR ESTRUCTURA - ID:".$this->idestructura,$sql);
	...
	function habilitar($conexion){
		...
		//REGISTRO TRANSACCIONES -- 2013-10-04
    $conexion->transaccion("CATALOG -  HABILITAR ESTRUCTURA - ID:".$this->idestructura,$sql);
	...
	function activar($conexion){
		...
		if($conexion->tipobd()=="mssql"){
			...
			//REGISTRO TRANSACCIONES -- 2013-10-04
    	$conexion->transaccion("CATALOG -  ACTIVAR ESTRUCTURA - CREANDO RESPALDO - ID:".$this->nombreestructura."_f".date('Y_m_d')."_h".date('H_i_s'),$sql_exec);
		...
		while($reg=$conexion->siguiente($result)){
			...
			switch($reg{'tipo'}){
				...
				case 'archivo_base':
						...
						$tipo.=$reg{'nombrecampo'}."_name varchar(255) NULL default '',  ";
						$tipo.=$reg{'nombrecampo'}."_size varchar(15) NULL default '',  ";
						$tipo.=$reg{'nombrecampo'}."_type varchar(25) NULL default '',  ";						
						break;
