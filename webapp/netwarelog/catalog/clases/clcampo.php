...
class campo{
	...
	function setformula($formula){ 
		//2010-11-30 Este if es por diferencia del sistema operativo
		//Para poder guardar apostrofes en la sección de fórmula.
		if(strpos($formula,"\\")){ 
			$this->formula = $formula;
		} else { 
			$formula = str_ireplace("'","\'",$formula);
			$this->formula = $formula; 
		}
					//echo $this->formula;
	}
	...
	function guardar($conexion){		
		if($this->idcampo!=-1){ 
      	...
      	$conexion->transaccion("CATALOG - ADMIN - AGREGA CAMPO - ".$this->nombrecampo,$sql);
