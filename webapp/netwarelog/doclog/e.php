<?php
	...
	$sqlw = $conexion->escapalog_leve($_GET['sw']);
	$idfolio = $conexion->escapalog($_GET['idfolio']);
	...
	$sqlcampofile.= "idestructura = ".$_SESSION['idestructura'];
	$resultcampofile = $conexion->consultar($sqlcampofile);
	while($rscc=$conexion->siguiente($resultcampofile)){
		$sql_nombrearchivo = " select ".$rscc{"nombrecampo"}." from ".$_SESSION['nombreestructura']." where ".$sqlw;
		$resultcampofile_datos = $conexion->consultar($sql_nombrearchivo);
		while($rscc_datos=$conexion->siguiente($resultcampofile_datos)){
			$directorioarchivo="../archivos/".$_SESSION["accelog_idorganizacion"]."/".$_SESSION["nombreestructura"];
	...
	$sql = " delete from ".$_SESSION["nombreestructuradetalle"]."  where  ".$_SESSION["campofolio"]."='".$idfolio."'";
	//echo $sql;
	$conexion->consultar($sql);
        ...
        $conexion->transaccion("CATALOG - ELIMINACION - ".$_SESSION['nombreestructura'],$sql);
