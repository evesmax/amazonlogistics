...
echo "<h1>Diagnostic Folio 5793</h1>";

// 1. Check reception
echo "<h2>1. Reception</h2>";
$sql_rec = "SELECT * FROM logistica_recepciones WHERE idrecepcion = 5793";
...
if ($res_rec && $conexion->count_rows($res_rec) > 0) {
    ...
    echo "<p style='color:red;'>Reception 5793 NOT found in logistica_recepciones!</p>";
}
...
$sql_mov = "SELECT * FROM inventarios_movimientos WHERE foliodoctoorigen = 5793 AND doctoorigen = 4";
...
if ($res_mov && $conexion->count_rows($res_mov) > 0) {
    ...
    echo "<p style='color:red;'>Inventory movement for Folio 5793 (doctoorigen=4) NOT found in inventarios_movimientos!</p>";
}
