...
echo "<table border=0 id='empresa' style='width:550px'><tr style='width:550px'><td style='text-align:center;width:550px'><b style='text-align:center;'>".$empresa['nombreorganizacion']."</b></td></tr></table>";
	...
	$r = $conexion->consultar($myQuery);
	
	$tipoCuenta = mysql_fetch_assoc($r);
	$tipoCuenta  = $tipoCuenta['TipoNiveles'];
	...
	if($tipoCuenta == 'm')
			...
			if($tipoCuenta == 'a')
			{
				...
				for($i=3;$i<=8;$i++)
				{
					...
					$masSplitsM .= "REPLACE(SUBSTRING(SUBSTRING_INDEX(Code, '.', $i), LENGTH(SUBSTRING_INDEX(Code, '.', $i -1)) + 1),'.', '') AS h$i, ";
				}
			}
...
$sql = "SELECT 
	...
	Manual_Code,
	Clasificacion, 
	REPLACE(SUBSTRING(SUBSTRING_INDEX(Code, '.', 1),
       LENGTH(SUBSTRING_INDEX(Code, '.', 1 -1)) + 1),
       '.', '') AS h1,
...
REPLACE(SUBSTRING(SUBSTRING_INDEX(a.account_code, '.', 2),
	...
	IF(TipoMovto = 'Cargo',Importe, 0) AS Cargos, 
	IF(TipoMovto = 'Abono',Importe, 0) AS Abonos, 
	'Movimientos Corrientes' AS Flag , p.idperiodo AS idperiodo
...
ORDER BY 
	CAST(h1 AS UNSIGNED),CAST(h2 AS UNSIGNED) , $orden, Fecha, idperiodo ;";
 // echo $sql;
