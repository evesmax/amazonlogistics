...
$data->read(dirname(__FILE__).'/productos_tempPrices.xls');
...
for ($i = 2; $i <= $data->sheets[0]['numRows']; $i++) 
{
    ...
    $dato[1] = trim($data->sheets[0]["cells"][$i][1]); //ID
    $dato[2] = trim($data->sheets[0]["cells"][$i][2]); //Codigo
    $dato[3] = trim($data->sheets[0]["cells"][$i][3]); //Nombre
    $dato[4] = trim($data->sheets[0]["cells"][$i][4]); //Precio
    $dato[5] = trim($data->sheets[0]["cells"][$i][5]); //Costo
    $dato[6] = trim($data->sheets[0]["cells"][$i][6]); //idProve
    $dato[7] = trim($data->sheets[0]["cells"][$i][6]); //Fecha Modif
    $dato[8] = trim($data->sheets[0]["cells"][$i][7]); // Estatus
    $dato[9] = trim($data->sheets[0]["cells"][$i][8]); // modificar
    
    if($dato[1] != '' && $dato[4] != '')
        $this->ProductoModel->updatePrice($dato);
    else
    {
