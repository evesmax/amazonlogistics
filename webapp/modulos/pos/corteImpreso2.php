                ...
                <table   id="pagosGrid" style="font-size:9px; border: 1px solid black;">
                 <tr style="background-color:#dcdcdc;">
                                                ...
                                                foreach ($resumenCorte['ventas'] as $key => $value) {
                                                    ...
                                                    $Ni += $value['Ni'];
                                                    $contenido.= '<td align="center">$'.$value['TVales'].'</td>';
                                                    $TVales += $value['TVales'];
                                                ...
                                                $contenido.= '<td>$'.number_format($TVales,2).'</td>';
...
$contenido .='<div style="width:100%; align:center;"><h4>Tarjetas</h4></div>';
$contenido .='<div style="width:100%; align:center;"><table style="border: 1px solid black;">  
                                            ...
                                            foreach ($resumenCorte['tarjetas'] as $key => $value) {
                                                $contenido.= '<tr>';
                                                $contenido.= '<td>'.$value['tarjeta'].'</td>';
                                                $contenido.= '<td>$'.number_format($value['total'],2).'</td>';
                                                $contenido.= '</tr>';
                                            }
...
$contenido .='<div style="width:100%; align:center;"><h4>Abonos de Caja</h4></div>';
$contenido .='<div style="width:100%; align:center;"><table style="border: 1px solid black;">
                                                <tr style="background-color:#dcdcdc;">
                                                    <th>ID Abono</th>
                                                    <th>Fecha</th>
                                                ...
                                                foreach ($resumenCorte['abonos'] as $key => $value) {
                                                    ...
                                                    $contenido.= '<td>'.$value['usuario'].'</td>';
                                                    $contenido.= '<td align="center">$'.number_format($value['cantidad'],2).'</td>';
                                                    $cantidad4 += $value['cantidad'];
                                                    $contenido.= '</tr>';
                                                }
                                                $contenido.= '<tr style="background:white;">';
                                                $contenido.= '<td colspan="4">Totales</td>';
                                                $contenido.= '<td align="center" style="background:#A9F5A9;">$'.number_format($cantidad4,2).'</td>';
                                                $contenido.= '</tr></table></div>'; 
