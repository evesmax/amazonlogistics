...
if(!isset($print) || $print == 'true')
{
	...
	$(function(){
		...
		if(isMobile.mobilecheck()){
			...
			<?php
				$src = '../../netwarelog/archivos/1/organizaciones/' . $organizacion[0]['logoempresa'];
				if(file_exists($src)){
			...
			jsonCaja = generarTicket(jsonCaja, "<?php echo utf8_decode($organizacion[0]['nombreorganizacion']);?>", 0, 0, 2);
			jsonCaja = generarTicket(jsonCaja, "RFC: <?php echo utf8_decode($organizacion[0]['RFC']);?>", 0, 0, 2);
			jsonCaja = generarTicket(jsonCaja, "<?php echo utf8_decode($datosSucursal[0]['direccion']." ".$datosSucursal[0]['municipio'].",".$datosSucursal[0]['estado']);?>", 0, 0, 2);
			
			<?php if($organizacion[0]['paginaweb']!='-'){?>
					jsonCaja = generarTicket(jsonCaja, "<?php echo $organizacion[0]['paginaweb']?>", 0, 0, 2);
			<?php } ?>
			<?php if(strcmp($venta[0]['estatus'],"Cancelada")==0){?>
				jsonCaja = generarTicket(jsonCaja, "<?php echo "Venta ".$venta[0]['estatus'];?>", 0, 0, 2);
			<?php }  ?>

			jsonCaja = generarTicket(jsonCaja, "Sucursal: <?php echo $datosSucursal[0]['nombre']; ?>", 0, 0, 2);
			jsonCaja = generarTicket(jsonCaja, "Ticket de compra", 0, 0, 2);
			jsonCaja = generarTicket(jsonCaja, "Cliente: <?php echo $venta[0]['cliente']; ?>", 0, 0, 2);
			jsonCaja = generarTicket(jsonCaja, "Cajero: <?php  echo $venta[0]['empleado']; ?>", 0, 0, 2);
			jsonCaja = generarTicket(jsonCaja, "<?php echo $cajaController->formatofecha($venta[0]['fecha']);?>", 0, 0, 2);
			jsonCaja = generarTicket(jsonCaja, "Id venta:<?php  echo $venta[0]['folio']; ?>", 0, 0, 2);
			...
			// Valida si la instancia tiene Foodware, para mostrar los dolares
				...
				// Consulta los ajustes de Foodware
					...
					// Imprime los datos de la comanda
						if (!empty($_SESSION['detalles_mesa'])) { ?>
							jsonCaja = generarTicket(jsonCaja, '________________________________', 1, 0, 1);
							<?php if (!empty($_SESSION['detalles_mesa']['nombre_mesero'])) { ?>
								jsonCaja = generarTicket(jsonCaja, "Mesero: <?php echo  $_SESSION['detalles_mesa']['nombre_mesero'] ?>", 0, 0, 2);
							<?php } ?>
							<?php if (!empty($_SESSION['detalles_mesa']['persona'])) { ?>
								jsonCaja = generarTicket(jsonCaja, "Personas: <?php echo  $_SESSION['detalles_mesa']['persona'] ?>", 0, 0, 2);
							<?php } ?>
							jsonCaja = generarTicket(jsonCaja, "Mesa: <?php echo $_SESSION['detalles_mesa']['nombre_mesa']; ?>", 0, 0, 2);
					 		jsonCaja = generarTicket(jsonCaja, "<?php echo $_SESSION['detalles_mesa']['codigo']; ?>", 0, 0, 2);
			...
			jsonCaja = generarTicket(jsonCaja, 'Subtotal: $<?php echo number_format($sub,2,".",","); ?>', 1, 0, 3);
			<?php 
				if($venta[0]['descuento'] > 0){?>
					jsonCaja = generarTicket(jsonCaja, 'Descuento: $<?php echo number_format($venta[0]["descuento"],2); ?>', 1, 0, 3);
			<?php } ?>
			<?php 
				$totalimpuestos = 0;
				foreach ($impuestos_venta as $key2 => $value2) {?>
					jsonCaja = generarTicket(jsonCaja, '<?php echo $key2; ?>: $<?php echo number_format($value2,2); ?>', 1, 0, 3);
			<?php
				$totalimpuestos+=$value2;
			 } ?>
			jsonCaja = generarTicket(jsonCaja, 'Total: $<?php echo number_format((($sub+$totalimpuestos) - $venta[0]["descuento"]),2,".",",").' '.$venta[0]["codigo"]; ?>', 1, 0, 3);

			<?php 
				foreach ($pagos as $key => $value) {?>
					jsonCaja = generarTicket(jsonCaja, '<?php echo $value["nombre"]; ?>: $<?php echo number_format($value["monto"],2).$venta[0]["codigo"]; ?>', 1, 0, 3);
			<?php } ?>

			jsonCaja = generarTicket(jsonCaja, 'Cambio: $<?php echo number_format($venta[0]["cambio"],2,".",",").$venta[0]["codigo"]; ?>', 1, 0, 3);
			<?php
		...
		// Valida si la instancia tiene Foodware, para mostrar los dolares
			...
			// Valida si se debe de mostrar la informacion de los dolares
				...
				// Convierte a dolares
					...
					jsonCaja = generarTicket(jsonCaja, 'Dolar americano: $<?php echo $conversion ?>', 1, 0, 3);
					<?php
				}
			} ?>
			jsonCaja = generarTicket(jsonCaja, '________________________________', 1, 0, 1);
			...
			jsonCaja = generarTicket(jsonCaja, 'Ingresando el Siguiente codigo:', 1, 0, 2);
			...
			?> 
			jsonCaja = generarTicket(jsonCaja, '<?php echo $codigoFactura?>', 0, 0, 2);
			//<?php echo $codigoQrVenta; ?>


			// para QR en modulo print
			jsonCaja = generarTicket(jsonCaja, 'Para obtener su factura ingrese a la direccion:', 1, 0, 2);
			jsonCaja = generarTicket(jsonCaja, '<?php echo $url?>', 0, 0, 2);
			jsonCaja = generarTicket(jsonCaja, '<?php echo $url2?>', 0, 0, 2);



			<?php } ?>
			jsonCaja = generarTicket(jsonCaja, '<?php echo $leyenda?>', 0, 0, 2);
			...
			if(moduloPrint == 0){
				...
				// Valida si la instancia tiene Foodware, para mostrar los dolares
					...
					// Consulta los ajustes de Foodware
						...
						// Imprime los datos de la comanda
							if (!empty($_SESSION['detalles_mesa'])) { ?>
								jsonCaja = generarTicket(jsonCaja, separador, 1, 0, 1);
								<?php if (!empty($_SESSION['detalles_mesa']['nombre_mesero'])) { ?>
									jsonCaja = generarTicket(jsonCaja, "Mesero: <?php echo  $_SESSION['detalles_mesa']['nombre_mesero'] ?>", 0, 0, 2);
								<?php } ?>
								<?php if (!empty($_SESSION['detalles_mesa']['persona'])) { ?>
				...
				var segundo = 0;
	            var impresionTexto = ""; 
	            console.log("textomadafaka")
	            console.log(JSON.stringify(jsonCaja[1]));

	            $.each(jsonCaja, function(index, element){
	                if(segundo == 0){
	                    segundo = 1;
	                }else{
	            ...
	            impresionTexto = impresionTexto +  "by Appministra";
...
<div id="receipt_wrapper">
	...
	<div id="customer" style="text-align: center; font-size:13px;font-family: Tahoma,'Trebuchet MS',Arial;">Cliente:<?php echo $venta[0]['cliente']; ?> - Puntos:<?php echo $venta[0]['puntos']; ?> <?php echo $consumo ?></div>

	<?php 
		if($consumo == 1){
		?>
			<?php if($venta[0]['rfc'] != '') { ?> <div style="text-align: center; font-size:13px;font-family: Tahoma,'Trebuchet MS',Arial;">RUC: <?php echo $venta[0]['rfc']; ?></div> <?php } ?>
			<?php if($venta[0]['email'] != ''){ ?> <div style="text-align: center; font-size:13px;font-family: Tahoma,'Trebuchet MS',Arial;">Correo: <?php echo $venta[0]['email']; ?></div> <?php } ?>
		<?php	
		}
...
// Imprime los datos de la comanda
		if (!empty($_SESSION['detalles_mesa'])) { ?>
			...
			<div id="receipt_general_info" style="text-align:center;">
				...
				<?php // para mostrar direccion en pedidos a domicilio 
					if($_SESSION['detalles_mesa']['id_cliente'] != NULL)
						{
					?>						
						<div style="width: 99%; float: left; text-align: left;font-size:13px;font-family: Tahoma,'Trebuchet MS',Arial; padding-left:5%;">Dirección: <?php echo $_SESSION['detalles_mesa']['domicilio']; ?></div>
						<div style="width: 99%; float: left; text-align: left;font-size:13px;font-family: Tahoma,'Trebuchet MS',Arial; padding-left:5%;">Colonia: <?php echo $_SESSION['detalles_mesa']['colonia']; ?></div>
						<div style="width: 99%; float: left; text-align: left;font-size:13px;font-family: Tahoma,'Trebuchet MS',Arial; padding-left:5%;">Referencia: <?php echo $_SESSION['detalles_mesa']['referencia']; ?></div>						
						<div style="width: 45%; float: left; text-align: left;font-size:13px;font-family: Tahoma,'Trebuchet MS',Arial; padding-left:5%">Tel:  <?php echo $_SESSION['detalles_mesa']['telefono1']; ?></div>
						<div style="width: 45%; float: left; text-align: right;font-size:13px;font-family: Tahoma,'Trebuchet MS',Arial; padding-left:5%">Cel: <?php echo $_SESSION['detalles_mesa']['celular']; ?></div>						
					<?php
						}
					?>
			</div>
...
<!-- puntos-->
 <?php 
	if($venta[0]['puntosTarjataRegalo']  != '' || $venta[0]['puntos'] != ''){
 ?>
		<!-- <div> <strong>Puntos x venta actual:</strong> <?php echo $venta[0]['puntosTarjataRegalo'];?> </div> -->
		<div> <strong>Puntos x venta actual:</strong>  <?php echo $_SESSION['puntos_venta']?> </div>		
 <?php 
	}
 ?> 
...
<?php
	...
	<tr>
		<td style=" text-align:center; max-width: 400px"><p>by Appministra.</p></td>
	</tr>
</table> 
