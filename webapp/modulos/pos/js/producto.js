...
function claveAsing(){
    $('#claveAsing').val($('#claveSat').val());
    if($('#claveAsing').val()!='01010101'){
        $('#claveGnerica').prop( "checked",false );
    }
}
function guardar(){
    ...
    var prd_terminado = $('#prd_terminado').val();
    ...
    if(precio  < 0){
       ...
       if($('#checkAntibiotico').is(':checked')){
            antibiotico = 1;
       }else{
            antibiotico = 0;
       }
   ...
   /* console.log(impuestos_ids);
    ...
    $("#productoskitList tr").each(function (index)
    {
        contador++;
        idProve = $(this).attr('idproduct');
        precioPro = $('#prod_'+idProve).val();
        if(precioPro!=''){
            productosKit +='/'+idProve+'-'+precioPro;
        }
    });
...
//
// krmn master
var empaquexcaja = $("#empaquesxcaja").val();
var cantidadxempaque = $("#cantidadxempaque").val();
// fin krmn master
    ...
    $.ajax({
        ...
        data: {idProducto: idProducto,
        	...
        	cantidadxempaque:cantidadxempaque,
                ...
                insumvar: $('#insumvar').prop ("checked")? 1 : 0,
                vendible: $('#vendible').prop ("checked")? 1 : 0
...
function servicio(){
   ...
   $('[name="pesoCheck"]').attr('disabled', false);
...
function asignar_agru_req($objeto){
    ...
    id=$objeto['id'];
    idagru=$('#selid_'+id).val();
    idagrunom=$('#selid_'+id+' option:selected').text();
    $objeto['agrupador']=idagru;
    $objeto['agrupadornom']=idagrunom;
    ...
    $.ajax({
        data : $objeto,
        url : 'ajax.php?c=producto&f=asignar_agru_req',
        type : 'POST',
        dataType : 'json'
    }).done(function(resp) {
        console.log('----> Done asignar_cant_req');
...
function agregaProductoKit(){
    var idProd = $('#selectProductoKit').val();
    var cantidad = $('#productoKitPrecio').val();
    var a = 1;
    $("#productoskitList tr").each(function (index)
    {
    ...
    $.ajax({
        url: 'ajax.php?c=producto&f=nombreProducto',
        type: 'POST',
        dataType: 'json',
        data: {idProd: idProd},
    })
    .done(function(data) {
        console.log(data);
        $('#productoskitList tr:last').after('<tr idproduct="'+data[0].id+'" id="idProd_'+data[0].id+'"><td><span class="glyphicon glyphicon-remove" onclick="removeProduct('+data[0].id+');"></span></span></td><td>'+data[0].nombre+'</td><td>'+cantidad+'<input type="hidden" value="'+cantidad+'" id="prod_'+data[0].id+'"></td></tr>');
        $('#selectProductoKit > option[value="0"]').attr('selected', 'selected');
        $('#productoKitPrecio').val('');
