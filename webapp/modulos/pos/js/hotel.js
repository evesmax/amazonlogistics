...
function guardarHotel() {
    $.ajax({
        url: 'ajax.php?c=hotel&f=guardar',
        type: 'POST',
        dataType: 'json',
        data: {
    ...
    .done(function(result) {
        ...
        window.location = window.location.protocol + '//'+document.location.host+window.location.pathname+'?c=hotel&f=indexGridHoteles';
    })
    .fail(function() {
        console.log("error");
    })
