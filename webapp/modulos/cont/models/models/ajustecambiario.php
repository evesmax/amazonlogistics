<?php
	...
	class AjustecambiarioModel extends Connection
	{
		...
		function utilidadperdida($cuenta){
			...
			$sql = $this->query("select cu.* from cont_config c,cont_accounts cu where cu.father_account_id=".$cuentaok);
			return $sql;
		}
		
		function savePoliza2($idorg,$ejercicio,$periodo,$tipo,$concepto,$fecha)
		{
				$myQuery = "INSERT INTO cont_polizas(idorganizacion,idejercicio,idperiodo,numpol,idtipopoliza,concepto,fecha,fecha_creacion,activo,eliminado) VALUES($idorg,$ejercicio,$periodo,".($this->UltimoNumPol($periodo,$ejercicio,$tipo)+1).",".$tipo.",'".$concepto."','$fecha',NOW(),1,0)";
		...
		function ajustecambiario($periodo,$ejer,$moneda){//saca los importes por movimiento
			...
			where m.cuenta=c.account_id and  c.currency_id=".$moneda." and p.idejercicio=".$ejer." and c.`father_account_id`!=conf.CuentaBancos
		...
		function provisinpagar($periodo,$ejer,$moneda){
			...
			from cont_movimientos m,cont_polizas p,cont_accounts c,cont_config conf
			where m.cuenta=c.account_id and  c.currency_id=".$moneda."  and c.`father_account_id`!=conf.CuentaBancos and p.concepto!='POLIZA DE AJUSTE POR DIFERENCIA CAMBIARIA'
		...
		function cuentabancos(){//kite esto and p.idejercicio=1 
			...
			and c.`father_account_id`=conf.CuentaBancos and p.idtipopoliza!=3 and m.TipoMovto not like '%M.E%'
		...
		function bancos($periodo,$ejer,$moneda){//le agrege a banco esto and p.idtipopoliza!=3 porq toma en cuenta nose xq las polizas de ajuste
			...
			from cont_movimientos m,cont_polizas p,cont_accounts c,cont_config conf
			where m.cuenta=c.account_id and  c.currency_id=".$moneda." and p.idejercicio=".$ejer." and c.`father_account_id`=conf.CuentaBancos and p.idtipopoliza!=3
		...
		function moviextranjeros($ejer,$poli){
			...
			where m.IdPoliza=p.id and m.Cuenta=c.`account_id` and c.currency_id!=1 and m.Activo=1 
			 and p.idejercicio=".$ejer." and c.father_account_id!=con.CuentaBancos and p.id=".$poli);
			return $sql;
			
		}
		function compruebapoliza($periodo){
			$sql=$this->query("select id from cont_polizas where concepto='POLIZA DE AJUSTE POR DIFERENCIA CAMBIARIA' and  activo=1 and idperiodo=".$periodo);
			if($sql->num_rows>0){
		...
		switch($opc){
			...
			$sql=$this->query("select m.IdPoliza,m.Cuenta,sum(m.Importe) importe,m.TipoMovto,c.description,p.relacionExt,p.concepto,p.idperiodo,c.manual_code
			from cont_movimientos m,cont_polizas p,cont_accounts c,cont_config conf
			where m.cuenta=c.account_id and  c.currency_id=".$moneda."  and c.`father_account_id`=conf.CuentaBancos and m.TipoMovto='".$mov."'
			and m.IdPoliza=p.id  and p.activo=1 and m.Activo=1 and p.idperiodo<=".$periodo." 
			and p.id not in(select id from cont_polizas where concepto='POLIZA DE AJUSTE POR DIFERENCIA CAMBIARIA' and idperiodo=".$periodo.")
			group by  m.Cuenta,p.idperiodo,m.TipoMovto");
			return $sql;
	...
	function provisionescargos($peri,$ejer,$moneda,$opc){
		...
		$sql = $this->query('select m.IdPoliza,m.Cuenta,m.Importe importe,m.TipoMovto,c.description,p.relacionExt,p.concepto,c.manual_code,p.idperiodo
			...
			where m.cuenta=c.account_id and
			  ...
			  c.`father_account_id`!=conf.CuentaBancos and 
