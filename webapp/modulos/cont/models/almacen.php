...
class AlmacenModel extends Connection
{
	...
	function comun_facturacion()
	{
		$myQuery = "SELECT rfc
					FROM pvt_configura_facturacion
					WHERE id = 1";
	...
	function listaFacturas($vars)
	{
		...
		{
			$where = " AND f.cancelada = 0 AND temporal = 1";
			$asig = ", 0 AS asig ";
		}
		...
		if(intval($vars['tipo_facturas_2']) == 1)//Facturas
		{
			$where .= " AND json LIKE '%TipoDeComprobante\":\"I\"%' AND version = '3.3'";
		}

		if(intval($vars['tipo_facturas_2']) == 2)//Pagos
		{
			$where .= " AND json LIKE '%TipoDeComprobante\":\"P\"%' AND version = '3.3'";
		}

		if(intval($vars['tipo_facturas_2']) == 3)//NC
		{
			$where .= " AND json LIKE '%TipoDeComprobante\":\"E\"%' AND version = '3.3'";
			$where .= " AND json LIKE '%ClaveProdServ\":\"84111506\"%'";
		}

		if(intval($vars['tipo_facturas_2']) == 4)//Nomina
		{
		...
		if(intval($vars['prov']))
		{
			$where .= "AND f.uuid COLLATE utf8_general_ci NOT IN (SELECT uuid FROM cont_facturas_poliza WHERE uuid = f.uuid COLLATE utf8_general_ci AND tipo_poliza = 0 AND activo = 1) ";
		}
		

		$myQuery = "SELECT f.id, f.folio, f.json, f.serie, f.importe, f.version, f.uuid, f.xml, f.fecha, f.rfc, f.emisor, f.receptor, f.tipo, f.fecha_subida, IFNULL((SELECT id FROM app_respuestaFacturacion WHERE borrado = 0 AND folio COLLATE utf8_general_ci = f.uuid),0) AS pos, f.moneda $asig 
	...
	{
		$myQuery = "SELECT p.id , p.idtipopoliza, p.numpol, p.idperiodo, (SELECT NombreEjercicio FROM cont_ejercicios WHERE Id = p.idejercicio) AS ejercicio 
					FROM cont_movimientos m USE INDEX (idx_factura, movimientos_IdPoliza_idx)
					...
					SELECT p.id AS IdPoliza, p.idtipopoliza, p.numpol, p.idperiodo, (SELECT NombreEjercicio FROM cont_ejercicios WHERE Id = p.idejercicio) AS ejercicio 
					FROM cont_grupo_facturas g USE INDEX (idx_uuid)
	...
	function polizasAsignadas2($uuid)
	{
		$myQuery = "SELECT fp.uuid, fp.tipo_poliza, p.id, p.numpol, p.idperiodo, (SELECT NombreEjercicio FROM cont_ejercicios WHERE Id = p.idejercicio) AS ejercicio
			FROM cont_facturas_poliza fp USE INDEX (idx_uuid)
			INNER JOIN cont_polizas p ON p.id = fp.id_poliza
			 WHERE fp.uuid LIKE '%$uuid%' AND fp.activo = 1";
		$res = $this->query($myQuery);
		return $res;
	}
	...
	function guardaFacturaCaja($vars,$azurian,$idcliprov,$cliprov)
	{
		if(strpos($vars['xml'],'facturas'))
		{
			$xmlnopath = explode('/',$vars['xml']);
		...
		$folio = str_replace('.xml', '', $vars['xml']);
		$myQuery = "INSERT INTO app_respuestaFacturacion (idSale, idOs, idFact, folio, factNum, serieCsdSat, serieCsdEmisor, selloDigitalSat, selloDigitalEmisor, fecha, borrado, tipoComp, idComprobante, cadenaOriginal, trackid, xmlfile, origen, proviene, cancelre, id_poliza_mov, version) 
					VALUES(0,0,(SELECT id FROM comun_facturacion WHERE rfc LIKE '%".$vars['rfc']."%' AND cliPro = $cliprov LIMIT 1),'$folio',0,'0','0','','','".$vars['fecha']."',0,'P','','$azurian',0,'".$this->quitar_tildes($vars['xml'])."',1,0,0,0,'".$vars['version']."');";
		$this->query($myQuery);
	}
	...
	{
		if($arr)
			$myQuery = "INSERT IGNORE INTO cont_facturas_relacion VALUES('".$vars['uuid']."','".$vars['IdDocumento'][$i]."',".floatval($vars['ImpPagado'][$i]).",".floatval($vars['ImpSaldoAnt'][$i]).",".floatval($vars['ImpSaldoInsoluto'][$i]).",'".$vars['MonedaDR'][$i]."','".$vars['MetodoDePagoDR'][$i]."',".$vars['NumParcialidad'][$i].");";
		else
			$myQuery = "INSERT IGNORE INTO cont_facturas_relacion VALUES('".$vars['uuid']."','".$vars['IdDocumento']."',".floatval($vars['ImpPagado']).",".floatval($vars['ImpSaldoAnt']).",".floatval($vars['ImpSaldoInsoluto']).",'".$vars['MonedaDR']."','".$vars['MetodoDePagoDR']."',".$vars['NumParcialidad'].");";
	...
	function insertarPoliza($datos_factura,$fecha,$numpol,$xmls,$tipo)
	{
		...
		$myQuery = "INSERT INTO cont_polizas(idorganizacion, idejercicio, idperiodo, numpol, idtipopoliza, concepto, origen, idorigen, fecha, fecha_creacion, activo, eliminado, pdv_aut, usuario_creacion, usuario_modificacion)
					 VALUES(1,(SELECT Id FROM cont_ejercicios WHERE NombreEjercicio = ".$fecha[0]."),".intval($fecha[1]).",$numpol,".$datos_factura['id_tipo_poliza'].",'".$datos_factura['nombre_poliza']." $concepto ".$datos_factura['serie']." ".$datos_factura['folio']."','$origen',".$xmls.",'$fecha[0]-$fecha[1]-$fecha[2]',DATE_SUB(NOW(), INTERVAL 6 HOUR), 1, 0, 0, ".$_SESSION["accelog_idempleado"].", 0)"; 
		$idpoliza = $this->insert_id($myQuery);
		$this->query("INSERT INTO cont_facturas_poliza VALUES(0,(SELECT uuid FROM cont_facturas WHERE id = $xmls LIMIT 1),$tipo,$idpoliza,1)");
		return $idpoliza;
	}
	...
	function insertarMovimientos($id_poliza_acontia,$datos_factura,$fecha,$xmls,$json)
	{
		...
		$provision 	= $datos_factura['provision'];

		//Hace una busqueda , si la factura ya se creo no lo vuelve a realizar
		$existe = $this->query("SELECT IFNULL(SUM(1),0) AS num FROM cont_facturas_poliza WHERE uuid LIKE '%$uuid%' AND tipo_poliza = $provision AND activo = 1 AND id_poliza != $id_poliza_acontia ;");
		$existe = $existe->fetch_assoc();

		if(!intval($existe['num']))
		{
			...
			while($cp = $cuentas_poliza->fetch_assoc())
			{
				...
				else
				{
					...
					foreach($json['Comprobante']['cfdi:Impuestos']['cfdi:Traslados']['cfdi:Traslado'] as $impu){
						if($impu['@TasaOCuota'] == '0.160000')//Impuesto IVA 16%
							$ivas = $impu['@Importe'];
					}
				...
				$description = $json['Comprobante']['cfdi:Conceptos']['cfdi:Concepto']['@Descripcion'];
				...
				if(intval($cp['id_dato']) == 6)
				{
				 ...
				 $mov = $this->insert_id("INSERT INTO cont_movimientos(IdPoliza, NumMovto, IdSegmento, IdSucursal, Cuenta, TipoMovto, Importe, Referencia, Concepto, Activo, FechaCreacion, Factura, FormaPago, tipocambio) 
									VALUES($id_poliza_acontia, $cont, ".$datos_factura['idsegmento'].", ".$datos_factura['idsucursal'].", ".$cp['id_cuenta'].", '$tipo_movto', $importe, '".$datos_factura['uuid']."','". $description ."', 1, DATE_SUB(NOW(), INTERVAL 6 HOUR), '".$datos_factura['xml']."', $forma_pago, $tipo_cambio);");
				 $movs .= $mov.",";
			}
		}
		else
		{
			$this->query("UPDATE cont_polizas SET activo = 0 WHERE id = $id_poliza_acontia");
			$this->query("UPDATE cont_facturas_poliza SET activo = 0 WHERE id_poliza = $id_poliza_acontia");
		}
	...
	function pagaCargo($datos_factura,$json,$movs)
	{
		...
		$this->query("INSERT INTO app_pagos_relacion(id, id_pago, id_tipo, id_documento, cargo, abono, id_poliza_mov, usuario, activo) VALUES (0, $pago, 0, $idcargo, 0, ".$datos_factura['importe'].", '$movs', ".$_SESSION["accelog_idempleado"].", 1);");
	}

	function quitaTemporales($datos_factura)
	{
