...
class EduModel extends Connection
{
	...
	function __construct()
	{
			...
			if($instancia[3] == "webapp")
				$this->connection_edu = mysqli_connect("34.66.63.218","nmdevel","nmdevel","netwarstore");
				
			else
				$this->connection_edu = mysqli_connect("u34.66.63.218","unmdev","&=98+69unmdev","netwarstore");
...
";
		...
		*/
		...
		function esDespacho()
	    {
	        $tipo = $this->query("SELECT tipoinstancia,sincronizar FROM organizaciones WHERE idorganizacion = 1");
	        $tipo = $tipo->fetch_assoc();
	        return $tipo;
	    }


	    function consultas($padre,$hija)
	    {
	    	$padre .= '.';
	    	$hija .= '.';
	    	...
	    	//PRODUCTOS-------------------------------------------------------------------
	    	//Borrar
	    	$myQuery = "TRUNCATE TABLE $hija app_campos_foodware; 
	    				DELETE FROM $hija app_productos; 
	    				ALTER TABLE $hija app_productos AUTO_INCREMENT=1;
	    				TRUNCATE $hija app_producto_caracteristicas;
	    				TRUNCATE $hija app_producto_impuesto;
	    				TRUNCATE $hija app_producto_lotes;
	    				TRUNCATE $hija app_producto_politica;
	    				DELETE FROM $hija mrp_proveedor; 
						ALTER TABLE $hija mrp_proveedor AUTO_INCREMENT=1;
	    				TRUNCATE $hija app_producto_proveedor;
	    				TRUNCATE $hija app_departamento;
	    				TRUNCATE $hija app_familia;
	    				TRUNCATE $hija app_linea;
	    				TRUNCATE $hija app_costos_proveedor;
	    				";
	    	//Agregar
	    	$myQuery .= "INSERT INTO $hija app_productos SELECT* FROM $padre app_productos;
	    				 INSERT INTO $hija app_campos_foodware SELECT* FROM $padre app_campos_foodware;
	    				 INSERT INTO $hija app_producto_caracteristicas SELECT* FROM $padre app_producto_caracteristicas;
	    				 INSERT INTO $hija app_producto_impuesto SELECT* FROM $padre app_producto_impuesto;
	    				 INSERT INTO $hija app_producto_lotes SELECT* FROM $padre app_producto_lotes;
	    				 INSERT INTO $hija app_producto_politica SELECT* FROM $padre app_producto_politica;
	    				 INSERT INTO $hija mrp_proveedor SELECT* FROM $padre mrp_proveedor;
	    				 INSERT INTO $hija app_producto_proveedor SELECT* FROM $padre app_producto_proveedor;
	    				 INSERT INTO $hija app_departamento SELECT* FROM $padre app_departamento;
	    				 INSERT INTO $hija app_familia SELECT* FROM $padre app_familia;
	    				 INSERT INTO $hija app_linea SELECT* FROM $padre app_linea;
	    				 INSERT INTO $hija app_costos_proveedor SELECT* FROM $padre app_costos_proveedor;
	    				 ";

			//CLIENTES-------------------------------------------------------------------
			//Borrar
	    	$myQuery .= "DELETE FROM $hija comun_cliente; 
						 ALTER TABLE $hija comun_cliente AUTO_INCREMENT=1;";
			//Agregar	   
			$myQuery .= "INSERT INTO $hija comun_cliente SELECT* FROM $padre comun_cliente;"; 				 

			//MONEDERO-------------------------------------------------------------------
			//Borrar
	    	//$myQuery .= "TRUNCATE $hija app_politicas_tarjeta;
	    	//			 TRUNCATE $hija tarjeta_regalo;";
			//Agregar	   
			//$myQuery .= "INSERT INTO $hija app_politicas_tarjeta SELECT* FROM $padre app_politicas_tarjeta;
			//			 INSERT INTO $hija tarjeta_regalo SELECT* FROM $padre tarjeta_regalo;"; 				 

			//PROMOCIONES-------------------------------------------------------------------
			//Borrar
	    	$myQuery .= "TRUNCATE $hija com_promociones;
	    				 TRUNCATE $hija com_promocionesXproductos;";
			//Agregar	   
			$myQuery .= "INSERT INTO $hija com_promociones SELECT* FROM $padre com_promociones;
						 INSERT INTO $hija com_promocionesXproductos SELECT* FROM $padre com_promocionesXproductos;"; 				 
	    	
	    	return $myQuery;
	    }

	    function sincronizar($consultas)
	    {
	    	$this->connection_edu->multi_query($consultas);
	    }
