	...
	class ReportsModel extends Connection
	{
	   ...
	   where ((`m`.`Activo` = 1) and (`p`.`activo` = 1) and ((`p`.`idperiodo` < 13) or ((`p`.`idperiodo` = 13) and `a`.`account_id` in (select `cont_accounts`.`account_id` from `cont_accounts` USE INDEX (accounts_account_idx) where (`cont_accounts`.`main_father` = (select `cont_config`.`CuentaSaldos` from `cont_config`)))))))";
		...
		function getData_movcuentas_despues($cuentas, $fecha_antes, $fecha_despues, $rango, $tipo, $saldo, $segmento, $sucursal=null)
		{
			...
			IFNULL((SELECT SUM(mm.Importe) FROM cont_movimientos mm  USE INDEX (movimientos_Cuentasx, movimientos_IdPoliza_idx) INNER JOIN cont_polizas pp ON pp.id = mm.IdPoliza WHERE mm.TipoMovto = 'Abono' AND mm.Cuenta = m.Cuenta AND mm.Activo=1 $segSub $sucSub $ssSub AND ((pp.fecha < p.fecha) OR (pp.id=p.id AND mm.NumMovto < m.NumMovto) OR (pp.fecha = p.fecha AND pp.id < p.id))),0)-
			IFNULL((SELECT SUM(mm.Importe) FROM cont_movimientos mm USE INDEX (movimientos_Cuentasx, movimientos_IdPoliza_idx) INNER JOIN cont_polizas pp ON pp.id = mm.IdPoliza WHERE mm.TipoMovto = 'Cargo' AND mm.Cuenta = m.Cuenta AND mm.Activo=1 $segSub $sucSub $ssSub AND ((pp.fecha < p.fecha) OR (pp.id=p.id AND mm.NumMovto < m.NumMovto) OR (pp.fecha = p.fecha AND pp.id < p.id))),0)
			WHEN 2 THEN
			IFNULL((SELECT SUM(mm.Importe) FROM cont_movimientos mm USE INDEX (movimientos_Cuentasx, movimientos_IdPoliza_idx) INNER JOIN cont_polizas pp ON pp.id = mm.IdPoliza WHERE mm.TipoMovto = 'Cargo' AND mm.Cuenta = m.Cuenta AND mm.Activo=1 $segSub $sucSub $ssSub AND ((pp.fecha < p.fecha) OR (pp.id=p.id AND mm.NumMovto < m.NumMovto) OR (pp.fecha = p.fecha AND pp.id < p.id))),0)-
			IFNULL((SELECT SUM(mm.Importe) FROM cont_movimientos mm USE INDEX (movimientos_Cuentasx, movimientos_IdPoliza_idx) INNER JOIN cont_polizas pp ON pp.id = mm.IdPoliza WHERE mm.TipoMovto = 'Abono' AND mm.Cuenta = m.Cuenta AND mm.Activo=1 $segSub $sucSub $ssSub AND ((pp.fecha < p.fecha) OR (pp.id=p.id AND mm.NumMovto < m.NumMovto) OR (pp.fecha = p.fecha AND pp.id < p.id))),0) END
			) AS SaldoAntes,*/
			...
			FROM
			cont_movimientos m USE INDEX (movimientos_Cuentasx, movimientos_IdPoliza_idx)
		...
		function Saldos($Cuenta,$Fecha,$tipo,$nivel,$segmento,$saldo,$sucursal=null)
		{
			...
			FROM cont_movimientos m USE INDEX (movimientos_Cuentasx, movimientos_IdPoliza_idx)
			INNER JOIN cont_polizas p ON p.id = m.idPoliza
		...
		function generarConsultaA29($ejercicio, $periodo_inicial, $periodo_final, $prov, $proveedor_inicial, $proveedor_final)
		{
			...
			/*0*********************/
			(SELECT SUM(importeBase) FROM cont_rel_pol_prov c INNER JOIN cont_polizas p ON p.id = c.idPoliza WHERE c.activo=1 AND p.activo=1 AND aplica=1 AND idProveedor = p.idPrv AND periodoAcreditamiento BETWEEN $periodo_inicial AND $periodo_final AND ejercicioAcreditamiento = $ejercicio AND (tasa = (SELECT id FROM cont_tasaPrv WHERE tasa='0%' AND visible=1 AND idPrv = p.idPrv ORDER BY id ASC LIMIT 1) OR tasa = (SELECT id FROM cont_tasaPrv WHERE tasa='Exenta' AND visible=1 AND idPrv = p.idPrv ORDER BY id DESC LIMIT 1))) AS p0,

			/*Exenta*********************/
			(SELECT SUM(importeBase) FROM cont_rel_pol_prov c INNER JOIN cont_polizas p ON p.id = c.idPoliza WHERE c.activo=1 AND p.activo=1 AND aplica=1 AND idProveedor = p.idPrv AND periodoAcreditamiento BETWEEN $periodo_inicial AND $periodo_final AND ejercicioAcreditamiento = $ejercicio AND (tasa = (SELECT id FROM cont_tasaPrv WHERE tasa='Exenta' AND visible=1 AND idPrv = p.idPrv ORDER BY id ASC LIMIT 1) OR tasa = (SELECT id FROM cont_tasaPrv WHERE tasa='Exenta' AND visible=1 AND idPrv = p.idPrv ORDER BY id DESC LIMIT 1))) AS ep,
			...
			INNER JOIN cont_rel_pol_prov r ON r.idProveedor = p.idPrv AND r.activo = 1
			INNER JOIN cont_polizas pl ON pl.id = r.idPoliza AND pl.activo = 1
		...
		function generarConsultaA29_2($ejercicio, $periodo_inicial, $periodo_final, $prov, $proveedor_inicial, $proveedor_final)
		{
			if($prov == 'algunos')
			{
				$rangoProv = "AND mp.idPrv BETWEEN $proveedor_inicial AND $proveedor_final";
			}
			...
			$myQuery = "SELECT  
						mp.idPrv,
						mp.idtipotercero,
						SUBSTRING_INDEX((SELECT tipotercero FROM cont_tipo_tercero WHERE id = mp.idtipotercero),'-',1) AS TipoTercero,
						SUBSTRING_INDEX((SELECT tipoOperacion FROM cont_tipo_operacion WHERE id = mp.idtipoperacion),'-',1) AS TipoOperacion,
						mp.razon_social,
						mp.rfc,
						mp.numidfiscal,
						mp.nombrextranjero,
						...
						WHERE r.activo=1 AND r.aplica=1 AND p.activo=1 AND r.periodoAcreditamiento BETWEEN $periodo_inicial AND $periodo_final 
						AND r.ejercicioAcreditamiento = $ejercicio $rangoProv
						AND mp.rfc != '' AND mp.idtipotercero != 7 AND mp.idtipotercero != 3 AND mp.idtipotercero != 4 AND mp.idtipoperacion != 4
						GROUP BY mp.idPrv
						ORDER BY mp.idPrv";
			$c = $this->query($myQuery);
			return $c;			
		...
		function EstadoResultadoxSegmento($ejercicio, $periodo, $sucursal, $segmento, $tipo, $tipoCuenta, $detalle, $idioma)
    {
      ...
      $Grupo = "";
		...
		*/
		...
		function quitaAsociacionXMLPoliza($xml,$idPoliza)
		{
			$uuid = CaptPolizasModel::getUUID($xml);
			$myQuery = "DELETE FROM cont_facturas_poliza WHERE id_poliza = $idPoliza AND uuid LIKE '%$uuid%';";
			$this->query($myQuery);
		}
