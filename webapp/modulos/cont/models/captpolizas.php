...
ORDER BY idtipopoliza,numpol";
		...
		function ActActivo($id)
		{
			...
			//Inactivar las asociaciones de esa poliza con las facturas
			$myQuery = "UPDATE cont_facturas_poliza SET activo=0 WHERE id_poliza=".$id;
			$this->query($myQuery);
			...
			if($Nuevo)
			{
				...
				$myQuery 	= "INSERT INTO cont_movimientos(IdPoliza,NumMovto,IdSegmento,IdSucursal,Cuenta,TipoMovto,Importe,Referencia,Concepto,Activo,FechaCreacion,Factura,MultipleFacturas,FormaPago,tipocambio) VALUES($IdPoliza,$Movto,$Segmento,$Sucursal,$Cuenta,'$TipoMovto',$Importe,'$Referencia','$Concepto',1,DATE_SUB(NOW(), INTERVAL 6 HOUR),'$Factura',$Sel_Multiple,$FormaPago,$tipocambio)";
					...
					if($insert_id = $this->insert_id($myQuery))
					{
						...
						$success = true;
						//if($Factura)
							//$this->guarda_factura_poliza($insert_id,$Factura,0);
		...
		function RestaurarPoliza($IdPoliza,$PDV)
		{
			...
			for($i=0;$i<=count($ids)-1;$i++)
			{
				...
				//Activar nuevamente las relaciones
				$myQuery = "UPDATE cont_facturas_poliza SET activo=1 WHERE id_poliza=".$IdPoliza;
				$this->query($myQuery);
			}
		}
		...
		function EliminarPoliza($IdPoliza)
		{
			...
			$myQuery = "DELETE FROM cont_facturas_poliza WHERE id_poliza = $IdPoliza;";
			$this->query($myQuery);
		}
...
AND m.IdSucursal = $sucursal;";
		...
		function InsertMov2($IdPoliza,$Movto,$segmento,$sucursal,$Cuenta,$TipoMovto,$Importe,$concepto,$persona,$xml,$referencia,$fomapago,$tipopoliza)
		{

			if($referencia == '')
				$referencia = $this->getUUID($xml);
			...
			if($this->query($myQuery))
			{
				$this->guarda_factura_poliza($IdPoliza,$xml,$tipopoliza);
				return true;
			}
			else
			{
		...
		function getUUID($xml)
		{
			if(strpos($xml, '_') === false){
				...
				$xml = explode('_', $xml);
				if(isset($xml[2]) && $xml[2] != '')
					$xml = str_replace('.xml', '', $xml[2]);
				elseif(isset($xml[1]) && $xml[1] != '')
		...
		function guarda_factura_poliza($IdPoliza,$xml,$tipopoliza=0)
		{

			if($tipopoliza == 1 || $tipopoliza == 2)//Si el tipo es de ingresos o egresos 
				$tipopoliza = 1;//lo convierte a poliza de pago o cobro
				
			if($tipopoliza == 3) //Si el tipo es de diario
				$tipopoliza = 0; //lo pasa a poliza de provision

			$xml = $this->getUUID($xml);
			if($xml != '')
			{
				$myQuery = "INSERT INTO cont_facturas_poliza(uuid,tipo_poliza,id_poliza,activo) 
							SELECT * FROM (SELECT '$xml' as xml, $tipopoliza as tipo, $IdPoliza as idpoliza,1 as activo) AS tmp 
							WHERE NOT EXISTS (SELECT uuid FROM cont_facturas_poliza WHERE uuid = '$xml' AND id_poliza = $IdPoliza) LIMIT 1;";
...
where   m.cuenta=c.account_id and  c.currency_id!=1   and m.IdPoliza=p.id and p.idperiodo=".$periodo." and p.idtipopoliza=3 and p.idejercicio=".$ejer." and p.activo=1 and c.account_id=".$idcuenta);
	...
	public function rfcOrganizacion()
	{
		$myQuery = "SELECT rfc AS RFC FROM pvt_configura_facturacion WHERE id = 1";
		$rfc = $this->query($myQuery);
		return $rfc->fetch_assoc();
	}
	...
	function guardaFactura($nombreArchivo,$vars,$pago)
	{
		$myQuery = "INSERT INTO cont_facturas (id, folio, uuid, er, tipo, serie, emisor, receptor, importe, moneda, rfc, fecha, fecha_subida, xml, version, cancelada, json, temporal, origen) 
					SELECT * FROM (SELECT 0 AS 'id', '".$vars['folio']."' AS 'folio', '".$vars['uuid']."' AS 'uuid', '".$vars['er']."' AS 'er', '".$vars['tipo']."' AS 'tipo', '".$vars['serie']."' AS 'serie', '".$vars['emisor']."' AS 'emisor', '".$vars['receptor']."' AS 'receptor', ".$vars['importe']." AS 'importe', '".$vars['moneda']."' AS 'moneda', '".$vars['rfc']."' AS 'rfc', '".$vars['fecha']."' AS 'fecha', DATE_SUB(NOW(), INTERVAL 6 HOUR) AS 'fecha_subida', '".$nombreArchivo."' AS 'xml', ".$vars['version']." AS 'version', ".$vars['cancelada']." AS 'cancelada', '".$vars['json']."' AS 'json', ".$vars['temporal']." AS 'temporal', 1 AS 'origen') AS tmp
                    WHERE NOT EXISTS (SELECT uuid FROM cont_facturas WHERE uuid = '".$vars['uuid']."') LIMIT 1;";
	...
	function listaTemporalesBD($vars)
	{
		if($vars['fechas'] == '')
			...
			$restriccion = '';
			if(isset($vars['tipo_fac']))
				$restriccion = "AND tipo LIKE '".$vars['tipo_fac']."%'";

			return $this->query("SELECT f.* FROM cont_facturas f WHERE $where AND f.temporal = 1 AND f.cancelada = 0 $restriccion;");
	}
	...
	function facturaRename($xml,$temp=0)
	{
		if(strpos($xml, "_") === false)
		...
		{
			$uuid = explode('_', $xml);
			if(isset($uuid[2]))//Si el formato es folio_razon_uuid
				$uuid = str_replace('.xml', '', $uuid[2]);
			elseif(isset($uuid[1]))//Si el formato es -Pago_uuid o -Cobro-uuid
				$uuid = str_replace('.xml', '', $uuid[1]);


		}
		$temporal = '';
		if($temp)//si es true, inactivar de temporal
			$temporal = ', temporal = 0';
		$this->query("UPDATE cont_facturas SET xml = '$xml' $temporal WHERE uuid = '$uuid' ;");
		
	}

	function reactiva_factura($xml)
	{
		if(strpos($xml, "_") === false)
			$uuid = str_replace('.xml', '', $xml);
		else
		...
		$this->query("UPDATE cont_facturas SET temporal = 1 WHERE uuid = '$uuid' ORDER BY id DESC LIMIT 2;");
	}

	function facturasAuto($xml)
	{
		if(strpos($xml, '_') !== false){
			$xml = explode('_',$xml);
			if(isset($xml[2]))
		...
		return $this->query("SELECT folio,IF(er='E',receptor,emisor) AS razon_social, uuid, xml FROM cont_facturas WHERE xml LIKE '%$xml%' AND cancelada = 0 AND temporal = 1 ;");
	}
