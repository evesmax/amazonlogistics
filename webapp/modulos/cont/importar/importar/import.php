...
$intPeriodoPrev = 0;
 ...
 //INICIA INSERCION DE  PROVEEDORES//////////////////
    ...
    for ($i = 2; $i <= intval($data->sheets[1]['numRows']); $i++)
    {
            ...
            $strProv14 = trim($data->sheets[1]["cells"][$i][13]);//id tipo iva
            $strProv15 = trim($data->sheets[1]["cells"][$i][14]);//id ietu
            ...
            $strSql = "INSERT INTO mrp_proveedor VALUES(0,'$strProv02','$strProv03','','','','',0,'1','1',0,0,0,$strProv05,$strProv06,'$strProv04',$strProv07,'$strProv08','$strProv09',0,'$strProv10',$strProv11,$strProv12,0,$strProv14,$strProv15,1)";
            mysqli_query($objCon,$strSql);
    ...
    for ($i = 2; $i <= intval($data->sheets[2]['numRows']); $i++)
    {
        ...
        $strSql = "INSERT INTO cont_segmentos VALUES(0,'$strSeg02','$strSeg01')";
        mysqli_query($objCon,$strSql);
        $strSql = '';
    }
...
for ($i = 2; $i <= $data->sheets[0]['numRows']; $i++)
{
    ...
    if($Tipo=="P")
    {
        ...
        $strSql = "INSERT INTO cont_polizas (id,idorganizacion,idejercicio,idperiodo,numpol,idtipopoliza,referencia,concepto,cargos,abonos,ajuste,fecha,fecha_creacion,activo,eliminado,pdv_aut,relacionExt,beneficiario,numero,rfc,idbanco,numtarjcuent) VALUES (0," . $Organizacion . "," . $Ejercicio . "," . $Periodo . "," . $numPol . ","  . $TipoPoliza . ",\"$Referencia\",\"$Concepto\",'" . $Cargos . "','" . $Abonos . "','" . $Ajuste . "','" . $Fecha . "', '" . $TimeStamp . "', " . $Activo . "," . $Eliminado ."," . $pdv_aut . ",0,0,0,NULL,0,0);";
