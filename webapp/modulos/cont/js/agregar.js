...
//-------------------------------------------------------------
	...
	// INICIA GENERACION DE BUSQUEDA
			$("#buscar").bind("keyup", function(evt){
                ...
                if(($(this).val().trim().length) < 36){
                    return;
                }
                if(($(this).val().trim().length) > 36){
                    var data = ($(this).val().trim());
                    data = data.split(',');
                    $("#lista tr:not(:containsIN('--'))").css('display','none');
                    for(var x = 0; x < data.length; x ++){
                        if(evt.type == 'keyup' && data[x].length == 36)
                        {
                            $("#lista tr:containsIN('"+data[x]+"')").css('display','table-row');
                ...
                var cargos = sum_class('cargos');
                var abonos = sum_class('abonos');
                cargos = new Intl.NumberFormat().format(cargos);
                abonos = new Intl.NumberFormat().format(abonos);
                $("#data_cargos").val(cargos);
                $("#data_abonos").val(abonos);
                $('#info').css('display','block');
            });
		// TERMINA GENERACION DE BUSQUEDA
function sum_class(name_class){
    var cargos = $("."+name_class+":visible");
    var sum_value = 0;
    for(var x = 0; x < cargos.length; x ++){
        var mystring = cargos[x].textContent;
        mystring = mystring.split('$').join('');
        mystring = mystring.split(',').join('');
        sum_value += parseFloat(mystring);
    }
    return sum_value;
}
 actualizaListaFac()
