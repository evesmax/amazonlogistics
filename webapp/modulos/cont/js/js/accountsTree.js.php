		...
		//INICIA OBTENCION DE ARBOL CONTABLE
			...
			if (jsonTree.length > 0)
			{
				for ( var i = 0 ; i < jsonTree.length ; i++ )
				{
					...
					contentLi += "<img /><span>( " + jsonTree[i].manual_code + " ) " + jsonTree[i].description + "</span><ul></ul></li>";
				...
				switch($(this).attr("class"))
				{
					...
					case 'reg':
						...
						else
						{
							...
							$(".new > img").after("<span>( "+number+" ) " +name+"</span>");
...
function setTypeByName()
{
	...
	switch(typeName)
	{
		...
		case 'PASIVO': break;
		case 'CAPITAL': break;
		case 'INGRESOS': typeName = 'RESULTADOS';break;
		case 'EGRESOS': typeName = 'RESULTADOS';break;
		default: 
		if(typeName.search('RESULTADO') != -1)
		{
			typeName = 'RESULTADOS'
		}
		else
