...
// 							
	...
	function visualizaXMLalmacen(){
		...
		for($i=0;$i<=$maximo;$i++){
			...
			if(file_exists($_POST['xml'][$i])){
				$xml = simplexml_load_file($_POST['xml'][$i]);
			}elseif(file_exists($_POST['xml'][$i] . "-Nomina__")){
				rename($_POST['xml'][$i] . "-Nomina__",$_POST['xml'][$i]);
				$xml = simplexml_load_file($_POST['xml'][$i]);
			}elseif(file_exists($_POST['xml'][$i] . "_")){
				rename($_POST['xml'][$i] . "_",$_POST['xml'][$i]);
				$xml = simplexml_load_file($_POST['xml'][$i]);
			}
			...
			{
				$infoFac = $this->NominaModel->buscaFacInfo($xmlreal);

				$separa[1] 	=	$infoFac['receptor'];
				$separa[2] 	=	$infoFac['uuid'] . '.xml';

				$xmld		=	$this->quitar_tildes("-Nomina_".$separa[2]);
			}
			else
			{
	...
	function creaProvisionNomina(){
		...
		foreach($_SESSION['provisionNomina'] as $data){
	 		...
	 		foreach ($data['percepciones'] as $percepcion){
	 			if($abono==true){
					...
					if($percepcion['cuenta']!="016"){
						$cuenta = $clave[2];
						$abono = $this->CaptPolizasModel->InsertMov2($numPoliza['id'],$numov,$segmento,$sucursal,$cuenta,"Cargo",$percepcion['importe'],$percepcion['concepto'],'3-',$xml,"$referencia",0,3);
						$numov++;
					}else{
				...
				foreach ($data['deducciones'] as $deduccion){
					if($cargo==true){
						$dedu = $deduccionesarray[$deduccion['cuenta']];
			 			$clave = explode("//",$dedu);
						/*se comento la linea para q cubria otras dedu */
						//if($deduccion['cuenta']!="004"){
			 				$cuenta = $clave[2];
							$cargo = $this->CaptPolizasModel->InsertMov2($numPoliza['id'],$numov,$segmento,$sucursal,$cuenta,"Abono",$deduccion['importe'],$deduccion['concepto'],'3-',$xml,"$referencia",0,3);
							$numov++;
						//}
			...
			if($error==false){
				$cargo = $this->CaptPolizasModel->InsertMov2($numPoliza['id'],$numov,$segmento,$sucursal,$cuenta,"Abono",$data['total'],"$conceptopoliza",'1-',$xml,"$referencia",0,3);
				$nombre = $data['nombre'][1];
				
				if($cargo==true){
					$numov++;
					copy($this->path()."xmls/facturas/temporales/".$xmlreal, $this->path()."xmls/facturas/".$numPoliza['id']."/".$xml);
					rename($this->path()."xmls/facturas/temporales/".$xmlreal, $this->path()."xmls/facturas/temporales/".$xml);
					$this->CaptPolizasModel->facturaRename($xml);
				}else{
					$error==true;
				}
			}
	...
	function visualizaPagoXML(){
		...
		for($i=0;$i<=$maximo;$i++){
			...
			$xp = new DOMXpath($xml);
			if($this->getpath("//cfdi:Comprobante/@version"))//Si la factura es 3.2
			{
				$data['folio'] 	= $this->getpath("//@folio");
				$data['nombre'] = $this->getpath("//@nombre");
				$data['total'] = $this->getpath("//@total");//el total sera el monto complementario
			...
			if($this->getpath("//cfdi:Comprobante/@Version"))//Si la factura es 3.3
			{
				$data['folio'] 	= $this->getpath("//@Folio");
				$data['nombre'] = $this->getpath("//@Nombre");
				$data['total'] = $this->getpath("//@Total");//el total sera el monto complementario
	...
	function creaPagoNomina(){
		...
		foreach($_SESSION['pagonomina'] as $data){
			...
			$cargo = $this->CaptPolizasModel->InsertMov2($numPoliza['id'],$numov,$segmento,$sucursal,$cuenta,"Cargo",number_format(floatval($data['total']),2,'.',''),"$conceptopoliza",'3-'.$_REQUEST['beneficiario'],$xml,"$referencia",$_REQUEST['formapago'],2);
			if($cargo==true){
				$numov ++;
				$abono = $this->CaptPolizasModel->InsertMov2($numPoliza['id'],$numov,$segmento,$sucursal,$data['banco'],"Abono",number_format(floatval($data['total']),2,'.',''),"$conceptopoliza",'3-'.$_REQUEST['beneficiario'],$xml,"$referencia",$_REQUEST['formapago'],2);
			}else{
				$error=true;
			}
			if($abono==true){
