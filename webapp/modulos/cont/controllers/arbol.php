...
class Arbol extends Common
{
	...
	function index()
	{
			...
			$habilitado = $this->ArbolModel->gethabilitado();
	...
	function subcuentas()
	{
		...
		if(!$this->ArbolModel->titulo())
			$titulo = "AND account_code != '1' AND account_code != '2' AND account_code != '4'";


		$subcuentade = $this->ArbolModel->fillSelect("account_id","CONCAT('( ',manual_code,' ) ',description)","cont_accounts","main_account != 3 AND removed = 0 $where $titulo ORDER BY account_code");
		echo $subcuentade;
	}
	...
	function habilitar()
	{
		...
		if($this->ArbolModel->habilitar($_POST['si'])){
			if(intval($_POST['si']))
				$mensaje = "Se habilitó correctamente";
			else
				$mensaje = "Se deshabilitó correctamente";
			$id = 1;

		}
		echo json_encode(array('id' => $id, 'mensaje' => $mensaje ));
	}
