...
class CaptPolizas extends Common
{
		...
		while($Mov = $NumMovs->fetch_assoc())
				{
					...
					{
						$CargosAbonos = "<td title='Cargos' class='cargos'>$".number_format($Mov['Importe'],3)."</td><td title='Abonos' class='abonos'>$0.000</td>";
					}
					if($Mov['TipoMovto'] == "Abono")
					{
						$CargosAbonos = "<td title='Cargos' class='cargos'>$0.000</td><td title='Abonos' class='abonos'>$".number_format($Mov['Importe'],3)."</td>";
					}

					if($Mov['TipoMovto'] == "Cargo M.E.")
					{
						$CargosAbonos = "<td title='Cargos' class='cargos' style='background: #585858;color: #FAFAFA'>".$codigomoneda." ".number_format($Mov['Importe'],3)."</td><td title='Abonos' class='abonos' style='background: #585858;color: #FAFAFA'>".$codigomoneda." 0.00</td>";
					}
					if($Mov['TipoMovto'] == "Abono M.E")
					{
						$CargosAbonos = "<td title='Cargos' class='cargos' style='background: #585858;color: #FAFAFA'>".$codigomoneda." 0.00</td><td title='Abonos' class='abonos' style='background: #585858;color: #FAFAFA'>".$codigomoneda." ".number_format($Mov['Importe'],3,'.','')."</td>";
					}
	...
	function GetPolizas()
	{
		...
		$confirm = $this->CaptPolizasModel->confirm();
		$bancos = $this->CaptPolizasModel->validaBancos();
	...
	function verpolicli()
	{
		...
		if($p=$periodo->fetch_array()){
			...
			if($p['PeriodoActual']!=13){
				...
				$lastExercise = $this->CaptPolizasModel->getFirstLastExercise(1);
				$Suc = $this->CaptPolizasModel->getSegmentoInfo();
				$facturasAuto = $this->CaptPolizasModel->facturasAuto('Cobro');
	...
	function verpoliprove()
	{
		...
		if($p=$periodo->fetch_array()){
			...
			if($p['PeriodoActual']!=13){
				...
				$Suc = $this->CaptPolizasModel->getSegmentoInfo();
				$listacuentasbancarias = $this->CaptPolizasModel->cuentasbancariaslista();
				$facturasAuto = $this->CaptPolizasModel->facturasAuto('Pago');
				//bet
				if($sql->num_rows>0){
					$bancos=$sql;
				}else{
...
//
	...
	function guarda(){
		...
		//pasar los valores a insertar
			$tipopoliza = 1;
			$poli=$this->CaptPolizasModel->savePoliza2($idorg,$idejer,$idperio,$tipopoliza,'Cobro a cliente',$fecha,0,$_REQUEST['numeroforma'],"",0,"",0,0);
			if($poli==0){
				$numPoliza = $this->CaptPolizasModel->getLastNumPoliza();
				...
				$UltimoMov=1;
					foreach($_SESSION['tabla'] as $cli){
						foreach($cli as $cliente){
							...
							$abono=$this->CaptPolizasModel->InsertMov2($numPoliza['id'],$UltimoMov,$segment[0],$sucu[0],$idcuenta,"Abono",number_format($cliente['importe'],2,'.',''),$cliente['concepto'],'1-'.$ccliente,$xmlrelacion,$referencia,$cliente['formapago'],$tipopoliza);
							 if($abono==true){
							 	if($unsolobanco==0){
							 		$UltimoMov+=1;
							 		$si=$this->CaptPolizasModel->InsertMov2($numPoliza['id'],$UltimoMov,$segment[0],$sucu[0],$cbanco,"Cargo",number_format($cliente['importe'],2,'.',''),$cliente['concepto'],'1-'.$ccliente,$xmlrelacion,$referencia,$cliente['formapago'],$tipopoliza);
							 	}
								else{
									$si = true;
								...
								if($statusIVA==1){
							 		if($cliente['IVA']>0){
							 			...
							 			$insertaivapendiente = 	$this->CaptPolizasModel->InsertMov2($numPoliza['id'],$UltimoMov,$segment[0],$sucu[0],$ivapendientecobro[0],"Cargo",number_format($cliente['IVA'],2,'.',''),$cliente['concepto'],'1-'.$ccliente,$xmlrelacion,$referencia,$cliente['formapago'],$tipopoliza);
										$UltimoMov+=1;
										$insertaivacobrado = $this->CaptPolizasModel->InsertMov2($numPoliza['id'],$UltimoMov,$segment[0],$sucu[0],$ivacobrado[0],"Abono",number_format($cliente['IVA'],2,'.',''),$cliente['concepto'],'1-'.$ccliente,$xmlrelacion,$referencia,$cliente['formapago'],$tipopoliza);
										$UltimoMov+=1;
									}
								}
								if($statusIEPS==1){
									if($cliente['IEPS']>0){//
							 			$insertaivapendiente = 	$this->CaptPolizasModel->InsertMov2($numPoliza['id'],$UltimoMov,$segment[0],$sucu[0],$iepspendientecobro[0],"Cargo",number_format($cliente['IEPS'],2,'.',''),$cliente['concepto'],'1-'.$ccliente,$xmlrelacion,$referencia,$cliente['formapago'],$tipopoliza);
										$UltimoMov+=1;
										$insertaivacobrado = $this->CaptPolizasModel->InsertMov2($numPoliza['id'],$UltimoMov,$segment[0],$sucu[0],$iepscobrado[0],"Abono",number_format($cliente['IEPS'],2,'.',''),$cliente['concepto'],'1-'.$ccliente,$xmlrelacion,$referencia,$cliente['formapago'],$tipopoliza);
										$UltimoMov+=1;
									}
								}
						 		if($si==false){
						 			 ...
						 			 if($carpeta==true){
										 ...
										 //str_replace("Parcial", "", $cliente['xml'])
						 			 	if($cliente['parcial']==0){//cobro
											if(rename($rutaOrigen, $this->path()."xmls/facturas/".$numPoliza['id']."/".$xmlrelacion ))
												$this->CaptPolizasModel->facturaRename($cliente['xml'],1);
											unlink($rutaOrigen);
										}
										else{
											copy($rutaOrigen, $this->path()."xmls/facturas/".$numPoliza['id']."/".$xmlrelacion );
											if(rename($rutaOrigen, $this->path()."xmls/facturas/temporales/Parcial-".$xmlrelacion))
					...
					if($unsolobanco==1){
				 		$si=$this->CaptPolizasModel->InsertMov2($numPoliza['id'],$UltimoMov,$segment[0],$sucu[0],$cbanco,"Cargo",number_format($totalbancos,2,'.',''),$conceptoBancos,'1-'.$ccliente,$xmlrelacion,$referencia,$cliente['formapago'],$tipopoliza);
				 	}
...
function guardaprove(){
		...
		$carpeta=false;
			...
			$tipopoliza = 2;
			$poli=$this->CaptPolizasModel->savePoliza2($idorg,$idejer,$idperio,$tipopoliza,'Pago a proveedores',$fecha,$beneficiario,$numero,$rfc,$idbanco,$numtarjcuent,$bancoorigen,1);
			if($poli==0){
				...
				$numPoliza = $this->CaptPolizasModel->getLastNumPoliza();
					...
					foreach($_SESSION['proveedor'] as $cli){
						foreach($cli as $prove){
							...
							if(strpos($prove['xml'], '_') !== false){
								$separa=explode('_',$prove['xml']);
								if(isset($separa[2]))
									$referencia=str_replace(".xml"," ",$separa[2]);
								elseif(isset($separa[1]))
									$referencia=str_replace(".xml"," ",$separa[1]);
							}
							else
								$referencia=str_replace(".xml"," ",$prove['xml']);
							...
							if($prove['concepto']==""){ $prove['concepto']="Pago a Proveedores";}
							 $abono=$this->CaptPolizasModel->InsertMov2($numPoliza['id'],$UltimoMov,$segment[0],$sucu[0],$idcuenta,"Cargo",number_format($prove['importe'],2,'.',''),$prove['concepto'],'2-'.$cprovee,$xmlrelacion,$referencia,$formapago,$tipopoliza);
							if($abono==true){
								if($unsolobanco==0){
									$UltimoMov+=1;
								  	$si=$this->CaptPolizasModel->InsertMov2($numPoliza['id'],$UltimoMov,$segment[0],$sucu[0],$cbanco,"Abono",number_format($prove['importe'],2,'.',''),$prove['concepto'],'2-'.$cprovee,$xmlrelacion,$referencia,$formapago,$tipopoliza);
								}else{
									...
									if($si==true){ $UltimoMov+=1;
										...
										if($statusIVA==1){
								 			if($prove['IVA']>0){
								 				$insertaivapendiente = 	$this->CaptPolizasModel->InsertMov2($numPoliza['id'],$UltimoMov,$segment[0],$sucu[0],$ivapendientepago[0],"Abono",number_format($prove['IVA'],2,'.',''),$prove['concepto'],'2-'.$cprovee,$xmlrelacion,$referencia,$formapago,$tipopoliza);
												$UltimoMov+=1;
												$insertaivapagado = $this->CaptPolizasModel->InsertMov2($numPoliza['id'],$UltimoMov,$segment[0],$sucu[0],$CuentaIVApagado[0],"Cargo",number_format($prove['IVA'],2,'.',''),$prove['concepto'],'2-'.$cprovee,$xmlrelacion,$referencia,$formapago,$tipopoliza);
												$UltimoMov+=1;
											}
										}
										if($statusIEPS==1){
											if($prove['IEPS']>0){//
									 			$insertaivapendiente = 	$this->CaptPolizasModel->InsertMov2($numPoliza['id'],$UltimoMov,$segment[0],$sucu[0],$iepspendientepago[0],"Abono",number_format($prove['IEPS'],2,'.',''),$prove['concepto'],'2-'.$cprovee,$xmlrelacion,$referencia,$formapago,$tipopoliza);
												$UltimoMov+=1;
												$insertaivapagado = $this->CaptPolizasModel->InsertMov2($numPoliza['id'],$UltimoMov,$segment[0],$sucu[0],$CuentaIEPSpagado[0],"Cargo",number_format($prove['IEPS'],2,'.',''),$prove['concepto'],'2-'.$cprovee,$xmlrelacion,$referencia,$formapago,$tipopoliza);
												$UltimoMov+=1;
											}
										}
							 		}
									 if($si==false){
							 			...
							 			if($carpeta==true){
							 				...
							 				if($prove['parcial']==0){
												...
												if(rename($rutaOrigen, $this->path()."xmls/facturas/".$numPoliza['id']."/".$xmlrelacion))
													$this->CaptPolizasModel->facturaRename($prove['xml'],1);
												unlink($rutaOrigen);
											}
											else{
												copy($rutaOrigen, $this->path()."xmls/facturas/".$numPoliza['id']."/".$xmlrelacion);
					...
					if($unsolobanco==1){
						$si=$this->CaptPolizasModel->InsertMov2($numPoliza['id'],$UltimoMov,$segment[0],$sucu[0],$cbanco,"Abono",number_format($totalbancos,2,'.',''),$conceptoBancos,'2-'.$cprovee,$xmlrelacion,$referencia,$formapago,$tipopoliza);
					}
    ...
    function actualizasegmentos(){
        $ListaSegmentos = $this->CaptPolizasModel->ListaSegmentos();
        while($li = $ListaSegmentos->fetch_assoc()){
            echo "<option value=".$li['idSuc'].">".$li['nombre']."</option>";
        }
    }
    function actualizasucursal(){
        $ListaSucursales =	$this->CaptPolizasModel->ListaSucursales();
        while($li = $ListaSucursales->fetch_assoc()){
            echo "<option value=".$li['idSuc'].">".$li['nombre']."</option>";
        }
    }
  ...
  // ini_set('display_errors', '1');
    ...
    ini_set('display_errors', '1');
		...
		for($i=0;$i<=$maximo;$i++){
			...
			$texto = preg_replace('{<Addenda.*/Addenda>}is', '', $texto);
            $texto = preg_replace('{<cfdi:Addenda.*/cfdi:Addenda>}is', '', $texto);
            $texto = preg_replace('{<implocal:TrasladosLocales.*//>}is', '', $texto);
            $texto = preg_replace('{<implocal:ImpuestosLocales.*/implocal:ImpuestosLocales>}is', '', $texto); 
			...
			if(strpos($archivopar[3], '_') === false)
			{
				if($comprobante==1){
					$xmld=$this->quitar_tildes("-Cobro_".$archivopar[3]);
				}
				if($comprobante==2){
					$xmld=$this->quitar_tildes("-Pago_".$archivopar[3]);
				}
			}
			else
			{
...
// 	//////////////////////////////////////////////////////
		if( $_SESSION['comprobante'] == 1 || $_SESSION['comprobante'] == 3 ){// si es ingresos el cliente sera el receptor
			...
			////////////////////////////
			$tipopoliza = 3;
			$poli = $this->CaptPolizasModel->savePoliza2($idorg,$idejer,$idperio,$tipopoliza,$conceptopoliza,$fecha,0,"","",0,"",0,0);
			if($poli == 0){
				$numPoliza = $this->CaptPolizasModel->getLastNumPoliza();
				if( mkdir($this->path()."xmls/facturas/".$numPoliza['id'],  0777)){
					$carpeta = true;
				...
				foreach($cli as $cliente){
					...
					if($_REQUEST['detalle']){
						...
						for($c=0;$c<=($maximo);$c++){
							...
							$abono = $this->CaptPolizasModel->InsertMov2($numPoliza['id'],$numov,$segmento,$sucursal,$cliente['cuentadetalle'][$c],"Abono",number_format($cliente['preciodetalle'][$c],2,'.',''),$cliente['conceptodetalle'][$c],'1-',$cliente['xml'],"$referencia",0,$tipopoliza);
							$numov++;
						}
					}else{
						...
						if($cliente['hayProrrateo']==0){
							...
							$abono = $this->CaptPolizasModel->InsertMov2($numPoliza['id'],$numov,$segmento,$sucursal,$cliente['cuentacompraventa'][0],$abonocomprobante,number_format($cliente['abono'],2,'.',''),$cliente['concepto'][0],'1-',$cliente['xml'],"$referencia",0,$tipopoliza);
							$numov++;
						}else{
							$p=0;
							...
							for($p;$p<=($maximo);$p++){
								...
								$abono = $this->CaptPolizasModel->InsertMov2($numPoliza['id'],$numov,$cliente['segmento'][$p],$cliente['sucursal'][$p],$cliente['cuentacompraventa'][$p],$abonocomprobante,number_format($monto,2,'.',''),$cliente['concepto'][$p],'1-',$cliente['xml'],"$referencia",0,$tipopoliza);
								$numov++;
							}
							$cliente['concepto'][0]="-";
						}
					...
					if( $abono == true ){
						...
						if($cliente['hayProrrateo']==0){
							...
							if( $cliente['ieps'] > 0 ){
								$insertaieps = $this->CaptPolizasModel->InsertMov2($numPoliza['id'],$numov,$segmento,$sucursal,$iepspendientecobro[0],$abonocomprobante,number_format($cliente['ieps'],2,'.',''),$cliente['concepto'][0],'1-',$cliente['xml'],"$referencia",0,$tipopoliza);
								$numov++;
							}
							if( $cliente['ish'] > 0){
								$insertish = $this->CaptPolizasModel->InsertMov2($numPoliza['id'],$numov,$segmento,$sucursal,$ish[0],$abonocomprobante,number_format($cliente['ish'],2,'.',''),$cliente['concepto'][0],'1-',$cliente['xml'],"$referencia",0,$tipopoliza);
								$numov++;
							}
						}else{
							if( $cliente['ieps'] > 0 ){
								$insertaieps = $this->CaptPolizasModel->InsertMov2($numPoliza['id'],$numov,$cliente['segmento'][$p],$cliente['sucursal'][$p],$iepspendientecobro[0],$abonocomprobante,number_format($cliente['ieps'],2,'.',''),$cliente['concepto'][0],'1-',$cliente['xml'],"$referencia",0,$tipopoliza);
								$numov++; $p++;
							}
							if( $cliente['ish'] > 0){
								$insertish = $this->CaptPolizasModel->InsertMov2($numPoliza['id'],$numov,$cliente['segmento'][$p],$cliente['sucursal'][$p],$ish[0],$abonocomprobante,number_format($cliente['ish'],2,'.',''),$cliente['concepto'][0],'1-',$cliente['xml'],"$referencia",0,$tipopoliza);
								$numov++; $p++;
							}
						}
						//nombre
						if($cliente['hayProrrateo']==0){
							if( $cliente['abono2'] > 0){//$ivaingre=$_REQUEST['ivaingre'];
								$abono2 = $this->CaptPolizasModel->InsertMov2($numPoliza['id'],$numov,$segmento,$sucursal,$ivapendientecobro[0],$abonocomprobante,number_format($cliente['abono2'],2,'.',''),$cliente['concepto'][0],'1-',$cliente['xml'],"$referencia",0,$tipopoliza);
								$numov++;
							}
						}else{
							if( $cliente['abono2'] > 0){//$ivaingre=$_REQUEST['ivaingre'];
								$abono2 = $this->CaptPolizasModel->InsertMov2($numPoliza['id'],$numov,$cliente['segmento'][$p],$cliente['sucursal'][$p],$ivapendientecobro[0],$abonocomprobante,number_format($cliente['abono2'],2,'.',''),'-','1-',$cliente['xml'],"$referencia",0,$tipopoliza);
								$numov++; $p++;
							}
						}
						if($cliente['hayProrrateo']==0){
							$si = $this->CaptPolizasModel->InsertMov2($numPoliza['id'],$numov,$segmento,$sucursal,$idcuentacliente,$cargocomprobante,number_format($cliente['cargo'],2,'.',''),$cliente['concepto'][0],'1-'.$idreceptor,$cliente['xml'],"$referencia",0,$tipopoliza);
							$numov++;
						}else{
							$si = $this->CaptPolizasModel->InsertMov2($numPoliza['id'],$numov,$cliente['segmento'][$p],$cliente['sucursal'][$p],$idcuentacliente,$cargocomprobante,number_format($cliente['cargo'],2,'.',''),'-','1-'.$idreceptor,$cliente['xml'],"$referencia",0,$tipopoliza);
							$numov++; $p++;
						}
						if( $si != false ){
				 	  	//////retencion/////
							foreach ( $cliente['retenidos'] as $key => $value){
								if($cliente['hayProrrateo']==1){
						 	  	...
						 	  	if( $key == "ISR"){
						 	  		if( $value > 0 ){
						 	  			$si = $this->CaptPolizasModel->InsertMov2($numPoliza['id'],$numov,$segmento,$sucursal,$isrretenido[0],$cargocomprobante,number_format($value,2,'.',''),$cliente['concepto'][0],'1-',$cliente['xml'],"$referencia",0,$tipopoliza);
						 	  			$numov++;
									}
								}
								if( $key == "IVA" ){
						 	  		if( $value > 0 ){
						 	  		 	$si = $this->CaptPolizasModel->InsertMov2($numPoliza['id'],$numov,$segmento,$sucursal,$ivaretenido[0],$cargocomprobante,number_format($value,2,'.',''),$cliente['concepto'][0],'1-',$cliente['xml'],"$referencia",0,$tipopoliza);
										$numov++;
									}
								}
							}
					 ...
					 if($error==false){
						if($carpeta==true){
							...
							if($_SESSION['comprobante'] == 3){
								if(rename($this->path()."xmls/facturas/temporales/".$cliente['xmlreal'], $this->path()."xmls/facturas/".$numPoliza['id']."/".$cliente['xml']))
									$rename = 1;
							}else{
								copy($this->path()."xmls/facturas/temporales/".$cliente['xmlreal'], $this->path()."xmls/facturas/".$numPoliza['id']."/".$cliente['xml']);
								if(rename($this->path()."xmls/facturas/temporales/".$cliente['xmlreal'], $this->path()."xmls/facturas/temporales/".$cliente['xml']))
									$rename = 1;
								//unlink($this->path()."xmls/facturas/temporales/".$cliente['xmlreal']);//Borrar de temporales
							}
							if($rename == 1)
			...
			foreach( $_SESSION['poliprove'] as $cli){
				foreach($cli as $prove){
					...
					if($_REQUEST['detalle']){
						...
						$maximo = (intval($maximo)-1);
						for($c=0;$c<=($maximo);$c++){
							$cargo=$this->CaptPolizasModel->InsertMov2($numPoliza['id'],$numov,$segmento,$sucursal,$prove['cuentadetalle'][$c],"Cargo",number_format($prove['preciodetalle'][$c],2,'.',''),$prove['conceptodetalle'][$c],'2-',$prove['xml'],"$referencia",9,$tipopoliza);
							$numov++;
						}
					}else{
						if($prove['hayProrrateo']==0){
							$cargo=$this->CaptPolizasModel->InsertMov2($numPoliza['id'],$numov,$segmento,$sucursal,$prove['cuentacompraventa'][0],$cargocomprobante,number_format($prove['cargo'],2,'.',''),$prove['concepto'][0],'2-',$prove['xml'],"$referencia",9,$tipopoliza);
							$numov++;
						}else{
							$p=0;
							...
							for($p;$p<=($maximo);$p++){
								...
								$cargo=$this->CaptPolizasModel->InsertMov2($numPoliza['id'],$numov,$prove['segmento'][$p],$prove['sucursal'][$p],$prove['cuentacompraventa'][$p],$cargocomprobante,number_format($monto,2,'.',''),$prove['concepto'][$p],'2-',$prove['xml'],"$referencia",9,$tipopoliza);
								$numov++;
							}
							$prove['concepto'][0]="-";
						}
					...
					if($cargo==true){
						...
						if($prove['hayProrrateo']==0){
							if($prove['ieps']>0){
								$insertiepss=$this->CaptPolizasModel->InsertMov2($numPoliza['id'],$numov,$segmento,$sucursal,$iepspendientepago[0],$cargocomprobante,number_format($prove['ieps'],2,'.',''),$prove['concepto'][0],'2-',$prove['xml'],"$referencia",9,$tipopoliza);
								$numov++;
							}
							if($prove['ish']>0){
								$ishinsert=$this->CaptPolizasModel->InsertMov2($numPoliza['id'],$numov,$segmento,$sucursal,$ish[0],$cargocomprobante,number_format($prove['ish'],2,'.',''),$prove['concepto'][0],'2-',$prove['xml'],"$referencia",0,$tipopoliza);
								$numov++;
							}
							if($prove['cargo2']>0){
								$cargo2=$this->CaptPolizasModel->InsertMov2($numPoliza['id'],$numov,$segmento,$sucursal,$ivapendientepago[0],$cargocomprobante,number_format($prove['cargo2'],2,'.',''),$prove['concepto'][0],'2-',$prove['xml'],"$referencia",9,$tipopoliza);
								$numov++;
							}
						}else{
							if($prove['ieps']>0){
								$insertiepss=$this->CaptPolizasModel->InsertMov2($numPoliza['id'],$numov,$prove['segmento'][$p],$prove['sucursal'][$p],$iepspendientepago[0],$cargocomprobante,number_format($prove['ieps'],2,'.',''),$prove['concepto'][0],'2-',$prove['xml'],"$referencia",9,$tipopoliza);
								$numov++; $p++;
							}
							if($prove['ish']>0){
								$ishinsert=$this->CaptPolizasModel->InsertMov2($numPoliza['id'],$numov,$prove['segmento'][$p],$prove['sucursal'][$p],$ish[0],$cargocomprobante,number_format($prove['ish'],2,'.',''),$prove['concepto'][0],'2-',$prove['xml'],"$referencia",0,$tipopoliza);
								$numov++; $p++;
							}
							if($prove['cargo2']>0){
								$cargo2=$this->CaptPolizasModel->InsertMov2($numPoliza['id'],$numov,$prove['segmento'][$p],$prove['sucursal'][$p],$ivapendientepago[0],$cargocomprobante,number_format($prove['cargo2'],2,'.',''),$prove['concepto'][0],'2-',$prove['xml'],"$referencia",9,$tipopoliza);
								$numov++; $p++;
							}
						}
						if($prove['hayProrrateo']==0){
					  		$si=$this->CaptPolizasModel->InsertMov2($numPoliza['id'],$numov,$segmento,$sucursal,$idcuentaprove,$abonocomprobante,number_format($prove['abono'],2,'.',''),$prove['concepto'][0],'2-'.$idemisor,$prove['xml'],"$referencia",9,$tipopoliza);
						}else{
							$si=$this->CaptPolizasModel->InsertMov2($numPoliza['id'],$numov,$prove['segmento'][$p],$prove['sucursal'][$p],$idcuentaprove,$abonocomprobante,number_format($prove['abono'],2,'.',''),$prove['concepto'][0],'2-'.$idemisor,$prove['xml'],"$referencia",9,$tipopoliza);
							$p++;
						}
				 	 	if($si!=false){
				 	  		$numov++;
				 	  		foreach ( $prove['retenidos'] as $key => $value){
					 	  		...
					 	  		if($key=="ISR"){
					 	  			if($value > 0){
					 	  				$si=$this->CaptPolizasModel->InsertMov2($numPoliza['id'],$numov,$segmento,$sucursal,$isrretenido[0],$abonocomprobante,number_format($value,2,'.',''),$prove['concepto'][0],'2-',$prove['xml'],"$referencia",9,$tipopoliza);
					 	  				$numov++;
									}
								}
								if($key == "IVA"){
									if($value > 0){
					 	  				$si=$this->CaptPolizasModel->InsertMov2($numPoliza['id'],$numov,$segmento,$sucursal,$ivaretenido[0],$abonocomprobante,number_format($value,2,'.',''),$prove['concepto'][0],'2-',$prove['xml'],"$referencia",9,$tipopoliza);
					 	  				$numov++;
									}
								}
							}
					...
					if($error==false){
						if($carpeta==true){//$archivopar[3]
							if($_SESSION['comprobante'] == 4){
								if(rename($this->path()."xmls/facturas/temporales/".$prove['xmlreal'], $this->path()."xmls/facturas/".$numPoliza['id']."/".$prove['xml']))
									$rename = 1;
							}else{
								copy($this->path()."xmls/facturas/temporales/".$prove['xmlreal'], $this->path()."xmls/facturas/".$numPoliza['id']."/".$prove['xml']);
								if(rename($this->path()."xmls/facturas/temporales/".$prove['xmlreal'], $this->path()."xmls/facturas/temporales/".$prove['xml']))
									$rename = 1;
								//unlink($this->path()."xmls/facturas/temporales/".$prove['xmlreal']);//Borrar de temporales
							}
							if($rename == 1)
...
// 
			...
			$_SESSION['compruebagasto'][]=$previo;
			if(rename($_POST['xml'][$i],$this->path().'xmls/facturas/temporales/'.($xmld)))
				$this->CaptPolizasModel->facturaRename($xmld);
...
// 
	...
	function guardacomprobacion(){
		...
		$tipopoliza = 3;
		$poli=$this->CaptPolizasModel->savePolizaGasto($idorg,$idejer,$idperio,$tipopoliza,'Comprobacion Gastos '.$poliza['concepto'],$fecha,0,"","",0,"",0,$separa[5]);
			...
			foreach( $_SESSION['compruebagasto'] as $cli){
				foreach($cli as $prove){
					...
					if(isset($prove['cuentanodeducible'])){
						$cargo=$this->CaptPolizasModel->InsertMov2($numPoliza['id'],$numov,$segmento,$sucursal,$prove['cuentacompraventa'],"Cargo",number_format($prove['cargo'],2,'.',''),$prove['concepto'],'2-','',"$referencia",9,$tipopoliza);
						$numov++;
					}else{
						...
						$referencia = $prove['referencia'];
						$cargo=$this->CaptPolizasModel->InsertMov2($numPoliza['id'],$numov,$segmento,$sucursal,$prove['cuentacompraventa'],"Cargo",number_format($prove['cargo'],2,'.',''),$prove['concepto'],'2-',$prove['xml'],"$referencia",9,$tipopoliza);
						$numov++;
					}
					if($cargo==true){
						if($statusIVA==0){
						...
						if($prove['cargo2']>0){
							$cargo2=$this->CaptPolizasModel->InsertMov2($numPoliza['id'],$numov,$segmento,$sucursal,$ivapagado[0],"Cargo",number_format($prove['cargo2'],2,'.',''),$prove['concepto'],'2-',$prove['xml'],"$referencia",9,$tipopoliza);
							$numov++;
						}
						if($statusIEPS==0){
						...
						if($prove['ieps']>0){
							$insertiepss=$this->CaptPolizasModel->InsertMov2($numPoliza['id'],$numov,$segmento,$sucursal,$iepspago[0],"Cargo",number_format($prove['ieps'],2,'.',''),$prove['concepto'],'2-',$prove['xml'],"$referencia",9,$tipopoliza);
							$numov++;
						}
						if($prove['ish']>0){
							$ishinsert=$this->CaptPolizasModel->InsertMov2($numPoliza['id'],$numov,$segmento,$sucursal,$ish[0],"Cargo",number_format($prove['ish'],2,'.',''),$prove['concepto'],'2-',$prove['xml'],"$referencia",0,$tipopoliza);
							$numov++;
						}
					  	//$si=$this->CaptPolizasModel->InsertMov2($numPoliza['id'],$numov,$segmento,$sucursal,$separa[1],"Abono",$prove['abono'],$prove['concepto'],'2-'.$idemisor,$prove['xml'],"$referencia",9,$tipopoliza);
				 	 	//if($si!=false){
				 	 		//$numov++;
				 	 		foreach ( $prove['retenidos'] as $key => $value){
					 	  		if($key=="ISR"){
					 	  			if($value > 0){
					 	  				$si=$this->CaptPolizasModel->InsertMov2($numPoliza['id'],$numov,$segmento,$sucursal,$isrretenido[0],"Abono",number_format($value,2,'.',''),$prove['concepto'],'2-',$prove['xml'],"$referencia",9,$tipopoliza);
					 	  				$numov++;
									}
								}
								if($key == "IVA"){
									if($value > 0){
					 	  				$si=$this->CaptPolizasModel->InsertMov2($numPoliza['id'],$numov,$segmento,$sucursal,$ivaretenido[0],"Abono",number_format($value,2,'.',''),$prove['concepto'],'2-',$prove['xml'],"$referencia",9,$tipopoliza);
					 	  				$numov++;
									}
								}
							}
			...
			if($error==false){
				...
				$si=$this->CaptPolizasModel->InsertMov2($numPoliza['id'],$numov,$nombredeudor[3],$nombredeudor[4],$nombredeudor[1],"Abono",number_format($nombredeudor[2],2,'.',''),"Comprobacion gastos ".$poliza['concepto'],'2-','',"",9,$tipopoliza);
				$numov++;
				if($_REQUEST['cuentadiferencia']>0){
	 	 			if($_REQUEST['cargodiferencia']>0){
	 	 				$dife=$this->CaptPolizasModel->InsertMov2($numPoliza['id'],$numov,$nombredeudor[3],$nombredeudor[4],$_REQUEST['cuentadiferencia'],"Cargo",number_format($_REQUEST['cargodiferencia'],2,'.',''),"Comprobacion gastos ".$poliza['concepto'],'2-','',"",9,$tipopoliza);
					}
					if($_REQUEST['abonodiferencia']>0){
						$dife=$this->CaptPolizasModel->InsertMov2($numPoliza['id'],$numov,$nombredeudor[3],$nombredeudor[4],$_REQUEST['cuentadiferencia'],"Abono",number_format($_REQUEST['abonodiferencia'],2,'.',''),"Comprobacion gastos ".$poliza['concepto'],'2-','',"",9,$tipopoliza);
					}
	 	 		}
				unset($_SESSION['compruebagasto']);
				unset($_SESSION['deudornombre']);
	...
	function CuentaAgregaAuto(){
		...
		if($movViejo){
			$resultado = $this->CaptPolizasModel->InsertMov2($_REQUEST['IdPoliza'], $_REQUEST['Movto'], 1, 1, $ctaBanca['account_id'], $_REQUEST['TipoMovto'], $_REQUEST['importe'], $_REQUEST['concepto'], $_REQUEST['persona'], "-", $_REQUEST['referencia'], $_REQUEST['fomapago'],1);
			if($resultado==false){
				echo 1;
			}
		}else{
	...
	function CuentaPrvAgregaAuto(){
		...
		if($ctaArbol['cuenta']){
			if($movViejo){
				$resultado = $this->CaptPolizasModel->InsertMov2($_REQUEST['IdPoliza'], $_REQUEST['Movto'], 1, 1, $ctaArbol['cuenta'], $_REQUEST['TipoMovto'], $_REQUEST['importe'], $_REQUEST['concepto'], $_REQUEST['persona'], "-", $_REQUEST['referencia'], $_REQUEST['fomapago'],1);
				if($resultado==false){
					echo 1;
				}
			}else{
	...
	function listaTemporalesProvisionBD()
	{
      ...
      $uuid = strtoupper($d->uuid);
      $uri = "xmls/facturas/temporales/{$uuid}.xml";
			...
			$listaTemporales .= "<tr>
				...
				<td width='50' style='text-align:center;'><input title='Sólo copiar' type='checkbox' name='radio-$cont' id='copiar-$cont' value='{$uri}' class='copiar'></td>
				</tr>";
        $cont++;
	...
	function copiaRepetidos(){
		if($_REQUEST['opc']==1){
			...
			for($i=0;$i<=$maximo;$i++){
				...
				if(rename($_POST['xml'][$i], $this->path()."xmls/facturas/temporales/".$name[3]))
					$this->CaptPolizasModel->reactiva_factura($name[3]);
			}
		}
	...
	function datosFac($xml)
	{
		$xml = explode('.xml',$xml);
		$datos = $this->CaptPolizasModel->facturasAuto($xml[0]);
		$datos = $datos->fetch_assoc();

		if(strpos($datos['xml'], 'Cobro')!==false)
			$cobro = "Cobro";
		if(strpos($datos['xml'], 'Parcial')!==false)
			$cobro = "Parcial";
		if($datos['folio'] == '')
			$datos['folio'] = 'S/F';

		return $cobro . " " .$datos['folio'] . " " . $datos['razon_social'] . " " .$datos['uuid'];
	}
