...
class Almacen extends Common
{
	...
	function listaFacturas2()
	{
		...
		while($l = $lista->fetch_assoc())
		{
			...
			if(intval($ok))
			{
				...
				$polpagos = $polprovisiones = '';
				$asignadas = 0;
				$pinta = 1;
				$dir = "temporales";

					$polizas = $this->AlmacenModel->polizasAsignadas2($l['uuid']);
					while($pol = $polizas->fetch_object())
					{
						if(!intval($pol->tipo_poliza))
						{
							$polprovisiones .= "<a href='index.php?c=CaptPolizas&f=ModificarPoliza&id=$pol->id' target='_blank'>".$pol->numpol." (".$pol->idperiodo."-".$pol->ejercicio.")</a> ";
						}
						else
						{
							$polpagos .= "<a href='index.php?c=CaptPolizas&f=ModificarPoliza&id=$pol->id' target='_blank'>".$pol->numpol." (".$pol->idperiodo."-".$pol->ejercicio.")</a> ";
						}
						$dir = "$pol->id";
						$asignadas++;
					}

					if(intval($l['asig']) && $polpagos == '' && $polprovisiones == '')
					{
						$pinta = 0;
					}
				...
				if($pinta)
				{
					$check = "<input type='checkbox' idfac='".$l['id']."' xml='"."xmls/facturas/".$dir."/".$l['xml']."'  onchange = 'actions()'>";
					if(intval($l['pos']))
					{
						$check = "Caja";
					}
			        ...
			        'total' => $l['importe'],
			        //'serie' => $l['serie'],
			        'folio' => $l['serie'].' '.$l['folio'],
			        'uuid' => $l['uuid'],
			        'version' => $l['version'],
			        'provision' => $polprovisiones,
			        'pagos' => $polpagos,
			        'check' => $check,
	...
	function hacer_polizas()
	{
		$ruta = $this->path()."xmls/facturas/";
		$xmls = $_POST['xmls'];
		$tipopoliza = $_POST['tipopol'];
			...
			if($tipopoliza == 1)
			{
				if($datos_factura = $this->AlmacenModel->datos_factura($xmls[$i],$_POST['tipofac'],1))
				{
					if(intval($datos_factura['idTpl']) && $datos_factura['uuid'])
					{
						...
						//Crea la poliza
						$id_poliza_acontia = $this->AlmacenModel->insertarPoliza($datos_factura,$fecha,$numpol,$xmls[$i],0);
			...
			if($tipopoliza == 2)
			{
				if($datos_factura = $this->AlmacenModel->datos_factura($xmls[$i],$_POST['tipofac'],0))
				{
					if(intval($datos_factura['idTpl']) && $datos_factura['uuid'])
					{
						...
						//Crea la poliza
						$id_poliza_acontia = $this->AlmacenModel->insertarPoliza($datos_factura,$fecha,$numpol,$xmls[$i],1);
						if(intval($id_poliza_acontia))
						{
							//Crea la carpeta si no existe y agrega el xml
							if(!file_exists($ruta.$id_poliza_acontia))//Si no existe la carpeta de ese poliza la crea
							...
							if($datos_factura['er'] == 'R')
								//$this->AlmacenModel->pagaFactura($datos_factura,$json,$movs);
								//CREAR FUNCION QUE PAGA LA FACTURA DE EGRESO////////////////////////////******//////////******///////

							$this->AlmacenModel->quitaTemporales($datos_factura);
	...
	function guardarxmlBD()
	{
		...
		$rfcOrg = $this->AlmacenModel->comun_facturacion();
		$rfcOrg = $rfcOrg['rfc'];
		...
		if(isset($_POST['caja']))
			$this->AlmacenModel->guardaFacturaCaja($vars,$_POST['azurian'],$_POST['idcliprov'],$_POST['cliprov']);
    ...
    function dowloadZip(){
        ...
        $path = "../cont/xmls/facturas/temporales/";
