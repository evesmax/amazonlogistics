  ...
  function Eliminarxmls() {
    ...
    foreach ($_POST['xmls'] as $xml => $valor) {
      ...
      if ($ruta[2] == "temporales") {
        ...
        if(count($nombre_xml) > 2){
            //Removemos la extension del archivo
            $uuid = preg_replace('/\\.[^.\\s]{3,4}$/', '', $nombre_xml[2]);
        }else{
            //Removemos la extension del archivo
            $uuid = preg_replace('/\\.[^.\\s]{3,4}$/', '', $nombre_xml[0]);
        }
        array_push($lista_uuid, $uuid);          
        unlink($valor);     
      }else{
          if(file_exists('../cont/xmls/facturas/temporales/'.$ruta[3])){
              unlink('../cont/xmls/facturas/temporales/'.$ruta[3]);
              $nombre_xml = explode("_",$ruta[3]);
              if(count($nombre_xml) > 2){
                  $uuid = preg_replace('/\\.[^.\\s]{3,4}$/', '', $nombre_xml[2]);
              }else{
              ...
              array_push($lista_uuid_relacion, $ruta[3]);
          }
      }
    }
    if(count($lista_uuid_relacion) > 0){
        $response = "El XML se encuentra asignado a una poliza, desasocie el XML para su eliminación: \n";
        for($x = 0; $x < count($lista_uuid_relacion); $x ++){
            $response.=$lista_uuid_relacion[$x]." \n ";
        }
        echo($response);
    }
    if(count($lista_uuid) > 0){
        $result = $this->BackupModel->esconder_facturas($lista_uuid);
        if($result == 1){
            echo('El XML ha sido eliminado');
        }
    }
  }

  function Eliminarxmls2() 
  {
    $eliminados = '';
    foreach ($_POST['xmls'] as $xml => $valor){
      $ruta = explode("/",$valor);
      ...
      if(strpos($ruta[3], '_') === false){
          $uuid = str_replace('.xml', '', $ruta[3]);
      }
      else{
          $uuid = explode('_', $ruta[3]);
          if(isset($uuid[2]))
            $uuid = str_replace('.xml', '', $uuid[2]);
          elseif(isset($xml[1]))
            $uuid = str_replace('.xml', '', $uuid[1]);
      }
      if($this->BackupModel->inactivar_factura($uuid))
        $eliminados .= "$uuid , ";

    }
    echo "Estos XMLs han sido eliminados: $eliminados";
  }
