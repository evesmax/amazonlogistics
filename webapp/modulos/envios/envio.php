            ...
            $(document).ready(function(){
                ...
                //PESOS Y DIMENCIONES 
                    $('#txtcantidad2').bind('focusout', function() {  
                        if($('#txtcantidad2').val()>100){
                            alert('Debe escribir una cantidad mayor al permitido por el tipo de transporte');
                            $('#txtcantidad1').focus();
                        } 
                    });                    
            });
...
//RECUPERANDO VARIABLES
    ...
    //OBTENIENDO INFORMACION BASICA
		...
		$sqlestatus="Select lt.referencia1 otfc, lt.fecha,  
                                ...
                                of.idfabricante, obo.idbodega, obd.idbodega idbodegadestino, 
								ot.idtransportista, lt.idproducto,obo.responsable
		...
		$conexion->cerrar_consulta($result);                        
         ...
         //Verifica Politica para Seleccionar a otros transportistas
            ...
            $cmbtransportista="<select id=cmbtransportista name=cmbtransportista>";
                    $sqltrans="Select idtransportista, razonsocial, ifnull(limitetm,35) limitetm from operaciones_transportistas order by razonsocial";
                    ...
                    while($rs = $conexion->siguiente($result)){
                            ...
                            $sel="";
							$limitetm=$rs{"limitetm"};
                    }
                    $conexion->cerrar_consulta($result);  
            $cmbtransportista.="</select>";   
			$txtlimitetm="";
			$txtlimitetm="<input type=hidden id='txtlimitetm' name='txtlimitetm' value='".$limitetm."'>";
         //Genera  empleado
            $nombrecapturista="";
...
//DATOS BODEGA ORIGEN
				$html.="<td width='40%' valign='top'>";
					...
					$html.="<table class='reporte' width='100%'>";
						...
						$html.="<tr align=left>
                                                            <td colspan=2>
                                                                VALOR DECLARADO POR BULTO:<input readonly type=text id='txtpreciobulto' name='txtpreciobulto' value=$preciobulto Size=20><br>
                                                                VALOR TOTAL DE LA CARGA:<input readonly type=text id='txtimportetotal' name='txtimportetotal' value=0 Size=20>
                                                            </td>
                                                        </tr>";                                                
		...
		$html.="<tr><td>"; //Mega tabla
                    $html.="<br><br><table width='100%'>";	
                            ...
                            $html.="<td width='20%'>";
                                    $html.="<table class='reporte' width='100%'>";
                                            $html.="<tr class='trencabezado'><td>RESPONSABLE BODEGA</td></tr>";
                                            $html.="<tr height='60' valign='top'>";
                                                    $html.="<td>";
										...
										function redireccion() {
											...
											document.location.href=pagina;
                                                                                ...
                                                                                function recalcula(factor,edita,saldo) {
                                                                                    var jfactor=0,jfactord=0,jedita=0,jsaldo1=0,jsaldo2=0, cant1=0, cant2=0,jsaldo=0,precio=0;
                                                                                    jfactor=factor;
                                                                                    jedita=edita;
                                                                                    jfactord=jfactor;
                                                                                    jsaldo=saldo;
                                                                                    ...
                                                                                    jsaldo2=format_number(jsaldo,3);
                                                                                    jsaldo1=valor(jsaldo1)*1;
                                                                                    jsaldo2=valor(jsaldo2)*1;

                                                                                    cant1=valor(format_number(document.getElementById('txtcantidad1').value,2))*1;
                                                                                    cant2=valor(format_number(cant1*jfactor,3))*1; 
                                                                                    
                                                                                    //alert(cant1+'<='+jsaldo1+' And '+cant2+'<='+jsaldo2);
                                                                                    if((cant1<=jsaldo1) && (cant2<=jsaldo2)){
                                                                                        ...
                                                                                        alert('Las cantidades Exeden el Saldo de la Instruccion '+cant1+'<='+jsaldo1+' And '+cant2+'<='+jsaldo2);
                                                                                        document.envio.txtcantidad1.value=format_number(0,2);
                                                                                        document.envio.txtcantidad2.value=format_number(0,2);
                                                                                    }
                                                                                    precio=valor(document.getElementById('txtpreciobulto').value);
                                                                                    document.envio.txtimportetotal.value=format_number(cant1*precio,2);
