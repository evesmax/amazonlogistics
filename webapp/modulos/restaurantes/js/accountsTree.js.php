	...
	**/
	// INICIA DEFINICION DE DATOS, PLUGINS E INTERFACES
		...
		//INICIA OBTENCION DE ARBOL CONTABLE
			...
			{
				var contentLi = "<li data-father ='" + jsonTree[i].father_account_id + "' data-id-acc='" + jsonTree[i].account_id + "'>";
				contentLi += "<img /><span>" + jsonTree[i].description + "</span><ul></ul></li>";
		...
		$("#type,#nature,#main,#coin,#status,#sucursal").select2({
			width : '100%'
		});
	...
	// INICIAN EVENTOS DE FORMULARIO
		$("#sve").click(function(){
			...
			if(alt === true)
			{
				...
				alert("Existen campos vacios. Revise su formulario.");
			}
			else
			{
				...
				switch($(this).attr("class"))
				{
					case 'mod':
						...
						if(data == true)
						{
							...
							{
								alert("Modifiacacion Satisfactoria");
							}
							$('.selected').text( obj.description.toUpperCase() );// Cambio de Texto...
						}
						else
						{
							alert("La modificacion ha fallado.");
					...
					case 'reg':
						...
						else
						{
							...
							$(".new > img").after("<span>"+name+"</span>");
							$(".new > span").data(obj);
							$(".new > span").data('account_id',parseInt(data,10));
							if( obj.main_account == 1 )
							{// si se asigna como cuenta de mayor, debe obtenerse el registro para actualizar su main_father
								$('.new > span').data('main_father', $('.new > span').data().account_id );// asigno main_father
								data = sendObject($('.new > span').data(),'updateAccount');//envio actualizacion
								if( data != true )// si falla
								{
									alert("No ha sido posible realizar el registro. Intente nuevamente.");
									console.log(data);
									$("#cancel").click();
				...
				switch(id)
				{
					case 'accountNumber':
						...
						if(val.length > 0)
						{
							...
							{
								alert('El campo no puede contener un numero igual a cero. Su valor actual es: ' + $(this).val());
								$(this).val('').focus();
							}
						}
						break;
...
function getLastBro(type)
{
	...
	broAccountId.push($('.selected').data().account_code);

	$(".selected").parent().siblings().each(function(i)
	{
		broAccountId.push($("span",this).data().account_code);
	});

	broAccountId.sort();

	var lastBro = broAccountId[broAccountId.length - 1];
	lastArray = lastBro.split('.');
	lastArray[lastArray.length - 1]++;
	lastArray = implode(lastArray,".");
	if (type == 1)
	{
...
function getLastChildren(type)
{
	...
	var text = $(".selected").data().account_code + ".0";
	var childrenAccountId = [text];

	$('.selected').parent().children('ul').children("li").each(function(){
		childrenAccountId.push($('span',this).data().account_code );
	});
	childrenAccountId.sort();
	//console.log(childrenAccountId);
	var lastChildren = childrenAccountId[childrenAccountId.length - 1 ];
	lastArray = lastChildren.split('.');
	lastArray[lastArray.length - 1]++;
	lastArray = implode(lastArray,".");
	if (type == 1)
	{
...
function loadForm(data)
{
	...
	if( ( parseInt(data.main_father,10) > 0 && data.main_father != data.account_id && parseInt( data.main_account , 10) !== 0 ) || ( mainChildrens  === true) )
	{
		...
		{
			$("#s2id_main > a:nth-child(1)").text("CUENTA DE MAYOR");
			$("#main").val(1);
		}
	}
	else
