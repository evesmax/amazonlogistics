...
// Como parametros recibe:
    ...
    convertir_dataTable2 : function($objeto) {
        console.log('objeto convertir dataTable');
        console.log($objeto);
...
// Como parametros recibe:
    ...
    cambiar_tipo : function($objeto) {
        ...
        if ($objeto['tipo'] == 1) {
            ...
            $('#div_precio_fijo').hide();
            $('#div_por_cantidad').hide();
            $('#div_lista_precios').hide();
            $('#div_promo_cumple').hide();
            // Limpia las cantidades
            $('#cantidad').val('');
            $('#cantidad_descuento').val('');
        } else if ($objeto['tipo'] == 4) {
            console.log("4");
            // Oculta la div de cantidades y muestra la de descuento
            $('#div_precio_fijo').show();
            ...
            // Limpia las cantidades
            $('#precio_fijo').val('');
        } else if ($objeto['tipo'] == 5) {
            console.log("5");
            configuracion.comprar_recibir = 1;
            // Oculta la div de cantidades y muestra la de descuento
            $('#div_precio_fijo').hide();
            $('#div_por_descuento').hide();
            $('#div_por_cantidad').hide();
        ...
        } else if ($objeto['tipo'] == 10) {
            console.log("4");
            // Oculta la div de cantidades y muestra la de descuento
            $('#div_precio_fijo').hide();
            $('#div_por_descuento').hide();
            $('#div_por_cantidad').hide();
            $('#div_lista_precios').show();
            $('#div_promo_cumple').hide();
            // Limpia las cantidades
            $('#cantidadL').val('');
        } else if ($objeto['tipo'] == 11) {
            console.log("4");
            // Oculta la div de cantidades y muestra la de descuento
            $('#div_precio_fijo').hide();
            $('#div_por_descuento').hide();
        ...
        if(configuracion.edit == 0){
            $.ajax({
                data: {tipo: $objeto['tipo']},
                url : 'ajax.php?c=configuracion&f=borrar_productos',
                type : 'post',
                dataType : 'json',
            }).done(function(resp) {
                console.log('----> Done borrar_productos');
                console.log(resp);
            }).fail(function(resp) {
                console.log('----------> fail borrar_productos');
                console.log(resp);
            });
            ...
            $("#div_productos_agregados").html('<br /><br /><blockquote style="font-size: 16px"><p>Selecciona <strong>"productos"</strong> o <strong>"productos preparados"</strong> para agregarlos.</p></blockquote>');
        }
        configuracion.edit = 0;
    },
...
// Como parametros recibe:
    ...
    guardar_promocion : function($objeto) {        
        ...
        // Valida que se ingrese un descuento o unas cantidades
        if($("#tipo").val() == 4){   
            if ($datos['precio_fijo'] == '') {
                var $mensaje = 'Agrega un precio fijo';
                $("#btn_guardar_promocion").notify($mensaje, {
                    position : "top left",
                    autoHide : true,
                    autoHideDelay : 4000,
        ...
        $.ajax({
            ...
            // Sin productos comprar :p
            if (resp['status'] == 4) {
                var $mensaje = 'Necesitas agregar productos a la seccion de comprar';
                $("#btn_guardar_promocion").notify($mensaje, {
                    position : "top left",
                    autoHide : true,
                    autoHideDelay : 5000,
            ...
            // Sin productos recibir :p
            if (resp['status'] == 5) {
                var $mensaje = 'Necesitas agregar productos a la seccion de recibir';
                $("#btn_guardar_promocion").notify($mensaje, {
                    position : "top left",
                    autoHide : true,
                    autoHideDelay : 5000,
...
// Como parametros recibe:
    ...
    editar_promocion : function($objeto) {
        ...
        // LLENA SELECT DE SUCURSAL 
        if($objeto['idsuc'] != null){
            $.each($objeto['idsuc'].split(","), function(i,e){
                $("#sucursal option[value='" + e + "']").prop("selected", true);            
            });
        }
        ...
        $.each($objeto["productos"], function(index, val) {
            ...
            val['check'] = false;
            if(val['comprar'] == 1){
                val['comprar_recibir'] = 1;
            } else {
                val['comprar_recibir'] = 2;
            }
            val['tipo'] = $objeto['tipo_promocion'];
            val['cantidad'] = $objeto['cantidad'];
            val['cantidad_descuento'] = $objeto['cantidad_descuento'];
            // Agrega el insumo al array de insumos agregados
            configuracion.agregar_producto(val);
        });
        //alert($objeto["tipo_promocion"])
        // LLena los campos
        $("#nombre").val($objeto['nombre']);
        $("#tipo").val($objeto['tipo_promocion']);
        configuracion.cambiar_tipo({
            tipo : $objeto['tipo_promocion']
        });
       
        $("#descuento").val($objeto['descuento']);
        $("#cantidad").val($objeto['cantidad']);
        $('#cantidadL').val($objeto['cantidad']);
        $("#cantidad_descuento").val($objeto['cantidad_descuento']);

        $("#precio_fijo").val($objeto['precio_fijo']);
        $("#descuento").val($objeto['descuento']);
        $("#inicio").val($objeto['inicio']);
        $("#fin").val($objeto['fin']);
        $('#listaPrecio').val($objeto['idLista']);

        $('#clavesat').val($objeto['clavesat']);
        $('#unidadventa').val($objeto['unidadventa']);
        ...
        if($objeto['tipo_promocion'] == 5){
            $("#btn_seleccionar_grupo_comprar").click();
        }        
        if($objeto["tipo_promocion"]==10){
            $('#desLista').hide();
        }else{
            $('#desLista').show();
        }
...
// Como parametros recibe:
    ...
    convertir_arbol : function($objeto) {
        ...
        $('#' + $objeto['id']).on('changed.jstree', function(e, data) {
			...
			$.ajax({
				url : 'ajax.php?c=configuracion&f=limpiar_menu_digital',
				type : 'POST',
				dataType : 'json',
            	async : false
			}).done(function(resp) {
				console.log('----> Done limpiar_menu_digital');
				console.log(resp);

			}).fail(function(resp) {
				console.log('----> Fail limpiar_menu_digital');
				console.log(resp);
            ...
            for ( i = 0, j = data.selected.length; i < j; i++) {
                ...
                $.ajax({
		            data : nodo,
		            url : 'ajax.php?c=configuracion&f=agregar_nodo',
		            type : 'POST',
		            dataType : 'json'
		        }).done(function(resp) {
		            console.log('----> Done agregar_nodo');
		            console.log(resp);
		            
		        }).fail(function(resp) {
		            console.log('----> Fail agregar_nodo');
		            console.log(resp);
...
// Como parametros recibe:
    ...
    vista_editar_menu : function($objeto) {
        ...
        $.ajax({
            ...
            configuracion.vista_nuevo_menu({
                div : 'div_menu',
                btn : 'btn_nueva',
                panel : 'primary'
            });
        ...
        $.ajax({
            ...
            if (resp['status'] == 1) {
                ...
                if ($objeto['imprimir'] == 1) {
                    $("#" + $objeto['div_imprimir']).printArea();
                    $.ajax({
                        data : {html: $("#" + $objeto['div_imprimir']).html()},
                        url : 'ajax.php?c=configuracion&f=descargar_pdf',
                        type : 'POST',
                        dataType : 'json'
                    }).done(function(resp) {
                        console.log('----> Done descargar_pdf');
                        console.log(resp);

                    }).fail(function(resp) {
                        console.log('----> Fail vista descargar_pdf:)u');
                        console.log(resp);
...
actualizar_sucursales_complementos : function(id){

    var sucursales = $("#sucursal_"+id).val();
    if(sucursales != 0){
        configuracion.complementos[id]['sucursales'] = sucursales;
    }else{
        configuracion.complementos[id]['sucursales'] = 0;
    }

},


///////////////// ******** ----             actualizar_cantidad_complementos             ------ ************ //////////////////
/////// Cambia la cantidad del producto
// Como parametros recibe:
    // id -> ID del producto
    // cantidad -> Nueva cantidad

    actualizar_cantidad_complementos : function($objeto) {
        console.log('----> objeto actualizar_cantidad_complementos');
        console.log($objeto);

        if($objeto['cantidad'] != ''){
            configuracion.complementos[$objeto['id']]['cantidad'] = $objeto['cantidad'];
        } else {
            configuracion.complementos[$objeto['id']]['cantidad'] = 0;
        }
    },

///////////////// ******** ----             FIN actualizar_cantidad_complementos         ------ ************ //////////////////
...
///////////////// ******** ----             seleccionar_grupo_promo                   ------ ************ //////////////////
/////// Selecciona el grupo, cambia el color del panel y lo expande
// Como parametros recibe:
    // grupo -> ID del grupo

    seleccionar_grupo_promo : function($objeto) {
        console.log('objeto seleccionar_grupo_promo');
        console.log($objeto);
        var tabla = $('#tabla_productos').dataTable();
        var tabla = tabla.fnGetNodes();
        $(tabla).each(function(index) {
        ...
        if($objeto['grupo'] == 'recibir'){
            ...
            $.each($("#tabla_recibir_5 tbody tr"), function(index, value) {
                var id_tr = value.id.split("_");
                $(tabla).each(function(index) {
                    id = $(this, tabla).attr('id');
                    if (id == "tr_"+id_tr[2] || id == "tr_preparado_"+id_tr[2]) {
                        console.log(id+" - "+id_tr[2]);
                        checkbox = $(this, tabla).find('input');
                        checkbox.prop("checked", true);
                        $(this, tabla).addClass('success');
                    }
            ...
            $.each($("#tabla_comprar_5 tbody tr"), function(index, value) {
                var id_tr = value.id.split("_");
                $(tabla).each(function(index) {
                    id = $(this, tabla).attr('id');
...
// Como parametros recibe:
    ...
    vista_editar_combo : function($objeto) {
        ...
        $.ajax({
            ...
            // Carga la vista de de nueva promocion
            configuracion.vista_nuevo_combo({
                div : 'div_combos',
                btn : 'btn_nuevo',
                panel : 'primary'
            });
...
// Como parametros recibe:
    ...
    eliminar_combo : function($objeto) {
        console.log('----> Objeto eliminar_combo2');
        console.log($objeto);
...
// Como parametros recibe:
    ...
    agregar_complemento : function($objeto) {
        ...
        if (!configuracion.complementos[$objeto['id']]) {
            configuracion.complementos[$objeto['id']] = $objeto;
        } else {
            delete configuracion.complementos[$objeto['id']];
        }
        
        var $datos = {};
        $datos['complementos'] = configuracion.complementos;

        console.log('======> Complementos');
        console.log(configuracion.complementos);

        $.ajax({
            data : $datos,
            url : 'ajax.php?c=configuracion&f=agregar_complemento',
            type : 'POST',
            dataType : 'html'
        }).done(function(resp) {
        ...
        // Carga la vista a la div
            ...
            if ($objeto['preparado'] == 1) {
                var tabla = $('#tabla_insumos_preparados').dataTable();
                var $tr = 'tr_preparado_';
            } else {
                var tabla = $('#tabla_insumos').dataTable();
                var $tr = 'tr_';
            }

            tabla = tabla.fnGetNodes();
            ...
            // Mensaje error
            $mensaje = 'Error al agregar el complemento';
            $.notify($mensaje, {
                position : "top center",
                autoHide : true,
                autoHideDelay : 5000,
...
// Como parametros recibe:
    ...
    guardar_complementos : function($objeto) {
        console.log('----> Objeto guardar_complementos');
        console.log($objeto);
    ...
    // Loader en el boton
        ...
        $.ajax({
            data : $objeto,
            url : 'ajax.php?c=configuracion&f=guardar_complementos',
            type : 'POST',
            dataType : 'json'
        }).done(function(resp) {
            console.log('----> Done guardar_complementos');
            console.log(resp);
		...
		// Mensaje error
            $mensaje = 'Error al guardar los complementos';
            $.notify($mensaje, {
                position : "top center",
                autoHide : true,
                autoHideDelay : 5000,
...
// Como parametros recibe:
    ...
    cambiar_cantidad_promo : function($objeto) {
        console.log('==========> Objeto cambiar_cantidad_promo');
        console.log($objeto);
        
        $.ajax({
            data : $objeto,
            url : 'ajax.php?c=configuracion&f=cambiar_cantidad_promo',
            type : 'POST',
            dataType : 'json'
        }).done(function(resp) {
            console.log('==========> Done cambiar_cantidad_promo');
            console.log(resp);
            
        }).fail(function(resp) {
            console.log('==========> Fail cambiar_cantidad_promo');
            console.log(resp);
...
// Como parametros recibe:
    ...
    cambiar_cantidad_combo : function($objeto) {
        console.log('==========> Objeto cambiar_cantidad_combo');
        console.log($objeto);
        
        $.ajax({
            data : $objeto,
            url : 'ajax.php?c=configuracion&f=cambiar_cantidad_combo',
            type : 'POST',
            dataType : 'json'
        }).done(function(resp) {
       		console.log('==========> Done cambiar_cantidad_combo');
            console.log(resp);
            
        }).fail(function(resp) {
       		console.log('==========> Fail cambiar_cantidad_combo');
            console.log(resp);
    ...
    },clicksFakes : function(){
        
        var oTable = $('#tabla_productos').dataTable();
        var allPages = oTable.fnGetNodes();
        //$('#modalMensajes').modal();
        cadena='';
        $('#tabla_productos_wrapper tr').each(function(){
            $('#'+$(this).attr('id')).trigger('click');
        }); 
        //alert('Gracias a Chais porla ayuda, le daremos una chela');
    },cambiaLista : function(){
        var desc =  $('#listaPrecio option:selected').attr('descuento');
