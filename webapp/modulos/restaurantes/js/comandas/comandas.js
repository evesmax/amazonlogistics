...
// Inicializamos el array de las mesas seleccionadas
	...
	tooltips : function(comanda, ids, lugar) {
		// tooltip para pedidos terminados
		...
		// tooltip para pedidos terminados fin
	},
	...
	buscaProductos : function($objeto) {
		...
		//SE OMITE POR VELOCIDAD
		// Consulta si hay productos terminados y los muestra en mapa de mesas
			$.ajax({
		...
		$.ajax({
			...
			if (data) {
				$.each(data, function(index, val) {		
					...
					if(data[index]["id_comanda"] != 0){ // son mesas activas con comanda
						if (data[index]["juntas"] == null) { // mesas juntas
						...
						if(data[index]["id_res"] != null){ // sin reservacion
						...
						} else if(data[index]["mesa_status"] == 4){ // bloqueda 4
							...
							$("#mesa_"+data[index]["mesa"]).attr("mesa_status", '1'); // libre
								...
								///// ELIMINA MESA TEMPORAR CREADA DESDE COMANDERA AL CERRAR COMANDA
									if(data[index]["tipoMesa"] == 3 && data[index]["statusMesa"] == 2){
										$("#"+data[index]["mesa"]).remove();
									}										
								///// ELIMINA MESA TEMPORAR CREADA DESDE COMANDERA AL CERRAR COMANDA FIN


						}
					}
					if(data[index]['notificacion'] == 1 || data[index]['notificacion'] == 2){
...
///////////////// ******** ---- 		listar_mesas_ch		------ ************ //////////////////
	...
	listar_mesas_ch : function() { 
		if(comandas.idioma == 0)
			comandas.get_idioma();

		$.ajax({
			url : 'ajax.php?c=comandas&f=listar_mesas_ch',
			type : 'GET',
			dataType : 'html',
			success : function(resp) {
				// console.log('------------> response listar_asignacion');
				// console.log(resp);
				$("#contenedor_ch").html(resp);
			}
		});
	},
...
///////////////// ******** ---- 		FIN asignar		------ ************ //////////////////
	...
	asignar_ch : function($objeto) {
		...
		$.ajax({
			data : $objeto,
			url : 'ajax.php?c=comandas&f=asignar_ch',
			type : 'GET',
			dataType : 'json',
			success : function(resp) {
				// console.log('-------------> response asignar');
...
///////////////// ******** ---- 		listar_asignacion		------ ************ //////////////////
	...
	listar_asignacion_ch : function($objeto) {
		// console.log('------------> $objeto listar_asignacion');
		// console.log($objeto);			
		if(comandas.idioma == 0)
			comandas.get_idioma();
		...
		$.ajax({
			data : $objeto,
			url : 'ajax.php?c=comandas&f=listar_asignacion_ch',
			type : 'GET',
			dataType : 'json',
			success : function(resp) {
				// console.log('------------> response listar_asignacion');
				...
				// Desmarca las mesas
					$.each(resp['mesas'], function(index, val) {
						$('#btna_' + val['mesa']).removeClass("btn-success");
					});

				// Marca solo las mesas asignadas al empleado
					$.each(resp['permisos'], function(index, val) {
						$('#btna_' + val).addClass('btn-success');
					});
		...
		$.ajax({
			...
			// Se agregaron las mesas al mesero
				...
				// Cierra la ventana de las mesas
					$('#btn_cerrar_mesas, #btn_cerrar_mesas_ch').click();
		...
		// Todo bien
			if(resp){
				...
				if(cliente_nuevo == 0){
					...
					var pin = "window.open('https://www.google.com/maps/place/"+$objeto['lat']+","+$objeto['lng']+"')";

					$("#tabla_servicio_domicilio").prepend('<tr id="tr_servicio_domicilio_'+cliente_nuevo+'">'+
						'<td id="idcli_'+cliente_nuevo+'">'+cliente_nuevo+'</td>'+
						'<td id="nom_'+cliente_nuevo+'">'+$objeto['nombre']+'</td>'+
						'<td id="dir_'+cliente_nuevo+'">'+$objeto['direccion']+'</td>'+
						'<td id="tel_'+cliente_nuevo+'" align="center">'+$objeto['cel']+'</td>'+								
						
						'<td id="loc'+cliente_nuevo+'"><i id="loci_'+cliente_nuevo+'"  class="fa fa-map-marker fa-lg" aria-hidden="true"></i></td>'+
						
						'<td id="zon_'+cliente_nuevo+'">'+$("#zona_reparto_domicilio option:selected").text()+'</td>'+
						'<td id="via_'+cliente_nuevo+'">'+via_c+'</td>'+
						'<td align="center"> <button  id="btn_edit_'+cliente_nuevo+'" data-loading-text="<i class='+"'"+'fa fa-refresh fa-spin'+"'"+'></i>"  onclick="comandera.llenar_campos('+json+')" data-toggle="modal"  data-target="#modal_editar_cliente_domicilio" class="btn btn-primary"> <i class="fa fa-pencil-square-o"></i> </button> </td>'+
						'<td align="center"> <button id="btn_servicio_domicilio_'+cliente_nuevo+'" data-loading-text="<i class='+"'"+'fa fa-refresh fa-spin'+"'"+'></i>" onclick="servicio_domicilio({ nombre:'+"'"+$objeto['nombre']+"'"+', btn:'+"'"+'btn_servicio_domicilio_'+cliente_nuevo+"'"+', domicilio:'+"'"+$objeto['direccion']+"'"+', via_contacto:'+"'"+$objeto['via_contacto']+"'"+', zona_reparto:'+"'"+$objeto['zona_reparto']+"'"+', tel:'+"'"+$objeto['cel']+"'"+', tipo_operacion: 1})" class="btn btn-success"><i class="fa fa-check"></i></button></td>'+
						'</tr>');

					$("#loci_"+cliente_nuevo+"").attr('onclick', pin);
					
					/// agrega el cliente a la tabla de clinetes de servico a domicilo 
					//comandas.convertir_dataTable({id:'tabla_servicio_domicilio'});
					//$("#tabla_servicio_domicilio").dataTable();
					...
					$("#referencia_servicio_domicilio").val('');
					...
					$("#tabla_para_llevar").prepend('<tr id="tr_para_llevar_'+cliente_nuevo+'">'+ 						
						'<td>'+cliente_nuevo+'</td>'+
						'<td>'+$objeto['nombre']+'</td>'+
						'<td align="center">'+$objeto['cel']+'</td>'+  
						'<td id="via_'+cliente_nuevo+'">'+via_c+'</td>'+
						'<td align="center"> <button  id="btn_edit_'+cliente_nuevo+'"  data-loading-text="<i class='+"'"+'fa fa-refresh fa-spin'+"'"+'></i>"  onclick="comandera.llenar_campos('+json2+')" data-toggle="modal"  data-target="#modal_editar_para_llevar" class="btn btn-primary"><i class="fa fa-pencil-square-o"></i></button></td>'+
						'<td align="center"><button id="btn_para_llevar_'+cliente_nuevo+'"  data-loading-text="<i class='+"'"+'fa fa-refresh fa-spin'+"'"+'></i>"  onclick="para_llevar({ btn: '+"'"+'btn_para_llevar_'+cliente_nuevo+"'"+', nombre: '+"'"+$objeto['nombre']+"'"+', via_contacto: '+"'"+$objeto['via_contacto']+"'"+', cel: '+"'"+$objeto['cel']+"'"+', tipo_operacion: 1 })" class="btn btn-success"><i class="fa fa-check"></i></button></td>'+
						'</tr>');
										
					/// agrega el cliente a la tabla de para llevar de servico a domicilo 
					//comandas.convertir_dataTable({id:'tabla_para_llevar'});
					//$("#tabla_para_llevar").dataTable();
			...
			if(resp){				
				...
				if(cliente_nuevo == 0){
					...
					if($objeto['servicio_domicilio'] == 1){
						...
						$("#modal_editar_cliente_domicilio").modal('hide');
						...
						$("#tr_servicio_domicilio_"+cliente_nuevo).html('<td id="idcli'+cliente_nuevo+'">'+cliente_nuevo+'</td>'+
							'<td id="nom_'+cliente_nuevo+'">'+$objeto['nombre']+'</td>'+
							'<td id="dir_'+cliente_nuevo+'">'+$objeto['direccion']+'</td>'+
							'<td id="tel_'+cliente_nuevo+'" align="center">'+$objeto['cel']+'</td>'+
							...
							'<td id="zon_'+cliente_nuevo+'">'+$("#editar_zona_reparto_domicilio option:selected").text()+'</td>'+
							'<td id="via_'+cliente_nuevo+'">'+via_c+'</td>'+
							'<td align="center"> <button  id="btn_edit_'+cliente_nuevo+'" data-loading-text="<i class='+"'"+'fa fa-refresh fa-spin'+"'"+'></i>"  onclick="comandera.llenar_campos('+json+')" data-toggle="modal"  data-target="#modal_editar_cliente_domicilio" class="btn btn-primary"> <i class="fa fa-pencil-square-o"></i> </button> </td>'+
							'<td align="center"> <button id="btn_servicio_domicilio_'+cliente_nuevo+'" data-loading-text="<i class='+"'"+'fa fa-refresh fa-spin'+"'"+'></i>" onclick="servicio_domicilio({ nombre:'+"'"+$objeto['nombre']+"'"+', btn:'+"'"+'btn_servicio_domicilio_'+cliente_nuevo+"'"+', domicilio:'+"'"+$objeto['direccion']+"'"+', via_contacto:'+"'"+$objeto['via_contacto']+"'"+', zona_reparto:'+"'"+$objeto['zona_reparto']+"'"+', tel:'+"'"+$objeto['cel']+"'"+', tipo_operacion: 1})" class="btn btn-success"><i class="fa fa-check"></i></button></td>');
											
						comandas.convertir_dataTable({id:'tabla_servicio_domicilio'});
						//alert('table domicilio');
						$("#loci_"+cliente_nuevo+"").attr('onclick', pin);
					...
					if($objeto['para_llevar'] == 1){
						...
						var tabla2 = $('#tabla_para_llevar').DataTable();
						//tabla2.destroy();
						$objeto['para_llevar'] = '1';
						$objeto['servicio_domicilio'] = '';
						var json2 = JSON.stringify($objeto);
						json2 = json2.replace(/"/g, "'");
						$("#tr_para_llevar_"+cliente_nuevo).html('<td>'+cliente_nuevo+'</td>'+
							'<td>'+$objeto['nombre']+'</td>'+
							'<td align="center">'+$objeto['cel']+'</td>'+
							'<td id="via_'+cliente_nuevo+'">'+via_c+'</td>'+
							'<td align="center"> <button  id="btn_edit_'+cliente_nuevo+'"  data-loading-text="<i class='+"'"+'fa fa-refresh fa-spin'+"'"+'></i>"  onclick="comandera.llenar_campos('+json2+')" data-toggle="modal"  data-target="#modal_editar_para_llevar" class="btn btn-primary"><i class="fa fa-pencil-square-o"></i></button></td>'+
						...
						//$('#modal_editar_cliente_domicilio').modal('hide');
						//$('#modal_editar_para_llevar').modal('hide');		
...
//////// Cambia el estatus de la comanda en la BD
	...
	cambiar_status : function($objeto) {
		...
		var tipos = 0;
		$.ajax({
			data : {idcomanda:$objeto.id_comanda},
			url : 'ajax.php?c=comandas&f=tipomesa',
			type : 'POST',
			async:false,
		}).done(function(resp) {
			if(resp == 1 || resp == 2)
	...
	// Loader en el boton OK
		...
		// Quita el loader
			...
			if(tipos == 1){ // para llevar y a domicilio al reabrir se debe recargar las mesas
				window.parent.quitartab("tb2156",2156,"Comandera");
				window.parent.agregatab("../../modulos/restaurantes/ajax.php?c=comandas&f=menuMesas","Comandera","",2156);
			}
...
//////// Crea un tooltip de la mesa
	...
	tooltip_mesa : function($objeto) {
		...
		if($objeto['notificacion'] == 1){ 			
			var notificacion = 'Solicitud de servicio!';
		}else if($objeto['notificacion'] == 2){			
			var notificacion = 'Solicitud de cuenta!';
		}
		
		
		$contenido = "	<div align='center'>";
		$contenido += "		"+notificacion+"<br/><br/>";
...
//////// Agrega una mesa al array de las mesas seleccionadas
	...
	seleccionar_mesa_ch : function($objeto) {
		console.log('=========> objeto seleccionar_mesa');
		console.log($objeto);
		
		if($("#btna_"+$objeto['id_mesa']).hasClass('btn-info')){
			delete comandas.mesas_seleccionadas['mesa_'+$objeto['id_mesa']];
			$('#btna_' + $objeto['id_mesa']).removeClass('btn-info');			
		}else{
			comandas.mesas_seleccionadas['mesa_'+$objeto['id_mesa']] = $objeto;
			$("#btna_"+$objeto['id_mesa']).addClass('btn-info');						
		}
...
//////// Agrega una mesa al array de las mesas seleccionadas
	...
	seleccionar_silla : function($objeto) {
		console.log('=========> objeto seleccionar_mesa');
		console.log($objeto);
...
//////// Carga la vista de las mesas
	...
	vista_juntar_sillas : function($objeto) {
		console.log('=========> objeto vista_juntar_sillas');
		console.log($objeto);
		if(comandas.idioma == 0)
			comandas.get_idioma();
		comandas.mesas_seleccionadas = {};
		...
		$.ajax({
			data : $objeto,
			url : 'ajax.php?c=comandas&f=vista_juntar_sillas',
			type : 'GET',
			dataType : 'html',
		}).done(function(resp) {
			console.log('=========> Done vista_juntar_sillas');
			console.log(resp);
		
		// Carga la vista a la div
			$('#'+$objeto['div']).html(resp);
		}).fail(function(resp) {
			console.log('=========> Fail vista_juntar_sillas');
			console.log(resp);
...
//////// Junta las mesas seleccionadas
	...
	juntar_sillas : function($objeto) {
		console.log('=========> objeto juntar_sillas');
		console.log($objeto);
		if(comandas.idioma == 0)
			comandas.get_idioma();
		...
		$.ajax({
			data : $objeto,
			url : 'ajax.php?c=comandas&f=juntar_sillas',
			type : 'POST',
			dataType : 'json'
		}).done(function(resp) {
			console.log('=========> done juntar_sillas');
			console.log(resp["result"]["id"]);
...
//////// Consulta las propinas y lo agrega a la div
	...
	// Loader en el boton OK
		...
		// Crea el datatable				
			//comandas.convertir_dataTable({id:'tabla_propinas',exportarxls:1});


			$('#tabla_propinas').DataTable( {dom: 'Bfrtip',
                                                            buttons: [                                                                          
                                                                        {	/*EXCEL*/
															                extend: 'excelHtml5',
															                exportOptions: {
