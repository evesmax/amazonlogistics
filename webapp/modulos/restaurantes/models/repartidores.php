...
// ch@
	function listarpedidosRep($idRep) {
        ...
        $sql = "SELECT br.*, concat(au.nombre,' ',au.apellidos) nombre, e.turno, c.total from com_bit_repartidores br 
        left join administracion_usuarios au on au.idempleado = br.id_repartidor 
        left join com_meseros m on m.id_mesero = br.id_repartidor 
        LEFT JOIN empleados e ON e.idempleado = au.idempleado
        left join com_comandas c on c.id = br.id_comanda where $filtro;";
...
//////// Consulta las comandas y las regresa en un array
	...
	function listar_empleados($objeto) {
		...
		$sql = "SELECT
					u.idempleado AS id, nombreusuario AS usuario, permisos, asignacion, e.turno
				FROM 
				...
				LEFT JOIN empleados e 
					ON 
						e.idempleado = u.idempleado
				WHERE
					u.idempleado != 'null'
				AND 
					u.idSuc =  " . $sucursal . " " . 
				$condicion . "
				ORDER BY e.turno desc";
		// return $sql;
...
//////// Consulta los ajustes de Foodware y los regresa en un array
	...
	function save_turn($objeto) {
		$sql = "UPDATE empleados 
				SET turno = '".$objeto['turno']."' 
				WHERE idempleado = '".$objeto['id']."';";
		return $this->query($sql);
	}
...
///////////////// ******** ----            		FIN listar_areas            		------ ************ //////////////////
	...
	function tablaDomicilio($post){		

		$desde = $post['desde']." 00:00:01";
        $hasta = $post['hasta']." 23:59:59";

        $filtro ='pv.fecha BETWEEN "'.$desde.'" and "'.$hasta.'"';
        if($post['empleado'] != 0){
        	$filtro .= ' AND u.idempleado = '.$post['empleado'].' ';
        }
        if($post['viacontacto'] != 0){
        	$filtro .= ' AND vc.id = '.$post['viacontacto'].' ';
        }
        if($post['sucursal'] != 0){
        	$filtro .= ' AND m.idSuc = '.$post['sucursal'].' ';
        }
        if($post['metodopago'] != 0){
        	$filtro .= ' AND fp.idFormapago = '.$post['metodopago'].' ';
        }


		$sql = "SELECT c.codigo, m.nombre, domicilio, vp.monto, fp.nombre pago, v.idVenta FROM com_comandas c
				LEFT JOIN com_mesas m on m.id_mesa = c.idmesa
				INNER JOIN app_pos_venta v on v.idVenta = c.id_venta
				LEFT JOIN app_pos_venta_pagos vp on vp.idVenta = v.idVenta
				LEFT JOIN view_forma_pago fp on fp.idFormapago = vp.idFormapago
				LEFT JOIN administracion_usuarios u on u.idempleado = c.idempleado
				LEFT JOIN com_vias_contacto vc on vc.id = m.id_via_contacto
				LEFT JOIN app_pos_venta pv on pv.idVenta = vp.idVenta
				WHERE m.tipo = 2 and ".$filtro." ORDER BY c.codigo desc;";

		$result = $this -> queryArray($sql);
		return $result['rows'];
	}

	function viacontacto(){
		$sql = "SELECT * FROM com_vias_contacto;";
		$result = $this -> queryArray($sql);
		return $result['rows'];
	}
	function metodopago(){
		$sql = "SELECT * FROM view_forma_pago;";
		$result = $this -> queryArray($sql);
		return $result['rows'];
	}
	function sucursales(){
