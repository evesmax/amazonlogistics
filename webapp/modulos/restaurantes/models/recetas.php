		...
		$sql="	INSERT INTO
				...
				VALUES
					('".$datos['codigo']."','".$datos['nombre']."',".$datos['precio_venta'].",
						1, ".$datos['costo_receta'].", ".$datos['unidad_venta'].", ".$datos['unidad_compra'].", ".$tipo."
					)";
		// return $sql;
	...
	// Guarda la receta y regresa el ID
		$sql="	UPDATE
				...
				SET
					...
					precio = ".$datos['precio_venta'].", costo_servicio = ".$datos['costo_receta'].",
		...
		$sql1 = "UPDATE app_costos_proveedor SET costo = ".$datos['costo_receta']." WHERE id_producto = ".$datos['id_receta'].";";
		$this->dataTransact($sql1);
...
//////// Inserta un registro en la tabla com_recetas con los datos de la receta
	...
	// Guarda la receta y regresa el ID
		$sql="	INSERT INTO
					com_recetas
						(id, nombre, precio, ganancia, ids_insumos, ids_insumos_preparados, preparacion, proveedores_insumos, ids_normales, costo_receta)
				VALUES
					(".$datos['id_receta'].", '".$datos['nombre']."',".$datos['precio_venta'].",
						".$datos['margen_ganancia'].",
						'".$datos['ids']."','".$datos['ids_preparados']."','".$datos['preparacion']."',
						'".$datos['proveedores_insumos']."','".$datos['normales']."','".$datos['costo_receta']."'
					)";
		// return $sql;
		$result =$this->insert_id($sql);
	...
	// Actualiza los datos de la receta
		$sql="	UPDATE
				...
				SET
					...
					ids_normales = '".$datos['normales']."',
					costo_receta = '".$datos['costo_receta']."'
				WHERE
					id = ".$datos['id_receta'];
		// return $sql;
...
///////////////// ******** ---- 		FIN actualizar_receta		------ ************ //////////////////
	...
	function eliminar_grupos($objeto){
		$sql="	DELETE FROM com_recetas_grupos WHERE id_receta =".$objeto['id_receta'];
		$result = $this->query($sql);
...
//////// Inserta un registro en la tabla com_recetas con los datos de la receta
	...
	// Valida el tipo de producto: 5-> Receta, 4-> Insumo preparado
		...
		if($datos['id_grupo'] > 0){
			$sql = "INSERT INTO com_recetas_grupos (`id_receta`, `id_insumo`, `id_grupo`)
			VALUES ('".$datos['id_receta']."', '".$datos['id']."', '".$datos['id_grupo']."');";
			$result =$this->query($sql);
		}
	...
	// Si es insumo preparado toma "p.costo_servicio", si no toma el costo del proveedor y si es null lo cambia por 0
		...
		if($total['total'] > 0){
			$sql="	SELECT
						...
						p.id_unidad_compra AS idunidadCompra, p.id_unidad_venta AS idunidad, p.tipo_producto,
						(SELECT
							nombre
						FROM
...
///////////////// ******** ---- 		listar_servicios			------ ************ ///////////////////////////////////
	...
	function listar_servicios($objeto){
	// Filtra por el ID del insumo si existe
		$condicion.=(!empty($objeto['id']))?' AND p.id='.$objeto['id']:'';
	// Filtra por el nombre y codigo de insumo del insumo si existe
		$condicion.=(!empty($objeto['nombre'])&&!empty($objeto['codigo']))?'
	...
	// Si es insumo preparado toma "p.costo_servicio", si no toma el costo del proveedor y si es null lo cambia por 0
		...
		if($total['total'] > 0){
			$sql="	SELECT
						p.id AS idProducto, p.nombre, IF(p.tipo_producto = 2, ROUND(p.costo_servicio, 2), IFNULL(pro.costo,0)) AS costo,
						p.id_unidad_compra AS idunidadCompra, p.id_unidad_venta AS idunidad, p.tipo_producto,
						(SELECT
							nombre
						FROM
...
//////// Consulta las recetas y los regresa en un array
	...
	// Filtra por tipo
		...
		$condicion.= ' or p.tipo_producto=8 '; // para producto de produccion
	...
	// Ordena  la consulta si existe
		...
		if($total['total'] > 0){
			$sql = "SELECT
						...
						if(if(p.costo_servicio <= 0,((select costo from app_costos_proveedor where id_producto = p.id limit 1)),p.costo_servicio) is null ,0,
							if(p.costo_servicio <= 0,((select costo from app_costos_proveedor where id_producto = p.id limit 1)),p.costo_servicio)) AS costo2,
							r.costo_receta costo,
						...
						r.preparacion, r.ganancia,
						p.precio AS precio,
						p.codigo,
						r.proveedores_insumos, p.tipo_producto
					FROM
						app_productos p
...
///////////////// ******** ---- 		listar_materiales	------ ************ ///////////////////////////////////
	...
	// Filtra por el nombre del insumo si existe
		...
		$sql = "SELECT
					p.id AS idProducto,
					p.nombre,
					IF(p.tipo_producto IN (2,4), ROUND(p.costo_servicio, 2), IFNULL(pro.costo,0)) AS costo,
					...
					WHERE
						...
						u.factor, m.cantidad, m.opcionales AS opcionales,
						m.costear,
						GROUP_CONCAT(pro.id_proveedor) as ids_proveedor,
						GROUP_CONCAT(pro.costo) as costos, p.tipo_producto,
						if(rg.id_grupo is null ,0,rg.id_grupo) id_grupo
				FROM app_productos p
				INNER JOIN app_producto_material m ON p.id=m.id_material
				LEFT JOIN app_unidades_medida u ON u.id=p.id_unidad_compra
				LEFT JOIN app_costos_proveedor pro ON pro.id_producto=p.id
				LEFT JOIN com_recetas_grupos rg on rg.id_insumo = m.id_material and rg.id_receta = ".$objeto['id_receta']."
				WHERE p.status=1
				AND m.id_producto = ".$objeto['id_receta']. $condicion . " group by idProducto";
						// return $sql;
      ...
      if ($id_receta) {
        ...
        if($id_receta_2){
          foreach ($value['insumos'] as $key2 => $value2) {
            $id_material = 0;
            $sql=
              "INSERT INTO app_producto_material(
                id_producto,
                cantidad,
                id_unidad,
                id_material,
                ...
                id_unidad = '{$value2["unidad_venta"]}',
                id_material = '{$value2["id_producto"]}',
                opcionales = '{$value2["tipo_modificador"]}',
                costear = '{$value2["costeo"]}';";
