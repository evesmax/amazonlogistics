...
class repartidores extends Common {
	...
	// Zonas de reparto
		$areas = $this -> repartidoresModel -> listar_areas();
		$areas = $areas['rows'];
	...
	function save_turn($objeto){
		$objeto = (empty($objeto)) ? $_REQUEST : $objeto;
		$result = $this -> repartidoresModel -> save_turn($objeto);
		if ($result) {
			$result['status'] = 1;
		} else {
			$result['status'] = 0;
...
///////////////// ******** ---- 		FIN guardar_asignacion		------ ************ //////////////////
	...
	function reporteDomicilio(){
		
		$empleados 		= $this-> repartidoresModel -> listar_empleados();
		$viacontacto 	= $this-> repartidoresModel -> viacontacto();
		$metodopago 	= $this-> repartidoresModel -> metodopago();
		$sucursales 	= $this-> repartidoresModel -> sucursales();

		require ('views/repartidores/reporteDomicilio.php');
		
	}

	function tablaDomicilio(){		
		$rows = $this -> repartidoresModel -> tablaDomicilio($_POST);
		...
		foreach ($rows as $k => $v){
        	...
        	$rowsR .= '<tr>
        				...
        				<td>'.$v['nombre'].'</td>
        				<td>'.$v['domicilio'].'</td>
        				<td>$'.bcdiv($v['monto'],'1',2).'</td>
        				<td>'.$v['pago'].'</td>
        				<td><button class="btn btn-primary" onclick="window.open(\'../../modulos/pos/ticket.php?idventa='.$v['idVenta'].'\')"><i class="fa fa-print" aria-hidden="true"></i></button></td>
        			</tr>';
        }

        $total = bcdiv($total,'1',2);

		$html = '<div class="pull-right"> <label> Total de Ventas: $'.$total.'</label></div>
				<table id="repartidores">
					<thead>											
                        <tr>
                            <td>ID</td>
                            <td>Comanda</td>                            
