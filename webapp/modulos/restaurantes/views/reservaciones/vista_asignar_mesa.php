		...
		<div class="tab-content ">
			<?php foreach ($areas as $key => $valor) { $si_exist = 0;?>
				...
				<div class="tab-pane <?php if($valor['id'] == $area_princ['id']) { ?>active<?php } ?>" id="tab_area_<?php echo $valor['id']?>">
	          		<?php foreach ($mesas_libres as $key2 => $row) { 
	          			if($row['idDep'] == $valor['id']){ $si_exist = 1;?>

	          				<div class="pull-left" style="padding:5px">
								<button 
									id="btn_mudar_mesa_<?php echo $row['mesa'] ?>" 
									type="button" 
									class="btn btn-default btn-lg"  
									onclick="reservaciones.asignar({
										cliente:<?php echo $objeto['cliente'] ?>,
										des:'<?php echo $objeto['des'] ?>',
										fecha:'<?php echo $objeto['fecha'] ?>',
										mesa: <?php echo $row['mesa']?>,
										id:<?php echo $objeto['id'] ?>,
										num_per: <?php echo $objeto['num_per'] ?>})" 
					...
					// Valida que halla mesas en esa area
						if ($si_exist == 0 && $key2 == (count($mesas_libres) - 1)) {?>
							<div align="center">
								<h3><span class="label label-default">No hay mesas disponibles</span></h3>
							</div><?php
						} ?>
