...
<div style="text-align:left;font-size:12px; font-style: bold;">
	...
	foreach ($comanda['rows'] as $key => $value) {
			...
			if($cc['costo_extra']){
				...
				foreach ($cc['costo_extra'] as $k => $v) { ?>
					...
					<div class="row" style="font-size:13px;font-family: Tahoma,'Trebuchet MS',Arial; font-style: bold;">
						...
						<div class="col-xs-3" style="text-align: center; font-style: bold;">$ <?php echo number_format(round((($v['costo']*1) * $cc['cantidad']), 2),2,'.',',') ?></div>
					</div><?php
		...
		if($comanda['mostrar'] == 1){ 
			$propina = $total_persona * ($que_mostrar["calculo_automatico"]/100); ?>
