	...
	foreach ($comanda['rows'] as $key => $value) {
				...
				<tr><td>
					...
					<?php if ($value['tipo'] != 2 && $value['tipo'] != 1 && is_numeric($value['nombreu'])) { ?>
								...
								<div id="mesa" style="width: 45%; float: left; text-align: left;font-size:10px;font-family: Tahoma,'Trebuchet MS',Arial; font-style: bold;">Mesa: #<?php echo $comanda['rows'][0]['nombre_mesa']; ?></div>
									...
									?>								
											<div id="mesa" style="width: 45%; float: left; text-align: left;font-size:10px;font-family: Tahoma,'Trebuchet MS',Arial; font-style: bold;">Cliente: <?php echo $comanda['rows'][0]['nombre_mesa']; ?></div>									
									<?php
											}
										}else{
									?>
			...
			// Servicio a domicilio
				if($value['tipo'] == "2"){ 
				     ...
				     */
				     if($que_mostrar["mostrar_domicilio"] == 1) { 
				     	if($value['domicilio']){ ?>
				     		<div style="padding-left: 32px; text-align: left; font-size:10px;font-family: Tahoma,'Trebuchet MS',Arial; font-style: bold;">
					    		Domicilio: <?php echo $value['domicilio']; ?>
					    	</div>
					    	<div style="padding-left: 32px; text-align: left; font-size:10px;font-family: Tahoma,'Trebuchet MS',Arial; font-style: bold;">
					    		<?php echo $value['direccion2']; ?> 
					    	</div>
					    	<?php
					 ...
					 if($que_mostrar["mostrar_tel"] == 1) { 
					 	$comanda['tel'] = $value['teldomicilio'];					 		
					 	if($comanda['tel']){ ?>
					 		<div style="padding-left: 32px; text-align: left; font-size:10px;font-family: Tahoma,'Trebuchet MS',Arial; font-style: bold;">
					    		<?php echo $comanda['tel']; ?>
					  		</div><?php
					 	}
					 }
		...
		<table border='0' style="width: 100%;  font-size:9px;  font-family: Tahoma,'Trebuchet MS',Arial;" align="center">
			...
			<tr>
				...
				<td class="col-xs-3" style="width: 40%; text-align: left;"><?php echo $value['nombre'] ?></td>
					...
					<?php 
						if($value['costo_extra'] && $que_mostrar["una_linea"] == 1){ // no entra por que ya se contempla en la misma linea del producto
							...
							foreach ($value['costo_extra'] as $k => $v) { ?>
								<tr>
									<td class="col-xs-3" style="width: 20%; text-align: center; "><?php echo $value['cantidad'] ?></td>
									<td class="col-xs-3" style="width: 40%; text-align: left;">=> Extra: <?php echo $v['nombre'] ?></td>
									<td class="col-xs-3" style=" width: 30%; text-align: center;">$<?php echo $v['costo'] ?></td>
									<td class="col-xs-3" style=" width: 20%;text-align: center; font-style: bold;" >$<?php echo number_format(round(($v['costo'] * $value['cantidad']), 2),2,'.',',') ?></td>													
								</tr>
						<?php			
		...
		<table border='0' style="width: 100%;  font-size:9px;  font-family: Tahoma,'Trebuchet MS',Arial;" align="center">
			...
			foreach ($value['promociones'] as $key5 => $value5) { ?>
				<tr>
					<td class="col-xs-3" style="width: 20%; text-align: center; "></td>
					<td class="col-xs-3" style="width: 40%; text-align: left;"><?php echo $value5['nombre'] ?></td>
					<td class="col-xs-3" style="width: 30%; text-align: center;"></td>
					<td class="col-xs-3" style="width: 20%;text-align: center; font-style: bold;" ></td>													
				</tr>

				<!-- <div class="row" style="font-size:13px; font-family: Tahoma,'Trebuchet MS',Arial; font-style: bold;">
					<div class="col-xs-3" style="text-align: center font-style: bold;"></div>
					<div class="col-xs-3" style="text-align: center;"><?php echo $value5['nombre'] ?></div>
		...
		<?php	///////////// SE COMNETA PARA MOVER DISEÑO A TABLA  	
	 	// if($value['costo_extra'] && $que_mostrar["una_linea"] == 1){ // no entra por que ya se contempla en la misma linea del producto
			// $costo_extra = 0;
			// foreach ($value['costo_extra'] as $k => $v) { 
		?>
					
					<!-- <div class="col-xs-3" align="right" style="font-style: bold;">1</div>
					<div class="col-xs-3" style="font-style: bold;">=> Extra: <?php echo $v['nombre'] ?></div>
