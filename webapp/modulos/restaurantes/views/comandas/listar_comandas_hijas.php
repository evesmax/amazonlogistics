				...
				<tr>
					...
					<td align='center'>
						...
						<img 
							...
							onclick="comandas.imprime_sub_comanda(<?php echo $comanda ?>)"/>
					</td>
					<td align='center'>
					<!-- imprimePedido es una funcion que viene desde js/comandas/reimprime.js -->
						<img 
							...
							title='Pedidos' 
							style='cursor:pointer;' 
							onclick="imprimePedido({id_comanda:'<?php echo $value['idpadre'] ?>', pedidos:'<?php echo $value['pedidos'] ?>', tipo:'<?php echo $value['tipo'] ?>'})">
					</td>
				</tr> <?php
			} ?>
		</tbody>
