							...
							<div class='row' id='layout_row'>
								...
								<div class='col-sm-6'>
									<form action='ajax.php?c=recetas&f=subeLayout' method='post' name='archivo' enctype="multipart/form-data" id='arch' onsubmit='return validar(this)'>
										...
										<button class="btn btn-success btn3" style="float:right; margin-right: 5%;" type='submit'><i id="spin" class="fa fa-refresh fa-spin hidden"></i>Subir</button>
									</form>
								</div>
							</div>
						</div>
...
// Carga la vista para editar las recetas
	...
	function validar(t) {
		if(t.layout.value == '') {
			...
			$(".btn3").text('').append('<i class="fa fa-refresh fa-spin">').prop( "disabled", true );			
		}
	}
