									...
									<tbody><?php
										foreach ($datos['insumos'] as $k => $v) { ?>
											<tr 
												...
												onclick="recetas.agregar_insumo({
...
// Valida que existan servicios
	if (!empty($datos['servicios'])) { ?>
		<div class="row">
			<div class="col-md-12 col-sm-12">
				<div class="panel-group" id="accordion_servicios" role="tablist" aria-multiselectable="true">
					<div class="panel panel-warning">
						<div hrefer class="panel-heading" id="heading_servicios" role="tab" role="button" style="cursor: pointer" data-toggle="collapse" data-parent="#accordion_servicios" href="#tab_servicios" aria-controls="collapse_servicios" aria-expanded="true">
							<h4 class="panel-title">
								<strong>Servicios</strong>
							</h4>
						</div>
						<div id="tab_servicios" class="panel-collapse collapse in" role="tabpanel" aria-labelledby="heading_servicios">
							<div class="panel-body">
								<table id="tabla_servicios" class="table table-striped table-bordered" cellspacing="0" width="100%">
									<thead>
										<tr>
											<th align="center"><strong>Insumo</strong></th>
											<th align="center"><strong>Costo</strong></th>
									...
									<tbody><?php
										foreach ($datos['servicios'] as $k => $v) {
											$preparacion =	utf8_decode($value["preparacion"]); ?>
											
											<tr id="tr_servicio_<?php echo $v['idProducto'] ?>" onclick="recetas.agregar_servicio({												
												preparado:2, 
												id:<?php echo $v['idProducto'] ?>, 
												nombre:'<?php echo $v['nombre'] ?>', 
												tipo_producto:'<?php echo $v['tipo_producto'] ?>', 
												unidad:'<?php echo $v['unidad'] ?>', 
												unidad_compra:<?php echo $v['idunidadCompra'] ?>, 
												costo:<?php echo $v['costo'] ?>, 
												id_unidad:<?php echo $v['idunidad'] ?>, 
												div:'div_insumos_agregados', 
												check:$('#check_servicios_<?php echo $v['idProducto'] ?>').prop('checked')})" style="cursor: pointer">
												<td align="center">
													<?php echo $v['nombre'] ?>
												</td>
												<td>
												...
												<td align="center">
													<input style="cursor: pointer" disabled="1" type="checkbox" id="check_servicios_<?php echo $v['idProducto'] ?>" />
												</td>
											</tr>
											<!--pinta de check el servicio-->

											<?php
											/*
												foreach ($datos['insumos_preparados'] as $kk => $vv) {
													?>
													<script> $("#check_servicios_<?php echo $vv['idProducto'];?>").prop('checked'); </script>
													<?php
												
												}
											*/
											 ?>
											 <!--pinta de check el servicio fin -->
		...
		<script>
		// Convierte la tabla en DataTable
			recetas.convertir_dataTable({id: 'tabla_servicios', botones: 2});
		</script><?php
	} ?>
