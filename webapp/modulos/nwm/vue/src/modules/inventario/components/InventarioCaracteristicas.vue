...
export default {
  name: 'InventarioCaracteristicas',
  ...
  data () {
    return {
      config: {
        thead: ['Almacén', 'Caracteristicas', 'Unidad de medida', 'Método de costeo', 'Monto Unitario', 'Existencia', 'Apartados', 'Disponible'],
        tfoot: ['Almacén', 'Caracteristicas', 'Unidad de medida', 'Método de costeo', 'Monto Unitario', 'Existencia', 'Apartados', 'Disponible'],
        footerCallback:  function( row, data, start, end, display ) {
            ...
            let dataCol = api.column(5).cache('search')
            for (let i = 0 ; i < dataCol.length ; i++)  
              total += Number( dataCol[i].replace(',','') ) 
            $( api.column( 5 ).footer() ).html( numberFormat(total, 4) )
            total = 0;
            dataCol = api.column(6).cache('search')
            for (let i = 0 ; i < dataCol.length ; i++)  
              total += Number( dataCol[i].replace(',','') ) 
            $( api.column( 6 ).footer() ).html( numberFormat(total, 4) )
            total = 0;
            dataCol = api.column(7).cache('search')
            for (let i = 0 ; i < dataCol.length ; i++)  
              total += Number( dataCol[i].replace(',','') ) 
            $( api.column( 7 ).footer() ).html( numberFormat(total, 4) )
        },
        columns: [
          {
