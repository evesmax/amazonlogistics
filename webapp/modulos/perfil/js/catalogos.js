...
function popularTabla(catalogo, parametros, tabla, funcion = "grid")
{
	tabla_modulo.clear().draw();
	$.ajax({
		type: "POST",
	  	url: "../perfil/ajax.php?c="+ catalogo +"&f=" + funcion,
	  	dataType: "json",
	  	data: { },
...
function popularTablaConParametros(catalogo, parametros, tabla, funcion = "grid", async = true)
{
	var total = 0;
	tabla.clear().draw();
	$.ajax({
		...
		async: async,
	  	...
	  	success: function(respuesta){
	  		if(respuesta.status !== undefined && respuesta.status == true){
	  			...
	  			total = respuesta.registros.length;
	  		}else{
	  			mensajeIcono("error", "Un momento...", respuesta.mensaje, function(){});
	  		}
	  	},
...
function editarRegistro(catalogo, identificador)
{
	$('#modal').off().on('shown.bs.modal', function () {
		$.ajax({
			type: "POST",
		  	url: "../perfil/ajax.php?c="+ catalogo +"&f=registro",
		  	dataType: "json",
...
function eliminarRegistro(catalogo, identificador)
{
	$.ajax({
		type: "POST",
	  	url: "../perfil/ajax.php?c="+ catalogo +"&f=estatus",
	  	dataType: "json",
	  	data: { id: identificador },
	  	success: function(respuesta){
	  		if(respuesta.status !== undefined && respuesta.status == true){
...
function guardarRegistro(catalogo, formulario)
{
	if(validarFormulario(formulario)){
		...
		$.ajax({
			type: "POST",
		  	url: "../perfil/ajax.php?c="+ catalogo +"&f=almacenar",
		  	dataType: "json",
		  	data: datosfrm,
		  	processData: false,
		  	contentType: false,
