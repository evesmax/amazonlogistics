<?php
	...
	class CFDIPDF{
		...
		function datosCFD($folioC,$fecha,$tipo,$moneda,$idOV,$solicito,$fecha_entrega){
			...
			if ($tipo=="remision" || $tipo=="Remision") {
				$tipo="Cotizacion";
			} else {
				$tipo=$tipo;
			}
		...
		function terminosCondiciones($terminoscondic){
			$this->terminosCondiciones[0]=$terminoscondic;
		}
			...
			if($l=='.'){
				...
				for($contador=0;$contador<$limit;){
					...
					if($conceptos[$contador] == null || $conceptos[$contador] == ''){
						$conceptos[$contador] = '<tr><td colspan="5"></td></tr>';
					   
					}else{
						 $conceptos[$contador] = '<tr><td colspan="5" style="text-align:center"><img src="'.'../pos/'.$conceptos[$contador].'" height="300" width="600"></td></tr>';
					...
					$this->contentConceptos.='<div><table style="border-collapse:collapse"><tr><td style="font-size:11px;border-bottom:1px dashed '.$this->color.';border-right:1px dashed '.$this->color.';width:81px;text-align:right;padding-right:3px">'.number_format((float)$conceptos[$contador+1],2,'.',',').'</td><td style="font-size:11px;border-bottom:1px dashed '.$this->color.';border-right:1px dashed '.$this->color.';width:81px;padding-left:3px">'.$conceptos[$contador+2].'</td><td style="font-size:11px;border-bottom:1px dashed '.$this->color.';border-right:1px dashed '.$this->color.';width:383px"><div style="width:381px;padding-top:-2px">'.wordwrap($conceptos[$contador+3],76,"</div><div style=\"width:380px;margin-top:3px\">",true).'</div></td><td style="font-size:11px;border-bottom:1px dashed '.$this->color.';border-right:1px dashed '.$this->color.';width:75px;text-align:right;padding-right:3px">'.number_format((float)$conceptos[$contador+4],2,'.',',').'</td><td style="font-size:11px;border-bottom:1px dashed '.$this->color.';width:93px;text-align:right">'.number_format((float)$conceptos[$contador+5],2,'.',',').'</td></tr>'.$conceptos[$contador].'</table></div>';
					    $contador+=6;     
				}
			}else{
				foreach ($cadena as $key => $value) {
					// RCA - Sirve para mostrar la imagen del producto en el pdf
					if ($value['imagen']!=""){ if ($value['imagen']=="images/noimage.jpeg"){ $aimagen=""; } else { $aimagen = '<tr><td colspan="6"  align="center"><img src="../mrp/'.$value['imagen'].'" width="600" height="300"></td></tr>'; } } else { $aimagen = ""; }

					if($key!='cargos' && $value['cantidad'] > 0){
		...
		function generar($imagen, $plantilla=0, $positionPath="",$facturacion=true){
			...
			switch($plantilla){
				case 0:
					// RCA - Se agrego la opcion de Terminos y condiciones al final del documento
					$content.='<tr><td style="font-size:10px;border-bottom:1px dashed '.$this->color.';border-right:1px dashed '.$this->color.';text-align:left">Total</td><td style="font-size:10px;border-bottom:1px dashed '.$this->color.';text-align:right">'.$this->total[1].' '.$this->datosCFD[3].'</td></tr></table></div><div style="margin-left:10px"><table><tr><td style="font-size:11px;border-bottom:1px dashed '.$this->color.';border-right:1px dashed '.$this->color.';padding-left:30px;padding-right:10px">TOTAL EN LETRA:</td><td style="font-size:10px;border-bottom:1px dashed '.$this->color.';padding-left:5px">'.$this->metodoPago[2].$this->datosCFD[3].'</td></tr></table></div><div style="font-weight:bold;font-size:11px;background:'.$this->color.';margin-top:10px;color:#000000;">Observaciones</div><div style="font-size:10px;border:1px solid '.$this->color.';padding:5px"><div style="width:381px;padding-top:-2px">'.wordwrap($this->observaciones,112,"</div><div style=\"width:380px;margin-top:3px\">",true).'</div><div style="width:381px;padding-top:3px;font-size:10px;">'.wordwrap("<b>Teléfono:</b>"." ".$this->telefono[0]).'</div></div><div>'.'<div></div><div></div><div style="font-weight:bold;font-size:13px;background:'.$this->color.';padding-right:10px">T&eacute;rminos y condiciones</div><div style="font-size:11px;background:'.$this->color.';padding-right:10px; text-align:right;"></div><div><table style="border:1px '.$this->color.';"><tr><td style="font-size:10px; text-align: right; width:750px"></td></tr><tr></tr><tr></tr><tr><td style="font-size:10px; text-align: justify; width:750px">'.wordwrap($this->terminosCondiciones[0].'<br /> <br />').'</td></tr></table></div><div></div><div></div><div></div>'.'</div></body></html>';
				break;

				case 1:
					// RCA - Se agrego la opcion de Terminos y condiciones al final del documento						
					$content.='<tr><td style="font-size:10px;border-bottom:1px dashed '.$this->color.';border-right:1px dashed '.$this->color.';text-align:left">Total</td><td style="font-size:10px;border-bottom:1px dashed '.$this->color.';text-align:right">'.$this->total[1].' '.$this->datosCFD[3].'</td></tr></table></div><div style="margin-left:10px"><table><tr><td style="font-size:11px;border-bottom:1px dashed '.$this->color.';border-right:1px dashed '.$this->color.';padding-left:30px;padding-right:10px">TOTAL EN LETRA:</td><td style="font-size:10px;border-bottom:1px dashed '.$this->color.';padding-left:5px">'.$this->metodoPago[2].'</td></tr></table></div><div style="font-weight:bold;font-size:11px;background:'.$this->color.';margin-top:10px">Observaciones</div><div style="font-size:10px;border:1px solid '.$this->color.';padding:5px"><div style="width:381px;padding-top:-2px">'.wordwrap($this->observaciones,112,"</div><div style=\"width:380px;margin-top:3px\">",true).'</div></div><div style="margin-top:10px"><table><tr><td style="font-weight:bold;font-size:11px;background:'.$this->color.';width:392px">Cadena original del complemento de certificacion del SAT</td><td style="font-weight:bold;font-size:11px;background:'.$this->color.';width:352px">Codigo original del complemento de certificacion del SAT</td></tr></table></div><div><table><tr><td style="font-size:8px;width:392px"><d iv><div>'.wordwrap($this->selloDigitales[0],58,"</div><div>",true).'</div></div><div style="font-weight:bold;font-size:11px;background:'.$this->color.';width:387px;margin-top:10px">Sello digital del emisor</div><div style="width:387px"><div>'.wordwrap($this->selloDigitales[1],58,"</div><div>",true).'</div></div><div style="font-weight:bold;font-size:11px;background:'.$this->color.';width:387px;margin-top:10px">Sello digital del SAT</div><div><div>'.wordwrap($this->selloDigitales[2],58,"</div><div>",true).'</div></div></td><td style="font-size:8px;width:352px;border-left:1px dashed '.$this->color.'"><img style="-webkit-user-select:none" src=""></td></tr></table></div><div style="font-size:8px;padding-left:100px"><table><tr><td>ESTE DOCUMENTO ES UNA REPRESENTACION IMPRESA DE UN CFDI</td><td style="padding-left:50px;border-collapse:collapse">Este Documento Fue Generado Utilizando Herramientas De Netwarmonitor</td></tr></table></div><div>Este Documento de creo con herramientas de Netwarmonitor</div> <div>'.'<div></div><div></div><div style="font-weight:bold;font-size:13px;background:'.$this->color.';padding-right:10px">T&eacute;rminos y condiciones</div><div style="font-size:11px;background:'.$this->color.';padding-right:10px; text-align:right;"></div><div><table style="border:1px '.$this->color.';"><tr><td style="font-size:10px; text-align: right; width:750px"></td></tr><tr></tr><tr></tr><tr><td style="font-size:10px; text-align: justify; width:750px">'.wordwrap($this->terminosCondiciones[0].'<br /> <br />').'</td></tr></table></div><div></div><div></div><div></div>'.'</div></body></html>';
					break;
			}
