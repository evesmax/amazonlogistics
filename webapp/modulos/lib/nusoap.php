	...
	function serialize_val($val,$name=false,$type=false,$name_ns=false,$type_ns=false,$attributes=false,$use='encoded',$soapval=false) {
        ...
        if($type != '' && isset($this->typemap[$this->XMLSchemaVersion][$type])){
			...
			if ($use == 'literal') {
				...
				$xml = "<$name$xmlns xsi:type=\"$type\"$atts>$val</$name>";
				$this->debug("serialize_val returning $xml");
	        	return $xml;
        	}
        }
	...
	*/
	...
	function serializeType($name, $type, $value, $use='encoded', $encodingStyle=false, $unqualified=false)
	{
    	...
    	if (is_object($value) && get_class($value) == 'soapval') {
    		...
    		$name = "anyType";
    		$type = "https://cfd.sicofi.com.mx:_ArrayOfAnyType_anyType_ContainedType";
    		if ($value->type_ns) {
			...
			if($ns == $this->XMLSchemaVersion || $ns == 'http://schemas.xmlsoap.org/soap/encoding/'){
				...
				if (is_null($value)) {
					if ($use == 'literal') {
						...
						$xml = "<$name$elementNS xsi:nil=\"true\" xsi:type=\"$uqType\"/>";
					}
				...
				if (!$this->getTypeDef($uqType, $ns)) {
					if ($use == 'literal') {
						if ($forceType) {
							$xml = "<$name$elementNS xsi:type=\"$uqType\">$value</$name>";
						} else {
							$xml = "<$name$elementNS>$value</$name>";
						}
					} else {
						$xml = "<$name$elementNS xsi:type=\"$uqType\"$encodingStyle>$value</$name>";
					}
					$this->debug("in serializeType: returning: $xml");
					return $xml;
				}
		...
		if ($phpType == 'struct') {
			...
			if (is_null($value)) {
				if ($use == 'literal') {
					...
					$xml = "<$elementName$elementNS xsi:nil=\"true\" xsi:type=\"$uqType\"/>";
				}
				$this->debug("in serializeType: returning: $xml");
				return $xml;
			}
			...
			if (is_array($value)) {
				...
				if ($use == 'literal') {
					if ($forceType) {
						$xml = "<$elementName$elementNS$elementAttrs xsi:type=\"$uqType\">";
					} else {
						$xml = "<$elementName$elementNS$elementAttrs>";
					}
				} else {
					
					$xml = "<$elementName$elementNS$elementAttrs xsi:type=\"$uqType\"$encodingStyle>";
				}
		...
		if (isset($typeDef['elements']) && is_array($typeDef['elements'])) {
			...
			foreach ($typeDef['elements'] as $eName => $attrs) {
				...
				if (isset($optionals)
					...
					if (  $eName == "Impuestos" || $eName == "Conceptos" || (isset($attrs['maxOccurs']) && ($attrs['maxOccurs'] == 'unbounded' || $attrs['maxOccurs'] > 1) && isset($v) && is_array($v))) {
						$vv = $v;
						if($eName == "Impuestos") {
							$xml .= "<Impuestos>";
						} else if($eName == "Conceptos") {
							$xml .= "<Conceptos>";
						}
