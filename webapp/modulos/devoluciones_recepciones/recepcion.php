   ...
   //VARIABLES DEL ENVIO
		...
		$conexion->cerrar_consulta($result);                        
         ...
         //Genera Combo Estados de producto
                    $sel="";
                    $cmbestado="<select id=cmbestado name=cmbestado>";
                            $sqlestado="Select idestadoproducto, descripcionestado
                                        from inventarios_estados order by idestadoproducto";

                            $result = $conexion->consultar($sqlestado);
                            while($rs = $conexion->siguiente($result)){
                                    if($rs{"idestadoproducto"}==$idestadoproducto){
                                        $sel=" SELECTED ";
                                    }
                                    $cmbestado.="<Option value=".$rs{"idestadoproducto"}." ".$sel.">".$rs{"descripcionestado"}."</option>";
                                    $sel="";
                            }
                            $conexion->cerrar_consulta($result);  
                    $cmbestado.="</select>";                    
	...
	$html.= "</head>";
        $html.=" <FORM id='envio' name='envio' method='post' action='recepcion_grabar.php'>
                    ...
                    <input type=hidden id='txtiddevolucion' name='txtiddevolucion' value='".$iddevolucion."'>";
		...
		$html.="<table width='100%'>";
				...
				$html.="<td width='30%' align=right>";
                    ...
                    //# POLITICA CONSULTA SI PUEDE EDITAR LA FECHA DE EMISION
                                            $st="";
                                            $sqlbodega="select * from logistica_politicas where idpolitica=10";
                                            $result = $conexion->consultar($sqlbodega);
                                            while($rs = $conexion->siguiente($result)){
...
//DATOS BODEGA ORIGEN
		...
		//Finaliza sección de conceptos
                ...
                $invlight=" style='text-align:right;color:black;background-color: #FFFFFF;border-width:0;font-size: 12px;'";
		...
		$html.="<center><table class='reporte' width='100%'>";
			...
			$html.="
                                <tr class='trencabezado'>
                                    ...
                                    <td colspan=2><center><Input size=20 readonly $invlight type=text value='DIFERENCIA' id=tdif name=tdif><center></td>
                                </tr>";
											...
											document.location.href=pagina;
                                                                                ...
                                                                                function recalcula(factor,edita,saldo) {
                                                                                    var jfactor=0,jfactord=0,jedita=0,jsaldo1=0,jsaldo2=0, 
                                                                                        cant1=0, cant2=0,scant1,scant2,jsaldo=0,vdif=0;
                                                                                    jfactor=factor;
                                                                                    jedita=edita;
                                                                                    jfactord=jfactor;
                                                                                    jsaldo=saldo;
                                                                                    ...
                                                                                    vdif=scant2-cant2;
                                                                                    if(vdif>0){
                                                                                        document.envio.tdif.value='Dif. Se envia a faltantes';
                                                                                    }
                                                                                }
                                                                                function printSelec() {
                                                                                        var c, tmp;
