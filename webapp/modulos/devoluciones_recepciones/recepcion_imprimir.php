<?php
   ...
   //VARIABLES DEL ENVIO
                    ...
                    $observaciones="";
                            ...
                            $idestadoproductod=0;            
                    $sqlestatus="
                            Select lr.idrecepcion, lt.referencia1,lt.fecha fechaot, le.fechaenvio, lr.fecharecepcion,
                                    ...
                                    ip.nombreproducto,ip.idproducto, ifnull(ie.descripcionestado,-1) estadoproducto, 
                                    ...
                                    obd.responsable, le.idenvio, lt.idtraslado, ld.folios, lr.idrecepcion, lt.idestadoproducto, 
                                    ld.cantidadrecibida1 recibida1, ld.cantidadrecibida2 recibida2, ld.diferencia1, ld.diferencia2,
                                    ifnull(ld.idbodega,-1) idbodegareal, ld.estiba estibad, ld.banco bancod, ld.foliosd, ifnull(ld.idestadoproducto,-1) idestadoproductod
		...
		while($rs = $conexion->siguiente($result)){
                        //Asignando Valores del Traslado
                                    ...
                                    $otfc=$rs{"referencia1"};
                                    $fechaotfc=$rs{"fechaot"}; 
                                    $nombreingenio=$rs{"nombrefabricante"};
                                    $idfabricante=$rs{"idfabricante"};
                                    $bodegaorigen=$rs{"bodegadestino"};
                                    $idbodegaorigen=$rs{"idbodegadestino"};
                                    $bodegadestino= $rs{"bodegaorigen"};
                                    $idbodegadestino= $rs{"idbodegaorigen"};
                                    $zafra= $rs{"zafra"};
                                    ...
                                    $saldoinicial= 0;
                                    $retirada= $rs{"enviadabts"};
                                    $recibida= $rs{"recibidabts"};
                                    $devuelta1=$rs{"devueltatm"};
                                    $saldo=$rs{"devueltabts"};
                                    $saldo1=$rs{"devueltatm"};
                                    $tipoimagen="i";
                        ...
                        //Asignando Valores del Envio Procesado
                                    ...
                                    $banco="";
                                    $estiba="";
                                    $cantidad1=$rs{"devueltabts"};
                                    $cantidad2=$rs{"devueltatm"};
                                    $consecutivobodega=0;
                                    $folios=$rs{"folios"};
                       ...
                       //Valores de recepcion de Devoluciones
                                    $recibida1=$rs{"recibida1"};
                                    $recibida2=$rs{"recibida2"};
                                    $diferencia1=$rs{"diferencia1"};
                                    $diferencia2=$rs{"diferencia2"};
                                    $idbodegareal=$rs{"idbodegareal"};
                                    $estibad=$rs{"estibad"};
                                    $bancod=$rs{"bancod"};
                                    $foliosd=$rs{"foliosd"};
                                    $idestadoproductod=$rs{"idestadoproductod"}; 
		}
		$conexion->cerrar_consulta($result);                        
            ...
            //Genera Domicilios Bodega Origen y Destino
                ...
                $conexion->cerrar_consulta($result);                   
				$domiciliobodegadestinoreal="Sin Recibir";
                            //Genera Domicilios Bodega REAL DESTINO
                $sqlbodega="select a.nombrebodega, concat(a.calle,' ', 
                                        case when (a.noexterior is null or a.noexterior='') then '' else concat(' No. ',a.noexterior) end,
                                        case when (a.nointerior is null or a.nointerior='') then '' else concat(' No. Int. ',a.nointerior) end
                                    ,' ',a.colonia) 'domicilio', 
                                mu.nombremunicipio municipio, es.nombreestado estado, 
                                ...
                                inner join general_estados es on es.idestado=a.idestado
                                inner join general_municipios mu on mu.idmunicipio=a.idmunicipio
                                Where a.idbodega=".$idbodegareal;
                $result = $conexion->consultar($sqlbodega);
                while($rs = $conexion->siguiente($result)){
                        $domiciliobodegadestinoreal=$rs{"nombrebodega"}."<br> ".$rs{"domicilio"}." ".$rs{"codigopostal"}." ".$rs{"telefonos"}." ".$rs{"municipio"}." ".$rs{"estado"};
                }
                $conexion->cerrar_consulta($result);    
         ...
         //Genera Combo Transportista
                    ...
                    $conexion->cerrar_consulta($result);    
        		 //echo "Error";
         //Genera Combo Estados de producto
                    $sel="";
                    $cmbestado="";
                            $sqlestado="Select idestadoproducto, descripcionestado
                                        from inventarios_estados 
                                        where idestadoproducto=$idestadoproductod
                                        order by idestadoproducto";
					//echo $sqlestado;
                            $result = $conexion->consultar($sqlestado);
                            while($rs = $conexion->siguiente($result)){
                                    $cmbestado.="<b>".$rs{"descripcionestado"}."</b>";
                                    $sel="";
                            }
                            $conexion->cerrar_consulta($result);  
                    $cmbestado.="";                    
         ...
         //Verifica Politica para Seleccionar a otras bodegas
            $cmbbodega="<b>$domiciliobodegadestinoreal</b>";
	...
	$html.= "</head>";
		...
		$html.="<table width='100%'>";
				...
				$html.="<td width='30%' align=right>";
					...
					//Serie y Folio
					$html.="<tr class='trencabezado'><td><b>RECEPCION - DEVOLUCION</b></td></tr>";
					$html.="<tr>";
					$html.="<td align=center>";
					$html.="	<b>Folio:</b> <font color=blue size=5>$iddevolucion</font>";
					$html.="	</font>";
					$html.="</td>";
					$html.="</tr>";
			...
			$html.="<td width='20%'>";
				$html.="<table class='reporte' width='100%'>";
					...
					$html.="<tr><td align=left>
                                                   ENVIADO: <b>".$retirada." BTS</b><br>
                                                   RECIBIDO: <b>".$recibida." BTS</b><br>
                                                   DEVUELTA:<b>".$devuelta1 ." BTS</b><br>    
                                                   SALDO: <b>".$saldo." BTS</b><br>    
                                                </td></tr>";																			
...
//DATOS BODEGA ORIGEN
			...
			//Obteniendo los datos
				...
				$html.="<tr class=trcontenido>";
					...
					$html.="<td align=center>".$zafra."</td>";
                                        ...
                                        $html.="<td align=center>".$cmbestado."</td>";			
				$html.="</tr>";				
			$html.="</table></center>";	
		$html.="</td></tr>"; //Mega tabla
			...
			//Obteniendo los datos
				...
				$html.="<tr class=trcontenido>";
					...
					$html.="<td align=right>".$cantidad2."<input ".$inv." type=hidden value=".$cantidad2." id='txtcantenv2' name='txtcantenv2' size=20></td>";			
                                        $html.="<td align=right><b>$recibida1</b></td>";
					$html.="<td align=right><b>$recibida2</b></td>";			
		...
		$html.="<tr><td>"; //Mega tabla
                    $html.="<br><br><table width='100%'>";	
                            ...
                            $html.="<td width='20%'>";
                                    $html.="<table class='reporte' width='100%'>";
                                            $html.="<tr class='trencabezado'><td>FIRMA CAPTRISTA</td></tr>";
