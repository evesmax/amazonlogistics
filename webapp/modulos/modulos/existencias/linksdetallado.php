...
set_time_limit($tiempo_timeout);
    ...
    switch ($idsubreporte) {
        ...
        case 8:
			...
			//Declara Matriz de Datos Adicionales de Transitos
			$ea=1;   //Elementos Arreglo
			$totb=0;
			$tott=0;
			$htmldadd=""; //Html donde imprime datos adicionales
			$htmldadd.="<table class=reporte'>
							<tr class='trencabezado' >
								<td>Orden Traslado</td><td>Folio</td><td>Fecha Envio</td><td>Ingenio</td><td>Transportista</td>
								<td>Operador</td><td>Licencia</td><td>Carta Porte</td><td>Placas Tractor</td><td>PLacas Remolque</td>
								<td>Bultos</td><td>Toneladas</td>
							</tr>";
			$dadd=array("ot","folio","fechaenvio","ingenio","transportista",
						"nombreoperador","licenciaoperador","cartaporte","placastractor","placasremolque",
						"bultos","toneladas");


			//Obtiene ya recibidos pero a la fecha estaban en transito
				$sqltr="select tr.referencia1 ot, lr.idrecepcion folio, le.fechaenvio, of.nombrefabricante ingenio,
					ot.razonsocial transportista, le.nombreoperador, le.licenciaoperador, le.cartaporte, le.placastractor,
					le.placasremolque, le.cantidad1 bultos, le.cantidad2 toneladas
					from logistica_recepciones lr left join logistica_traslados tr on tr.idtraslado=lr.idtraslado
						left join logistica_envios le on le.idenvio=lr.idenvio left join operaciones_fabricantes of on of.idfabricante=tr.idfabricante
						left join operaciones_transportistas ot on ot.idtransportista=tr.idtransportista
					where le.fechaenvio<='$fechacorte 23:59:59' and fecharecepcion>='$fechacorte 23:59:59' and tr.idfabricante=$idfabricante and tr.idmarca=$idmarca
						and tr.idloteproducto=$idloteproducto and tr.idbodegadestino=$idbodega and tr.idproducto=$idproducto
						and tr.idestadoproducto=$idestadoproducto";
				//echo $sqltr."<br><br>";		
				$resultado = $conexion->consultar($sqltr);
                while($rs = $conexion->siguiente($resultado)){
                     $dadd["ot"][$ea]=$rs{"ot"};
					 $dadd["folio"][$ea]=$rs{"folio"};
					 $dadd["fechaenvio"][$ea]=$rs{"fechaenvio"}; 
					 $dadd["ingenio"][$ea]=$rs{"ingenio"};
					 $dadd["transportista"][$ea]=$rs{"transportista"};
					 $dadd["nombreoperador"][$ea]=$rs{"nombreoperador"};
					 $dadd["licenciaoperador"][$ea]=$rs{"licenciaoperador"}; 
					 $dadd["cartaporte"][$ea]=$rs{"cartaporte"};
					 $dadd["placastractor"][$ea]=$rs{"placastractor"};
					 $dadd["placasremolque"][$ea]=$rs{"placasremolque"};
					 $dadd["bultos"][$ea]=$rs{"bultos"};
					 $dadd["toneladas"][$ea]=$rs{"toneladas"};

					 $htmldadd.="<tr class='trcontenido' >
									<td class='tdcontenido'>".$dadd["ot"][$ea]."</td>
									<td class='tdcontenido'>".$dadd["folio"][$ea]."</td>
									<td class='tdcontenido'>".$dadd["fechaenvio"][$ea]."</td>
									<td class='tdcontenido'>".$dadd["ingenio"][$ea]."</td>
									<td class='tdcontenido'>".$dadd["transportista"][$ea]."</td>
									<td class='tdcontenido'>".$dadd["nombreoperador"][$ea]."</td>
									<td class='tdcontenido'>".$dadd["licenciaoperador"][$ea]."</td>
									<td class='tdcontenido'>".$dadd["cartaporte"][$ea]."</td>
									<td class='tdcontenido'>".$dadd["placastractor"][$ea]."</td>
									<td class='tdcontenido'>".$dadd["placasremolque"][$ea]."</td>
									<td class='tdcontenido'>".$dadd["bultos"][$ea]."</td>
									<td class='tdcontenido'>".$dadd["toneladas"][$ea]."</td>
								</tr>";
					 
					 $totb=$totb+$rs{"bultos"};
					 $tott=$tott+$rs{"toneladas"};
					 $ea++;
                }        
                $conexion->cerrar_consulta($resultado);

		
			//Obtiene envios sin recibir a la fecha
				$sqltr="Select tr.referencia1 ot, le.idenvio folio, le.fechaenvio, of.nombrefabricante ingenio,
						ot.razonsocial transportista, le.nombreoperador, le.licenciaoperador, le.cartaporte, le.placastractor,
						le.placasremolque, le.cantidad1 bultos, le.cantidad2 toneladas
						from logistica_envios le
						left join logistica_traslados tr on tr.idtraslado=le.idtraslado
						left join operaciones_fabricantes of on of.idfabricante=tr.idfabricante
						left join operaciones_transportistas ot on ot.idtransportista=tr.idtransportista
					where le.fechaenvio<='$fechacorte 23:59:59' and le.idestadodocumento=2 and tr.idfabricante=$idfabricante and tr.idmarca=$idmarca
						and tr.idloteproducto=$idloteproducto and tr.idbodegadestino=$idbodega and tr.idproducto=$idproducto
						and tr.idestadoproducto=$idestadoproducto";
				//echo $sqltr."<br><br>";
				...
				$resultado = $conexion->consultar($sqltr);
                while($rs = $conexion->siguiente($resultado)){
                     $dadd["ot"][$ea]=$rs{"ot"};$dadd["folio"][$ea]=$rs{"folio"};
					 $dadd["fechaenvio"][$ea]=$rs{"fechaenvio"};$dadd["ingenio"][$ea]=$rs{"ingenio"};
					 $dadd["transportista"][$ea]=$rs{"transportista"};$dadd["nombreoperador"][$ea]=$rs{"nombreoperador"};
					 $dadd["licenciaoperador"][$ea]=$rs{"licenciaoperador"};  $dadd["cartaporte"][$ea]=$rs{"cartaporte"};
					 $dadd["placastractor"][$ea]=$rs{"placastractor"};$dadd["placasremolque"][$ea]=$rs{"placasremolque"};
					 $dadd["bultos"][$ea]=$rs{"bultos"};$dadd["toneladas"][$ea]=$rs{"toneladas"};
                ...
                $conexion->cerrar_consulta($resultado);
			$htmldadd.="<tr class='trsubtotal'>
							<td class='tdsubtotal' colspan=10>Total</td>
							<td class='trsubtotal'>".number_format($totb,0)."</td>
							<td class='trsubtotal'>".number_format($tott,3)."</td>
						</tr>";
			$htmldadd.="</table>";
...
echo "Diferencias - Transito <br><br><br>".$htmlr;    
