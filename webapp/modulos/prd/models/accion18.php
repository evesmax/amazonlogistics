...
class Accion18Model extends OrdenPrdModel {
	function historial18($idop, $idap, $opc) {
	...
	function savePaso18($idsProductos, $accion, $idop, $paso, $idap, $idempo, $opc, $ppf) {
	...
	function rebastoInsumos($productos, $solicitante, $obs, $idop, $idap,$matp) {
		date_default_timezone_set("Mexico/General");
		$creacion = date('Y-m-d H:i:s');
		$d = explode('_', $productos);
		$id_reabasto = $this -> insert_id("INSERT INTO prd_reabasto_insumos (id_oproduccion, id_operador, id_pasoaccion, observaciones, estatus, id_matp,fecha)
											VALUES
										( $idop, $solicitante, $idap, '$obs', 1, $matp,'$creacion');");
		if ($id_reabasto > 0) {
			$sql = "";
			foreach ($d as $k => $v) {
				$valores = explode('-', $v);
				$idprod = $valores[0];
				$cant = $valores[1];
				$sql .= "INSERT INTO prd_reabasto_insumos_detalle ( id_oproduccion, id_insumo, cantidad, estatus, id_reabasto)
						VALUES
						( $idop, $idprod, $cant, 1, $id_reabasto);";
			}
			if($this->dataTransact($sql) === true){
				return 1;
			}else{
				$this->query("delete from prd_reabasto_insumos where id=$id_reabasto;");
				return 0;
			}
		}else{
			return 0;
	...
	function reabastoAutorizado($idop){
		$sql = $this->query("select d.*,p.nombre as prd,e.nombreEmpleado from prd_reabasto_insumos_detalle d
							...
							where d.id_oproduccion=$idop and d.estatus=2");
		if($sql->num_rows>0){
			return $sql;
		}else{
			return 0;
