...
// AM
	...
	function departamento(){
		$sql = "select * from app_departamento;";
		$result = $this->query($sql);
	...
	function ciclo(){
		$sql = "select * from prd_ciclos_tipos;";
		$result = $this->query($sql);
	...
	function verproducirproducto($ciclo,$departamento,$familia,$linea){
		...
		if($ciclo != ''){
			$filtrociclo = "AND con.valormerma = $ciclo";
		}
		...
		if($linea != ''){
			$filtrolinea = "AND linea = $linea";
		}



		$sql =$this->queryArray("SELECT * from (select distinct ord.estatus,det.id_orden_produccion,pr.id,pr.nombre as producto,pr.tipo_producto,pr.departamento,pr.familia,pr.linea,pp.id as idpasos, pp.id_producto,
			CASE 
			     WHEN pap.id_accion is null and pp.id_producto is not null  THEN 'Ligero - Sin merma' 
			     WHEN pp.id_producto IS NULL  THEN 'Ningun ciclo'
			     else 'Ligero - Con merma' end as existemerma,
			     CASE 
			     WHEN pap.id_accion is null and pp.id_producto is not null  THEN '1' 
			     WHEN pp.id_producto IS NULL  THEN '0'
			     else '2' end as valormerma  
					from app_productos pr 
					left join prd_pasos_producto pp on pp.id_producto = pr.id 
					left join prd_pasos_acciones_producto pap on pp.id = pap.id_paso 
					and pap.id_accion = 14 
					left join prd_orden_produccion_detalle det on det.id_producto=pr.id left join prd_orden_produccion ord on ord.id= det.id_orden_produccion and ord.estatus=9
				where pr.tipo_producto in(8,9))as con where 1=1  $filtrociclo $filtrodepartamento $filtrofamilia $filtrolinea
				group by id;");
	...
	function  editarproductounico($idseleccionado){
	
		$sql = $this->queryarray("SELECT * from(SELECT distinct ord.estatus,det.id_orden_produccion,pr.id,pr.nombre as producto,pr.tipo_producto,pr.departamento,pr.familia,pr.linea,pp.id as idpasos, pp.id_producto, CASE WHEN pap.id_accion is null and pp.id_producto is not null THEN 'Ligero - Sin merma' WHEN pp.id_producto IS NULL THEN 'Ningun ciclo' else 'Ligero - Con merma' end as existemerma, CASE WHEN pap.id_accion is null and pp.id_producto is not null THEN '1' WHEN pp.id_producto IS NULL THEN '0' else '2' end as valormerma from app_productos pr left join prd_pasos_producto pp on pp.id_producto = pr.id left join prd_pasos_acciones_producto pap on pp.id = pap.id_paso and pap.id_accion = 14 
			left join prd_orden_produccion_detalle det 
			on det.id_producto=pr.id left join prd_orden_produccion ord 
			on ord.id=det.id_orden_produccion 
			and ord.estatus=9  where pr.tipo_producto in(8,9)) as con where id='$idseleccionado' limit 1");
	...
	function asignaciongeneral($ciclo,$masivo,$boton,$id){

		$idProductos =  implode(',', $masivo);
		$idProductos =  substr($idProductos, 0, -1);

		if ($boton=='1') {

			$valordeid = $idProductos;
		...
		$myQuery  = "DELETE from prd_pasos_acciones_producto where id_paso in(".  
		"select id from prd_pasos_producto where id_producto in (".
		$valordeid.
		")) ;";
		$myQuery .= "DELETE from prd_pasos_producto where id_producto  in (".$valordeid.");";
		

		if($this->dataTransact($myQuery) === true){

			$sql="INSERT INTO prd_pasos_producto (descripcion, id_producto,id_ciclo) 
			(select tip.nombre_paso, prod.id, $ciclo
			from prd_ciclos_tipos tip, (select id from app_productos  where id in (".$valordeid.") ) prod
			where id_tipociclo = $ciclo );";
			
			$sql .= "INSERT INTO prd_pasos_acciones_producto (id_paso,id_accion,alias,tiempo,actividad,pieza,tipo,estatus,id_etiqueta)    
			(						 
			Select p.id,id_accion,descripcion_accion,'0',1,0,1,1,0 from prd_ciclos_tipos_detalle ,
			(select id from	 prd_pasos_producto pp where id_producto  in (".$valordeid.")) p
			where id_tipociclo = $ciclo
		);";
...
function guardar_catalogo_ciclos($lasesion,$cicloSeleccionado,$accionesdelpaso,$data,$edicion,$tipociclo){

	if($edicion==1){

		$myQuery  = "DELETE  from prd_ciclos_tipos         where id_tipociclo = $tipociclo;";
		$myQuery .= "DELETE  from prd_ciclos_tipos_detalle where id_tipociclo = $tipociclo;";

		$this->dataTransact($myQuery);

	}

	$sql=$this->queryArray("SELECT MAX(id_tipociclo)  as idtipo FROM prd_ciclos_tipos;");
	$idciclo=$sql["rows"][0]['idtipo'];
	...
	foreach ($data as $k => $v) {
		...
		$sql = "INSERT INTO prd_ciclos_tipos (descripcion, visible, nombre_paso,id_tipociclo) VALUES ('$cicloSeleccionado', 1, $idpaso,$idincrementable)";

		$lastid = $this->insert_id($sql);

		$cad='';
		foreach($accionesdelpaso as $kk =>$vv){

			$cad='';
			if($vv['nombrePaso'] == $v['nombrepaso'] ){

				$cad.="('".$idincrementable."','".$vv['idAccion']."','".$vv['alias']."','".$lastid."'),";

				$cadtrim = trim($cad, ',');

				$sql2="INSERT INTO prd_ciclos_tipos_detalle (id_tipociclo, id_accion,alias,id_pasodetalle) VALUES  ".$cadtrim." ;";
...
// Editar el ciclo de produccion
function editarCatalogociclos($cicloeditar){



	$sql = $this->queryarray("SELECT  id_tipociclo, descripcion from prd_ciclos_tipos group by id_tipociclo;");
...
function listar_pasoscatalogo($ciclo){

	$sql=" SELECT ct.*,td.*,pa.nombre as nombre_accion from prd_ciclos_tipos ct 
 			inner join prd_ciclos_tipos_detalle  td 
	on  ct.id_paso=td.id_pasodetalle
	left  join prd_acciones pa on  pa.id=td.id_accion where ct.id_tipociclo=$ciclo;";
	$result = $this->queryArray($sql);
	return $result;
}

function cicloEliminar($cicloeliminar){
	

		$sql="SELECT id_orden_produccion 
			FROM prd_orden_produccion p
			inner join prd_orden_produccion_detalle d on d.id_orden_produccion=p.id 
			left join prd_pasos_producto pp on pp.id_producto = d.id_producto
			inner join prd_ciclos_tipos ct on ct.id_tipociclo = pp.id_ciclo
			WHERE p.estatus=9 and ct.id_tipociclo = $cicloeliminar";
			...
			$myQuery  = "DELETE from prd_ciclos_tipos where id_tipociclo = $cicloeliminar;";
			$myQuery .= "DELETE from prd_ciclos_tipos_detalle where id_tipociclo =$cicloeliminar;";
