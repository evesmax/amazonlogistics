...
class ConfigModel extends Connection {
	...
	function UpdateActtr($types, $attributes, $peso,$clasificador) {

		$myQuery = "update  app_atributo_producto   
         inner join 
            ( ";
		...
		$myQuery2 = "update  app_tipo_producto   
         inner join 
            ( ";
		foreach ($types as $item) {
			$myQuery2 .= "       Select " . $item -> id . " as id, " . $item -> visible . " as visible, " . $item -> vendible . " as vendible," . $item -> validaxpeso . " as validaxpeso ,".$item->caja_master." as caja_master union all";
		}
		...
		$myQuery2 .= "
                ...
                app_tipo_producto.validaxpeso = valores_atributos.validaxpeso ,
                 app_tipo_producto.caja_master = valores_atributos.caja_master;";
		...
		$sql = "update app_configuracion set usar_bascula=$peso, clasificador_producto=$clasificador;";
		if (!$this -> query($sql)) {
			echo 0;
		} else {
			echo 1;
	...
	function configproductos() {
		...
		$configuracion = $this -> query("SELECT usar_bascula,clasificador_producto FROM app_configuracion order by id asc limit 1;");
		$conf = $configuracion->fetch_object();
		
		return array('attributes' => $attr['rows'], 'types' => $type['rows'], 'conf' => $conf->usar_bascula,'clasifica'=>$conf->clasificador_producto);
