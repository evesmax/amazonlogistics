...
class OrdenPrdModel extends ListadoPrdModel {
	...
	function addProductoProduccion($idProducto) {
		$myQuery = "SELECT a.id, a.codigo, if(a.descripcion_corta='',a.nombre,a.descripcion_corta) as descripcion_corta, a.precio as costo, x.clave, a.tipo_producto,if(a.minimoprod is null,0,a.minimoprod) as minimo, a.factor,cant_x_lote FROM app_productos a
	...
	function editarordenp($idop) {
		...
		$myQuery = "SELECT a.id, SUBSTRING(a.fecha_inicio,1,10) as fi, SUBSTRING(a.fecha_entrega,1,10) as fe, d.idSuc as idsuc, a.solicitante as idsol,d.nombre as sucursal, concat(b.nombre,' ',b.apellidos) as username, a.estatus, a.prioridad, a.observaciones, b.idempleado,a.solicitante as idsol, concat('(',f.codigo,') ',f.nombre) as nombre, e.cantidad, f.peso_dimension,a.lote, e.id_producto 
	...
	function productosOp($idop, $m) {
		...
		if ($m == 1) {
			$myQuery = "SELECT a.*, c.id, c.codigo, c.nombre as nomprod, c.series, c.lotes, c.pedimentos, c.precio as precioorig, x.clave,c.minimoprod as minimos,c.insumovariable,c.cantidadxempaque,c.empaquexcaja
