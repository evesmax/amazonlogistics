...
// Valida que existan combos
	if (empty($datos)) { ?>
		...
		<blockquote style="font-size: 16px">
			<p>
				No se encontraron <strong> combos</strong>
			</p>
		</blockquote><?php
...
<div class="row">
	<div class="col-xs-12">
		<table id="tabla_combos_eliminar" class="table table-striped table-bordered" cellspacing="0" width="100%">
			<thead>
				<tr>
			...
			<tbody><?php
				...
				// Genera una cadena con los nombres de lo productos
					...
					<tr id="tr_eliminar_<?php echo $v['id_combo'] ?>">
						...
						<td align="center"><?php
							...
							<button 
								id="btn_eliminar_<?php echo $v['id_combo'] ?>" 
								data-loading-text="<i class='fa fa-refresh fa-spin'></i>" 
								class="btn btn-danger btn-lg" 
								title="eliminar" 
								onclick="configuracion.eliminar_combo({
											id_combo:<?php echo $v['id_combo'] ?>, 
											btn:'btn_eliminar_<?php echo $v['id_combo'] ?>'
										})">
	...
	<script>
		configuracion.convertir_dataTable({id: 'tabla_combos_eliminar'});
	</script>
</div> 