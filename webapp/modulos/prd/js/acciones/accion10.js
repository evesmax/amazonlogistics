...
function guardarpesocaja(numcaja,idop,empxcaja){
	if(!$("#numpeso").val()){
		alert("Debe pesar la caja.");
	}else{
		disabled_btn('#guardarpeso', 'Guardando...');
		$.post("ajax.php?c=Accion10&f=guardarPesoCaja",{
			idop:idop,
			peso:$("#numpeso").val(),
			empxcaja:empxcaja,
			numcaja:numcaja
...
function savePasoAccion10(accion,idop,paso,idap,idp,sobrante){
	var almacen  = 0;
	if(sobrante>0){
		if( !$("#sobrante").val() ){
	...
	$.ajax({
		url : "ajax.php?c=Accion10&f=a_guardarPaso10",
		type : 'POST',
		data : {
			accion : accion,
			paso : paso,
