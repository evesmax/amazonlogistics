<?php
//$nimps=explode('/(<div class="row">)/', '<div style="width:200px;">', $imps);
$nimps=explode('<div class="row">', $imps);
$table='<table>';
...
footer {
  ...
  text-align: center;
    ...
    <div class="clearfix" style="border-bottom:1px solid #ddd; margin:10px 0 20px 0;">
      ...
      <div class="clearfix" id="details">
        <div id="client">
          ...
          <div class="address">'.$msgcoti.' '.$data['reqdata']['cotizacion'].'</div>
          <div class="address">Fecha de '.$msgcoti.': '.$data['reqdata']['fecha'].'</div>
        </div>
      </div>
      <table width="100%" cellspacing="0" cellpadding="0" border="0">
        ...
        foreach ($data['prodata'] as $k => $v) {
          ...
          $html.='<tr>
            ...
            <td style="width:8px;" class="qty">'.$v['cantidad'].'</td>
            <td style="width:10px;" class="total">$'.($v['costo']*$v['cantidad']).'</td>
          </tr>';
      }
    ...
    require_once('../../modulos/xtructur/html2pdf/html2pdf.class.php');
    $html2pdf = new HTML2PDF('P','A4','fr');
    $html2pdf->WriteHTML($html);
    $html2pdf->Output("coti.pdf","F");
    $path = "coti.pdf";
...
$mail->FromName = MailingModel::fromName();
$mail->Subject = $msgemail;
$mail->AltBody = "NetwarMonitor";
$mail->CharSet = 'UTF-8';
$mail->MsgHTML($msgemail);
$mail->AddAddress($data['reqdata']['email'], $data['reqdata']['email']);
