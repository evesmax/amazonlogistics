...
<div id="modal-conf3" class="modal fade">
    <div class="modal-dialog">
        <div class="modal-content panel-success">
            <div class="modal-header panel-heading">
                <h4 id="modal-label">Pedido Guardado!</h4>
            </div>
            <div class="modal-body">
                <p>El pedido fue guardado exitosamente.</p>
            </div>
        </div>
    </div> 
</div> 
...
<div id="modal-conf1" class="modal fade">
    <div class="modal-dialog">
        <div class="modal-content panel-warning">
            ...
            <div class="modal-body">
                <p>Tienes un pedido sin guardar, ¿Deseas continuar sin guardar cambios?</p>
            </div>
            <div class="modal-footer">
...
<body>
    ...
    <div class="container well" style="padding:25px;">
        <div class="row" style="padding-bottom:20px;">
            <div class="col-xs-12 col-md-12" style="padding:0px 0px 0px 3px;"><h3>Pedidos Internos</h3></div>
        ...
        <div class="row" style="margin-bottom:10px;">
            <button class="btn btn-default" type="button" onclick="nreq();">Nuevo Pedido</button>
            <button class="btn btn-default" type="submit" onclick="listareq();">Lista de Pedidos</button>
        </div>
                ...
                <div class="row">
                    <div class="col-sm-3">
                        ...
                        <select  class="form-control" id="cliente">
                            <option value="0">-Seleccion una Sucursal-</option>
                            <?php 
                           // print_r($ventasIndex['clientes']);
                                foreach ($ventasIndex['sucursales'] as $key1 => $value1) {
                    ...
                    <div class="col-sm-3">
                        ...
                        <select id="empleado" class="form-control">
                            <option value="0">-Seleccion un Solicitante-</option>
                            <?php 
                                foreach ($ventasIndex['empleados'] as $key2 => $value2) {
        ...
        <div id="nreq" class="row" style="display:none;">
            <div class="panel panel-default">
                <div id="ph"  class="panel-heading"><span opt="1" class="label label-primary" style="cursor:pointer;">Nuevo Pedido</span></div>
                <div class="panel-body" style="padding:15px 0 10px 0; font-size:12px;">
                    ...
                    <div class="col-sm-12" style="padding-top:10px;">
                    <div class="form-group">
                        <label class="col-sm-2 control-label text-left">No. Pedido</label>
                        <div id="txt_nreq" class="col-sm-2" style="color:#ff0000;">
                        </div>
                        <label class="col-sm-2 control-label text-left">Fecha Solicitud</label>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-2" style="color:#ff0000;">
                            <select id="c_proveedor" style="width:100%;">
                                <option value="0">Seleccione</option>
                    ...
                    <div id="panel_tabla" class="col-sm-12" style="padding: 15px 37px 15px 31px; display:none;">
                        ...
                        <thead>
                          <tr>
                           <th  align="left" hidden>Seg.</th>
                           <th  width="20%" align="left">Codigo</th>
                           <th  width="20%" align="left">Descripcion</th>
                           <th  width="20%" align="left">Unidad</th>
                           <th  align="left" hidden>$Unitario</th>
                           <th  width="20%" align="left">Cantidad</th>
                           <th align="left " hidden>Subtotal</th>
                           <th width="20%" class="no-sort" align="right">&nbsp;</th>                           
                          </tr>
                        </thead>
                        <tbody id="filasprods">
                        </tbody>
                    ...
                    <div class="form-group">
                        ...
                        -->
                            ...
                            <button id="btn_savequit" class="btn btn-sm btn-success pull-center" type="button" style="height:28px;">Generar pedido</button>
...
return resultado;
    ...
    $(function() {
        ...
        $("#c_proveedor").change(function() {
            ...
            if(idproveedor>0){
                $.ajax({
                    url:"ajax.php?c=ventas&f=a_getProdMoneda2",
                    type: 'POST',
                    dataType: 'JSON',
                    data:{idMoneda:idMoneda,idproveedor:idproveedor},
                    success: function(r){
                        console.log(r);
                        if(r.success==1){
                            llenado='';
                            llenado+='<option value="0">Seleccione</option>';
        ...
        $("#btn_imprimir").click(function() {
            ...
            if(deten==1){
                enabled_btn('#btn_savequit','Generar pedido');
                return false;
            }
        ...
        $("#btn_savequit").click(function() {  
            ...
            idrequi = $('#idrequi').val();
            solicitante=$('#c_solicitante').val();
            idSuc=$('#c_sucursal').val();
            tipogasto=$('#c_tipogasto').val();
            moneda=1;
            proveedor=$('#c_proveedor').val();
            //proveedor=$('#c_proveedores').val();
            //cliente=$('#c_cliente').val();
            ...
            if(idsProductos==''){
                ...
                console.log(idsProductos);
                $.ajax({
                    url:"ajax.php?c=ventas&f=a_guardarRequisicionP",
                    type: 'POST',
                    data:{                        
                        idsProductos:idsProductos,
                        solicitante:solicitante,
    ...
    */
        ...
        $("#btn_addProd").click(function() {             
            ...
            if(idProducto>0){
                ...
                type: 'POST',
                dataType:'JSON',
                data:{idProducto:idProducto,idCliente:idCliente,idProveedor:idProveedor,pedidoInterno:1},
                success: function(r){
                    console.log(r);
                    if(r.success==1){
                        if(r.car!=''){ // caracteristicas
                            ...
                            $('#modal-car-uno').on('click',function(){
                                ...
                                */ //ch@
                                ...
                                if((ctipodesc == 2 || ctipodesc == 3) && false){ /// para no tener descuento en pedidos internos
                                ...
                                var Rowdata = "<tr ch='"+cadcar+"' 'newp'='0' oldp='0' tipoDesc='0' montoD ='0' id='tr_"+r.datos[0].id+"'>"+                                    
                                    "<td hidden>0</td>"+
                                    "<td>"+r.datos[0].codigo+"</td>"+
                                    "<td style='cursor:pointer;' onclick='modalDescuento("+r.datos[0].id+",\""+cadcar+"\");'>"+r.datos[0].descripcion_corta+" "+cadcartxt+"</td>"+
                                    "<td>"+r.datos[0].clave+"</td>"+
                                    "<td id='valUnit' hidden>"+r.adds+"</td>"+
                                    "<td><input style='width:60%;' onkeyup='refreshCants("+r.datos[0].id+",\""+cadcar+"\",1)' class='numeros' type='text' value='1'/></td>"+
                                    "<td hidden class='valImporte' implimpio='"+r.datos[0].costo+"' id='valImporte'>"+r.datos[0].costo+"</td>"+
                                    "<td><button onclick='removeProdReq("+r.datos[0].id+",\""+cadcar+"\");' id='btn_addProd' class='btn btn-sm btn-danger' type='button' style='height:26px;'>Quitar</button>"+btndescP+"</td>"+
                                "</tr>";
                            ...
                            if((ctipodesc == 2 || ctipodesc) == 3 && false){ /// para no tenr descuento enpedidos internos
                            ...
                            var Rowdata = "<tr ch='0' newp='0' oldp='0' tipoDesc='0' montoD='0' id='tr_"+r.datos[0].id+"'>"+
                                "<td hidden>0</td>"+
                                "<td>"+r.datos[0].codigo+"</td>"+
                                "<td style='cursor:pointer;' onclick='modalDescuento("+r.datos[0].id+",0);'>"+r.datos[0].descripcion_corta+"</td>"+
                                "<td>"+r.datos[0].clave+"</td>"+
                                "<td id='valUnit' hidden>"+r.adds+"</td>"+
                                "<td><input style='width:60%;' onkeyup='refreshCants("+r.datos[0].id+",0,1)' class='numeros' type='text' value='1'/></td>"+
                                "<td hidden class='valImporte' implimpio='"+r.datos[0].costo+"' id='valImporte'>"+r.datos[0].costo+"</td>"+
                                "<td><button onclick='removeProdReq("+r.datos[0].id+",0);' id='btn_addProd' class='btn btn-sm btn-danger' type='button' style='height:26px;'>Quitar</button>"+btndescP+"</td>"+
                            "</tr>";
    ...
    function nreq(){
        // INICIALIZA CAMPOS CH@
        $("#c_sucursal, #c_proveedor").val(0).trigger("change");
                ...
                $.ajax({
                    ...
                    success: function(r){
                        if(r.success==1){
                            ...
                            $('#ph').html('<span opt="1" class="label label-primary" style="cursor:pointer;">Nuevo Pedido</span>');
                            $('#nreq').css('display','block');
                    ...
                    if(r.success==1){
                        ...
                        $.each(r.productos, function( k, v ) {
                            ...
                            if(mod==0){
                                ...
                                if((ctipodesc == 2 || ctipodesc == 3) && false){ // no descuentos
                                    if(desc > 0){
                            ...
                            Rowdata="<tr ch='"+v.caracteristica+"' id='tr_"+v.id+"' oldp='"+v.precioorig+"' tipodesc='"+v.tipo_desc+"' montod='"+v.monto_desc+"' >"+                                
                                "<td hidden>0</td>"+
                                "<td>"+v.codigo+"</td>"+
                                "<td>"+v.nomprod+"</td>"+
                                "<td>"+v.clave+"</td>"+
                                "<td id='valUnit' hidden>"+v.adds+"</td>"+
                                "<td><input style='width:60%;' onkeyup='refreshCants("+v.id+",\""+v.caracteristica+"\",1)' class='numeros' type='text' value='"+v.cantidad+"'/></td>"+
                                "<td hidden class='valImporte' implimpio='"+v.costo+"' id='valImporte'>"+v.costo+"</td>"+
                                "<td>"+eliminProd+" "+btndescPE+"</td>"+
                            "</tr>";
                            table.row.add($(Rowdata)).draw();
            ...
            $('#modal-btnconf2-uno').on('click',function(){
                ...
                $('#example').DataTable( {
                    ...
                    ajax: {
                        ...
                        url:"ajax.php?c=ventas&f=a_listaRequisicionesP",
                        type: "POST",
                        data:  {
                            cliente : $('#cliente').val(),
        ...
        */
        ...
        .done(function(data) {
            ...
            $('#imps').append('<div class="row">'+btndescGR+                            
                            '<div hidden class="col-sm-6"><label>Subtotal:</label></div>'+
                            '<div hidden class="col-sm-6"><label>$'+subtotal+'</label></div>'+
                            '</div>');
            ...
            $.each(data.cargos.impuestosPorcentajes, function(index, val) {
                ...
                $('#imps').append('<div class="row">'+
                            '<div hidden class="col-sm-6"><label>'+index+':</label></div>'+
                            '<div hidden class="col-sm-6"><label>$'+valF+'</label></div>'+
                            '</div>');   
            });
            ...
            $('#total').val(total); //ch@
            $('#imps').append('<div class="row">'+
                            '<div hidden class="col-sm-6"><label>Total:</label></div>'+
                            '<div hidden class="col-sm-6"><label id="lbtotal">$'+total+'</label></div>'+
                            '</div>');
