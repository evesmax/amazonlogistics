...
function modificar_listaprec(id)
{
    ...
    $.post('ajax.php?c=configuracion&f=datos_listaprec', 
        ...
        function(data)
        {
            ...
            $("#status").val(datos[4])
            $("#selectTipoPrecio").val(datos[5])
            $("#blanco").hide()
            if( $('#selectTipoPrecio').val() == 1){
                $('#rowPorcentaje , #rowDescuento').show();
            }else  {
                $('#rowPorcentaje , #rowDescuento').hide();
                $("#porcentaje").val(0);
                $('#descuento').val(0);
            }
        });
