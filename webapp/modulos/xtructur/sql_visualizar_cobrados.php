...
if($filtro_semana!=0){
    ...
    $cadenafiltro.=" AND yearweek(b.fecha,1)='".$filtro_semana."' ";
}

if($filtro_mes!=0){
    $ym = explode('-', $filtro_mes);
    $cadenafiltro.=" AND YEAR(b.fecha) = '".$ym[0]."' AND MONTH(b.fecha) = '".$ym[1]."'";
}
...
if($filtro_proveedor!=0){
    $cadenafiltro2=" AND a.mpago = '".$filtro_proveedor."' ";
    
}


$i=0;
$SQL2="SELECT a.fp as metpag, a.id_esti, a.imp_sem, a.semana, a.proviene, a.id as idcobros,concat('Cob-',b.id,' Fecha: ',b.fecha) as cobro FROM constru_cobros a 
inner join constru_bit_cobros b on b.id=a.id_bit_cobros AND a.imp_sem>0
where  a.id_obra='$id_obra' ".$cadenafiltro.";";
...
while($row2 = $result2->fetch_array()) {
    ...
    if($row2['proviene']=='ESTCLI'){
           $SQL= "SELECT b.id_alta, a.id as iidd, a.id_cliente, a.fecha, concat('CLI-',b.id_alta,' - ',b.nombre,' ',b.paterno,' ',b.materno) Cliente, concat('ESTCLI-',a.id) estimacion, a.factura, a.imp_estimacion, c.no_cheque, c.banco,
concat('<input value=\"',0,'\" id=\"',a.id_cliente,'\" class=\"quis__\" name=\"',a.id,'\">') cant
FROM constru_estimaciones_bit_cliente a
left join constru_info_tdo b on b.id_alta=a.id_cliente
left join constru_cheques2 c on c.id_cobro=a.id
...
$row = $result->fetch_array();
        ...
        $SQL3 ="SELECT if(sum(a.imp_sem) is null,0,sum(a.imp_sem)) as imp_sem from constru_cobros a 
         where a.id_obra='$id_obra'  AND a.id_esti='".$row['iidd']."' and a.proviene='".$row2['proviene']."' AND a.proviene='".$row2['proviene']."';";
        ...
        $SQL5 ="SELECT if(a.imp_sem is null,0,a.imp_sem) as imp_semana, a.id as ididremesa from constru_cobros a 
         where a.id_obra='$id_obra'  AND a.id_esti='".$row['iidd']."' AND a.semana='".$row2['semana']."'  AND a.proviene='".$row2['proviene']."';";
        $result5 = $mysqli->query($SQL5);
        $row5 = $result5->fetch_array();
        $semsem=$row5['imp_semana'];

        $cad.=' <button class="btn btn-primary btn-xs" onclick="addCobro('.$row2['idcobros'].','.$row2['metpag'].','.$row2['id_esti'].');">Agregar cobro</button> ';
        ...
        //$cad.="<input style='font-size:10px;' type='checkbox' value='".$row['iidd']."' class='ccbox' style='cursor:pointer;' id='".$row['iidd']."' noremesa='".$row5['ididremesa']."' > ";


       // $cad.=$row['estimacion'].' / '.$row['Proveedor'].' /  Importe: '.$row['total'].' / Pagado: '.$sumap.' / Cheque: '.$semsem;

        $cad.=$row['estimacion'].' / '.$row['Cliente'].' /  Importe: '.$row['imp_estimacion'].' / Cobrado: '.$semsem.'';

        
             $SQL4 ="SELECT a.*,b.nombre as pago from constru_cheques2 a
             JOIN forma_pago b on b.idFormaPago=a.mpago 
         where a.id_obra='$id_obra' ".$cadenafiltro2." AND a.cobro='".$row2['idcobros']."';";
        ...
        if($result4->num_rows>0){
            while($row4 = $result4->fetch_array()) {
              ...
              if($stfa==2) $txt_stfa='Pago parcial';
               ...
               $responce->rows[$i]['id']=$row4['id'];
                $responce->rows[$i]['cell']=array($row2['cobro'],$row['Cliente'],$cad,$row4['no_cheque'],$row4['banco'],$row4['fecha_expedicion'],$txt_stche,$txt_stfa,$mpago,$row4['fecha']);
                $i++;
            ...
            $responce->rows[$i]['cell']=array($row2['cobro'],$row['Cliente'],$cad);
            $i++;
        }
