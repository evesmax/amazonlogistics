...
if(isset($oper) && $oper=='del'){
    $id = $_POST['id'];
    $mysqli->query("UPDATE constru_estimaciones_indirectos SET borrado=1 WHERE id in ($id);");
    exit();
}
...
if(isset($oper) && $oper=='edit'){
    ...
    $mysqli->query("UPDATE constru_estimaciones_indirectos SET clave='$clave', concepto='$concepto', unidtext='$unidtext',cantidad='$cantidad',pu_indirecto='$pu_indirecto' WHERE id='$id';");
    exit();
}


$SQL="SELECT COUNT(*) AS count FROM constru_estimaciones_indirectos WHERE id_obra='$id_obra' AND sestmp='$sestmp' AND id_bit_indirectos=0 and borrado=0;";
$result = $mysqli->query($SQL);
...
if($_search=='true'){
 ...
 $SQL = "SELECT a.id, a.clave, a.concepto, a.unidtext, a.cantidad, a.pu_indirecto, a.pu_indirecto*a.cantidad as importe
 from constru_estimaciones_indirectos a 
 WHERE a.id_obra='$id_obra' AND a.sestmp='$sestmp' AND a.id_bit_indirectos=0 AND a.borrado=0  ORDER BY a.$sidx $sord LIMIT $start,$limit";
$result = $mysqli->query($SQL);
