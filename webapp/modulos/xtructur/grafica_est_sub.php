...
$filtro1=' and a.id_subcontratista='.$p.' ';
...
$filtro2=' and a.estatus='.$s.' ';
   ...
   $categorias="SELECT a.id, sum(a.imp_estimacion), month(a.fecha) as mes,year(a.fecha) as year FROM constru_estimaciones_bit_subcontratista a where a.id_obra='$id_obra' and a.borrado=0".$filtro1.$filtro2." group by year,mes order by year,mes asc";
    $series="SELECT a.id, sum(a.imp_estimacion) as sestmp, month(a.fecha) as mes,year(a.fecha) as year FROM constru_estimaciones_bit_subcontratista a where a.id_obra='$id_obra' and a.borrado=0".$filtro1.$filtro2. " group by year,mes order by year,mes asc";
...
<html>
    <head>
        ...
        <title>Estimaciones Subcontratistas</title>
...
$(function () {
    $('#container').highcharts({
        ...
        title: {
            text: 'Estimaciones  Subcontratista '+ '<?php echo $obra; ?>'+'<br> Subcontratista: '+'<?php echo $pt; ?>'+' <br>Estatus: '+'<?php echo $st; ?>',
            x: -20 //center
        },
        subtitle: {
            text: '',
