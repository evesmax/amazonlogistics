...
CASE a.estatus
WHEN 3 THEN concat('Entradas Traspaso-',a.id ,' Obra ',d.obra,' Fecha: ',a.fecha_solicito,' <button type=\"button\" value=\"Cancelar\" style=\"cursor:pointer\" data-toggle=\"modal\" data-target=\"#myModal\"  data-eid=',a.id, '>Cancelar</button> ',' <input type=\"button\" value=\"Autorizar\" style=\"cursor:pointer\" onclick=\"autorizarestAll(\'tent\',',a.id,',',4,');\" >')
WHEN 2 THEN concat('Entradas Traspaso-',a.id ,' Obra ',d.obra,' Fecha: ',a.fecha_solicito,' <font color=\"#ff0000\">Entrada Traspaso Cancelada</font>')
WHEN 4 THEN concat('Entradas Traspaso-',a.id ,' Obra ',d.obra,' Fecha: ',a.fecha_solicito,' <font color=\"#070\">Entrada Traspaso Autorizada</font>')
END as Estimacion
from constru_traspasos b
left join constru_generales d on d.id=b.id_obra_ent
left join constru_bit_traspasos a on a.id=b.id_bit_traspaso
left join constru_insumos c on c.id=b.id_clave
where b.id_obra_ent='$id_obra' and a.id_quien_oe='$idusr'
