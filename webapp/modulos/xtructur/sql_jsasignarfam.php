...
$SQL = "SELECT a.*, (a.unidad*a.precio) as tc, (a.unidad*a.precio) as tv, if(a.id_familia=0,'No asignada',b.nomfam) as familia FROM constru_insumos a left join constru_famat b on b.id=a.id_familia WHERE 1=1 ".$cad." AND a.id_obra='$id_obra' AND a.borrado=0 ORDER BY a.id LIMIT $start,$limit";
$result = $mysqli->query($SQL);
...
while($row = $result->fetch_array()) {
    ...
    $responce->rows[$i]['cell']=array($row['naturaleza'],$row['clave'],$row['descripcion'],$row['unidtext'],$row['unidad'],$row['precio'],$row['tc'],$row['familia']);
    $i++;
}        
