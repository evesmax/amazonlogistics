...
a.fecha_entrega, c.clave, c.descripcion, c.unidtext,  concat('<input disabled rrr=\"1\" type=\"text\" class=\"quis__',a.id,'\" value=\"',b.cantidad,'\" id=\"', b.id,'\" />') as Rcantidad, c.precio, b.cantidad*c.precio importe, a.id reqid, b.id reqsid, c.id insuid, f.partida, h.especialidad, 
...
WHERE a.id_area is not null AND a.estatus=3 or a.estatus=4 and a.id_obra='$id_obra' AND a.borrado=0 AND b.borrado=0  ORDER BY a.id DESC, b.id, $sidx $sord LIMIT $start,$limit";
$result = $mysqli->query($SQL);
...
END as Requisicion, es.nombre as anom,
-- concat('REQ-',a.id) as Requisicion, 
a.fecha_entrega, c.clave, c.descripcion, c.unidtext,  concat('<input type=\"text\" class=\"quis__',a.id,'\"  disabled value=\"',b.cantidad,'\" id=\"', b.id,'\" />') as Rcantidad, c.precio, b.cantidad*c.precio importe, a.id reqid, b.id reqsid, c.id insuid, f.partida, h.especialidad,
case when a.fecha_utilizacion is null or date(a.fecha_utilizacion)='0000-00-00' then concat(emp.nombre,' ',emp.apellidos)
else concat(emp.nombre,' ',emp.apellidos,' - Fecha requerida de entrega ',substr(a.fecha_utilizacion,1,10))
end as Solicito,
CASE a.estatus 
