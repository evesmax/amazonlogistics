						...
						<table border="1">
							<tr>
								...
								<td width="122">Importe</td>
					    			...
					    			$SQL = "SELECT c.clave, c.descripcion, sum(b.cantidad) as cantidad, c.unidtext, c.precio, b1.fecha_captura, b.precio_compra, sum((b.precio_compra*b.cantidad)) as importexx,
					    			 ...
					    			 FROM constru_pedis a
										...
										-- left JOIN constru_info_tdo d on d.id_alta=a.solicito
										left JOIN constru_info_sp e2 on e2.id_alta=a.id_prov
										WHERE a.id_obra='$id_obra' AND a.borrado=0 AND b1.borrado=0 and a.id='$id' group by c.clave, b.precio_compra;";	
...
if(isset($opcion) && $opcion=='desc_insumos_matriz'){
	$return=array();
	$id_insumo=$_POST['id_insumo'];
	...
	$id_agrupador=$_POST['id_agrupador'];
	 ...
	 $SQL = "SELECT a.clave,a.descripcion,a.unidtext, if(b.cantidad*d.unidad is null,0,b.cantidad*d.unidad) as totcant FROM constru_insumos a
inner join constru_puconcepto b on b.id_insumo=a.id and b.id_obra=a.id_obra
inner join constru_bit_puconcepto c on c.id=b.id_bit_puconcepto
inner join constru_recurso d on d.id=c.id_concepto
WHERE a.id_obra='$id_obra' AND a.id='$id_insumo' AND a.borrado=0 AND c.id_concepto='$id_concepto';";
	$result = mysql_query( $SQL ) or die("Couldn t execute query.".mysql_error());
	if(mysql_num_rows($result)>0){
		while($row = mysql_fetch_assoc($result)) {
		    $return[]=$row;
		    ...
		    $SQL = "SELECT if( sum(a.cantidad) is null,0,sum(a.cantidad) ) as vol_est from constru_requisiciones a 
			Inner join constru_requis b on b.id=a.id_requi AND b.estatus!=2
			where a.id_requi>0 AND a.sestmp>0 AND a.id_obra='$id_obra' AND a.id_clave='$id_insumo'
			and a.id_agru='$id_agrupador' and a.id_area='$id_area' and a.id_esp='$id_especialidad' and a.id_part='$id_partida' and a.id_concepto='$id_concepto'
			  -- AND b.id_destajista='$id_des' 
			 order by a.id DESC LIMIT 1;";
			$result3 = mysql_query( $SQL ) or die("Couldn t execute query.".mysql_error());
			$row3 = mysql_fetch_assoc($result3);
			if(mysql_num_rows($result3)>0){
			...
			and c.id_agru='$id_agrupador' and c.id_area='$id_area' and c.id_esp='$id_especialidad' and c.id_part='$id_partida' and c.id_concepto='$id_concepto'
			group by a.id_req;";
...
if(isset($opcion) && $opcion=='insumos_con_matriz'){
	$return=array();
	$id_concepto=$_POST['id_concepto'];

	$SQL = "SELECT c.id, a.cantidad, c.descripcion, c.unidtext, c.precio from constru_puconcepto a 
	inner join constru_bit_puconcepto b on a.id_bit_puconcepto=b.id
	inner join constru_insumos c on c.id=a.id_insumo and c.id_obra=b.id_obra
	WHERE b.id_concepto='$id_concepto' and b.borrado=0 and b.id_obra='$id_obra';";
...
if(isset($opcion) && $opcion=='concepto_dinamic_combo'){
	$id_concepto=$_POST['id_concepto'];

	$SQL = "SELECT c.id, concat(c.clave,' - ',SUBSTRING(c.descripcion,1,100),'...') clave from constru_puconcepto a 
inner join constru_bit_puconcepto b on a.id_bit_puconcepto=b.id
inner join constru_insumos c on c.id=a.id_insumo and c.id_obra=b.id_obra
WHERE b.id_concepto='$id_concepto' and b.borrado=0 and b.id_obra='$id_obra';";
	...
	foreach ($o as $key1 => $oentrada) {
		...
		foreach ($entrada as $key => $value) {
			...
			if (array_key_exists($idpro, $arraypro)) {
			    ...
			    $arraypro[$v[4]][]=array('q0'=>$q[0], 'v0'=>$v[0], 'v1'=>$v[1], 'v2'=>$v[2], 'v3'=>$v[3], 'v5'=>$v[5], 'v6'=>$v[6]);
			}else{
	...
	foreach ($arraypro[$elprov] as $k => $v) {
		$SQL = "SELECT id FROM constru_pedidos WHERE id_obra='$id_obra' AND id_pedid='$last_id' AND id_requis='".$v['q0']."';";
			...
			if($v['v6']==''){
				...
				$add="  AND id_concepto='".$v['v6']."'  ";
			}

			$SQL="UPDATE constru_requisiciones SET id_pedido='".$last_id."', precio_compra='".$v['v2']."', cantidad='".$v['v3']."', elprov='".$elprov."' WHERE id_obra='$id_obra' AND id_requi='".$v['v0']."' AND id_clave='".$v['v1']."' ".$add." ; ";
			mysql_query( $SQL ) or die("Couldn t execute query.".mysql_error());
...
from constru_pedis a 
left join constru_pedidos b on b.id_pedid=a.id
left join constru_requis c on c.id=b.id_requis
left join constru_requisiciones d on d.id_requi=c.id
left join constru_insumos g on g.id=d.id_clave
left join constru_partida e on e.id=c.id_part
left join constru_cat_partidas hh on hh.id=e.id_cat_partida
left join constru_especialidad f on f.id=c.id_area
left join constru_generales h on h.id=$id_obra
left join administracion_usuarios i on i.idempleado=a.solicito
WHERE a.id='$last_id';";
     $result=mysql_query($SQL2) or die("Couldn t execute query.".mysql_error());
    while($row = mysql_fetch_assoc($result)) {
...
foreach ($addresses as $address) {
    $mail->AddAddress($address);
            ...
            $mail->Subject = "Se ha creado una Orden de compra";
            $mail->AltBody = "Xtructur";
            $mail->MsgHTML($data);
...
if(isset($opcion) && $opcion=='save_requi_matriz'){
    $return=array();
    $ag=0;
    $ar=0;
    $es=0;
    ...
    $SQL="INSERT INTO constru_requis (id_obra,solicito,estatus,fecha_entrega,id_agru,id_area,id_esp,id_part,xxano,fecha_captura,fecha_utilizacion,obs,matriz) VALUES ('$id_obra','$iduserlog',1,'$fecha_entrega','$ag','$ar','$es','$pa','$xxano','$fecha','$fecha_utilizacion','$obs',1);";
    mysql_query( $SQL ) or die("Couldn t execute query.".mysql_error());
    $last_id = mysql_insert_id();
...
left join constru_requisiciones d on d.id_requi=c.id
left join constru_insumos g on g.id=d.id_clave
left join constru_especialidad f on f.id=d.id_area
left join constru_generales h on h.id=$id_obra
left join administracion_usuarios i on i.idempleado=c.solicito
WHERE c.id='$last_id';";
     $result=mysql_query($SQL2) or die("Couldn t execute query.".mysql_error());
    while($row = mysql_fetch_assoc($result)) {
					...
					<br><br><br><br><br>
						...
						<table border="1">
								...
								<td width="122">Importe</td>
									...
									while($row = mysql_fetch_array($result)) {
										...
										/*if($row['cantvalida']==null){
								    		...
								    		$importe_concurso = $cantidad * $precio_concurso;
								    		$total_concurso = $importe_concurso + $total_concurso;	
								    		$precio_concursoF = number_format($precio_concurso,4);
								    		$importe_concursoF = number_format($importe_concurso,4);
								    		$total_concursoF = number_format($total_concurso,4);

								    		$precio_compra = $row['precio_compra'];
								    		$importe_compra = $cantidad * $precio_compra;
								    		$total_compra = $importe_compra + $total_compra;
								    		$precio_compraF = number_format($precio_compra,4);
								    		$importe_compraF = number_format($importe_compra,4);
								    		$total_compraF = $total_compra;
							...
							<tr>
								...
								<td width="265"style="font-size:10px;">'.$descripcion.'</td>
								<td style="text-align:right;">$'.$precio_compraF.'</td>
								<td style="text-align:right;">$'.number_format($importexx,2).'</td>
					    	</tr>';}
					    	$content.='
