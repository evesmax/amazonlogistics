...
$ingacumuant=0.00;
  ...
  foreach ($weekNumbers as $k => $r) {
      # code...
  //}
//for($r=1; $r<=52; $r++){

    $SQL = "SELECT sum(a.rem_aut) as rem_aut, '$r' as yw from constru_bit_remesas a
    WHERE a.id_obra='$id_obra' AND a.fecha between STR_TO_DATE('".$r." Monday', '%x%v %W') AND STR_TO_DATE('".$r." Sunday', '%x%v %W') GROUP BY yw ORDER BY yw desc;";
    $result = $mysqli->query($SQL);


    $SQLx = "SELECT sum(a.imp_estimacion) as imp_estimacion, '$r' as yw from constru_estimaciones_bit_cliente a
    WHERE a.id_obra='$id_obra' AND a.fecha between STR_TO_DATE('".$r." Monday', '%x%v %W') AND STR_TO_DATE('".$r." Sunday', '%x%v %W') AND a.estatus=1 GROUP BY yw ORDER BY yw desc;";
