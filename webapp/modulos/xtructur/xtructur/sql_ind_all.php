...
$SQL="SELECT e.clave, 
CASE a.estatus 
WHEN 0 THEN concat('Estimacion-',a.id,' Fecha: ',substr(fecha,1,10),' ','<input type=\"button\" value=\"Cancelar\" style=\"cursor:pointer\" onclick=\"autorizarestAll(\'ind\',',a.id,',',2,');\" > ',' <input type=\"button\" value=\"Autorizar\" style=\"cursor:pointer\" onclick=\"autorizarestAll(\'ind\',',a.id,',',1,');\" >')
...
while($row = $result->fetch_array()) {
    ...
    $responce->rows[$i]['cell']=array($row['Estimacion'],$row['clave'],$row['concepto'],$row['unidtext'],$row['cantidad'],$row['pu_indirecto'],$row['importe']);
    $i++;
}        
