...
WHERE 1=1 ".$cad." AND
 a.id_obra='$id_obra' AND a.borrado=0
...
while($row = $result->fetch_array()) {
	...
	$almacen=$almacen-$row['salidatras'];
    $responce->rows[$i]['id']=$row['id'];
    $responce->rows[$i]['cell']=array($row['naturaleza'],$row['clave'],$row['descripcion'],$row['unidtext'],$row['unidad'],$row['sumallego'],$row['sumasalio'],$row['entradatras'],$row['salidatras'],$restante,$almacen);
    $i++;
}        
