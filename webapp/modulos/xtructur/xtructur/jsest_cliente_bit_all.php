...
concat(bx.cliente,' EST-',b.id, ' SEM: ',b.xxano ) as Cliente, b.id
    ...
    WHERE b.id_obra='$idses_obra' AND a.sestmp>0  AND b.borrado=0 group by b.id";
      ...
      $SQL = "SELECT a.cliente as nombre FROM constru_generales a  where a.id='$idses_obra' and a.borrado=0 ;";
  $result = $mysqli->query($SQL);
  if($result->num_rows>0) {
    while($row = $result->fetch_array() ) {
      $proveedores[]=$row;
...
<div class="row">&nbsp;</div>
    ...
    <button onclick="graficar_ret('est_cliente','<?php echo $idses_obra; ?>');" class="btn btn-primary btn-xm"> Graficar</button>
