...
if(isset($oper) && $oper=='add'){
	$id_clave = $_POST['id_recurso'];
   	$SQL="SELECT id FROM constru_bit_solicitudes WHERE id_obra='$id_obra'  and borrado=0 AND id='$id_clave';";
	$result = $mysqli->query($SQL);
	if($result->num_rows>0) {
		echo 'RP';
		exit();
	}
	$codigo = 'EXT-'.$_POST['Codigo'];
...
$SQL="SELECT COUNT(*) AS count FROM constru_recurso WHERE id_obra='$id_obra' and naturaleza='Extra'  and borrado=0;";
$result = $mysqli->query($SQL);
$row = $result->fetch_array();
$count = $row['count'];
...
WHERE a.id_bit_solicitud=0 AND a.naturaleza='Extra' And a.id_obra='$id_obra' AND a.borrado=0 and a.sestemp='$sestmp' ORDER BY a.$sidx $sord LIMIT $start,$limit";
$result = $mysqli->query($SQL);
...
while($row = $result->fetch_array()) {
    ...
    $responce->rows[$i]['cell']=array($row['codigo'],$row['id_bit_solicitud'],$row['naturaleza'],$row['descripcion'],$row['justificacion'],$row['unidtext'],$row['unidad'],$row['precio_costo'],$row['tc'],$row['id']);
    $i++;
}        
