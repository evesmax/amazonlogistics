...
CASE a.estatus 
WHEN 1 THEN concat('REQ-',a.id,' / Area: ',es.nombre,' ','<input type=\"button\" value=\"Cancelar\" style=\"cursor:pointer\" onclick=\"modalcancelReq(',a.id,');\" > ',' <input type=\"button\" value=\"Autorizar\" style=\"cursor:pointer\" onclick=\"autorizaReq(',a.id,');\" >')
...
a.fecha_entrega, c.clave, c.descripcion, c.unidtext,  b.cantidad Rcantidad, c.precio, b.cantidad*c.precio importe, a.id reqid, b.id reqsid, c.id insuid, f.partida, h.especialidad, concat(emp.nombre,' ',emp.apellidos) as Solicito,
CASE a.estatus 
...
WHERE a.id_area is not null AND a.id_obra='$id_obra' AND a.borrado=0 AND b.borrado=0  ORDER BY a.id DESC, b.id, $sidx $sord LIMIT $start,$limit";
$result = $mysqli->query($SQL);
...
while($row = $result->fetch_array()) {
    ...
    $responce->rows[$i]['cell']=array($row['Requisicion'],$row['Solicito'],$row['anom'],$row['partida'],$row['especialidad'],$row['clave'],$row['descripcion'],$row['unidtext'],$row['Rcantidad'],$row['precio'],$row['importe'],$row['estatus']);
    $i++;
}        
