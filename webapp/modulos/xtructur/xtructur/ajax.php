...
left join constru_traspasos cc on cc.id_bit_traspaso=c.id and cc.id_clave=a.id
...
if(isset($opcion) && $opcion=='saveConfig'){
	...
	$time=$_POST['time'];
	$puaut=$_POST['puaut'];
	$SQL = "UPDATE constru_config SET autorizaciones='$radio', tiempo='$time', puaut='$puaut' WHERE id_obra='$id_obra';";
	mysql_query( $SQL ) or die("Couldn t execute query.".mysql_error());
	exit();
}
...
//id_ps,id_esti,imp_sem  
	...
	foreach ($o as $key => $value) {
			...
			if(mysql_num_rows($result)>0){
				...
				$id_remesa=$zzz['id'];
				$SQL="UPDATE constru_cobros SET imp_sem='$v2' WHERE id_obra='$id_obra'  AND id_esti='$v1' AND proviene='$v3' AND id_bit_cobros='$id_bit_rem';";
				mysql_query( $SQL ) or die("Couldn t execute query.".mysql_error());
			}else{ 
				
				$SQL="INSERT INTO constru_cobros (id_obra,id_esti, imp_sem,semana,id_bit_cobros,proviene) 
				VALUES ('$id_obra',  '$v1' ,'$v2','$sema','$id_bit_rem','$v3');";
				mysql_query( $SQL ) or die("Couldn t execute query.".mysql_error());
...
if(isset($opcion) && $opcion=='save_cheque2'){
	...
	$SQL="INSERT INTO constru_cheques2 (id_obra,id_cobro,no_cheque,banco,fecha_expedicion,estatus_cheque,estatus_factura,fecha,cobro,mpago) VALUES ('$id_obra','$id_est','$noc','$ban','$fee','$estc','$estf','$fecha','$remesa',$valpago);";
	mysql_query( $SQL ) or die("Couldn t execute query.".mysql_error());
	exit();
}
...
while($info = mysql_fetch_array($result)) {
   $filas=$filas."Material: <br> Clave:".$info['clave']."<br> descripcion: ".$info['descripcion']."<br> cantidad: ".$info['cantidad']."<br> unidad: ".$info['unidtext']."<br> precio: ".$info['precio']."<br> importe: ".$info['precio']*$info['cantidad']." <br>";
}

$filas=$filas."Total ".$row['total'];
...
if(isset($opcion) && $opcion=='save_requi'){
    ...
    $SQL="INSERT INTO constru_requis (id_obra,solicito,estatus,fecha_entrega,id_agru,id_area,id_esp,id_part,xxano,fecha_captura) VALUES ('$id_obra','$iduserlog',1,'$fecha_entrega','$ag','$ar','$es','$pa','$xxano','$fecha');";
    mysql_query( $SQL ) or die("Couldn t execute query.".mysql_error());
    $last_id = mysql_insert_id();
		...
		if($r==4){
			...
			if(mysql_num_rows($result)>0){
				while($row = mysql_fetch_assoc($result)) {
					...
					$SQL2 = "SELECT id FROM constru_insumos WHERE calve='".addslashes($row['clave'])."' AND id_obra='".$row['id_obra_ent']."' AND borrado=0;";
					$result2 = mysql_query( $SQL2 ) or die("Couldn t execute query.".mysql_error());
					...
					if(mysql_num_rows($result2)>0){
						...
						$INS = "INSERT INTO constru_insumos (SELECT * FROM constru_insumos WHERE id ='".$row['id']."' );";
