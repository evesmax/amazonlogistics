...
$result = $mysqli->query('SELECT * FROM forma_pago;');
...
order by a.id)
UNION ALL
(SELECT b.id_alta, a.id as iidd, a.id_prov, a.fecha, concat('INDI-',b.id_alta,' - ',b.razon_social_sp) Proveedor, concat('ESTIND-',a.id) estimacion, a.factura, a.total, 
...
concat('<input value=\"',0,'\" id=\"',a.id,'\" class=\"quis__\" est=\"ESTCAJA\" name=\"',a.id,'\">') cant, 8 as TMP_ORDER
...
while($row = $result->fetch_array()) {
    ...
    inner join constru_bit_remesa c on c.id=b.id_bit_remesa
     where a.id_obra='$id_obra' AND a.id_prov='".$row['id_prov']."' AND c.estatus=2 AND a.id_esti='".$row['iidd']."';";
