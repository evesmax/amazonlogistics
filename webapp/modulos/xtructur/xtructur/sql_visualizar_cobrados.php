...
$i=0;
$SQL2="SELECT a.id_esti, a.imp_sem, a.semana, a.proviene, a.id as idcobros,concat('Cob-',b.id,' Fecha: ',b.fecha) as cobro FROM constru_cobros a 
...
$row = $result->fetch_array();
       ...
       // $cad.=$row['estimacion'].' / '.$row['Proveedor'].' /  Importe: '.$row['total'].' / Pagado: '.$sumap.' / Cheque: '.$semsem;
        ...
        $cad.=$row['estimacion'].' / '.$row['Cliente'].' /  Importe: '.$row['imp_estimacion'].' / Cobrado: '.$semsem.' / Factura: '.$row['factura'];

             $SQL4 ="SELECT a.*,b.nombre as pago from constru_cheques2 a
             JOIN forma_pago b on b.idFormaPago=a.mpago 
         where a.id_obra='$id_obra' AND a.cobro='".$row2['idcobros']."';";
