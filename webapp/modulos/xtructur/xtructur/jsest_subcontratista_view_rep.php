<?php  
  ...
  $SQL = "SELECT estatus,fgp,rep FROM constru_estimaciones_bit_subcontratista where id='$id_des' and borrado=0 AND id_obra='$id_obra' LIMIT 1;";
  $result = $mysqli->query($SQL);
  if($result->num_rows>0) {
    $row = $result->fetch_array();
  }

  $SQL = "SELECT a.*, b.nombre agru, c.nombre area, dd.especialidad esp, ee.partida, f.cargo, concat('RT-',g.id,' ',h.nombre,' ',h.paterno,' ',h.materno) as tenico, a.id as idest FROM constru_estimaciones_bit_subcontratista a 
...
//total amortizado
  /*
$SQL = "SELECT if(sum(amortizado_estimacion) is null,0,sum(amortizado_estimacion)) as totamor from constru_estimaciones_bit_subcontratista where id_obra='$id_obra' and estatus=1 and  id<='$id_des' ;";
  $result = $mysqli->query($SQL);
  if($result->num_rows>0) {
...
$SQL = "SELECT if(sum(amortizado_estimacion) is null,0,sum(amortizado_estimacion)) as amorant from constru_estimaciones_bit_subcontratista where id_obra='$id_obra' and estatus=1 and id<'$id_des';";
  $result = $mysqli->query($SQL);
  if($result->num_rows>0) {
...
$SQL = "SELECT if(sum(amortizado_estimacion) is null,0,sum(amortizado_estimacion)) as totamor from constru_estimaciones_bit_subcontratista where id_obra='$id_obra' and estatus=1 and  id<='$id_des';";
  ...
  $SQL = "SELECT imp_cont,ade1,ade2,ade3,anticipo FROM constru_info_sp  where id_alta='$idsub' LIMIT 1;";
  $result = $mysqli->query($SQL);
  if($result->num_rows>0) {
  ...
  //por amortizar
  $poramor = $row2['anticipo']-$row5['totamor'];
...
//$row[clave],$row[descripcion],$row[unidtext],$row[vol_tope],$row[precio],$row[total]
  ...
  $.ajax({
    ...
    type: 'POST',
    dataType: 'JSON',
    data: {opcion:'est_control_subcontratista'},
    success: function(r){
      sestmp=$('#sestmp').val();
      jQuery("#grid").jqGrid('navGrid','#pcrud',{});
      jQuery("#jq_requisiciones").jqGrid({
        url:'sql_jsest_subcontratistas_rep.php?id_des=<?php echo $id_des; ?>&q=3&sestmp='+sestmp,
        datatype: "json",
...
<input id="totcant" disabled="disabled" type="text" role="textbox" class="FormElement ui-widget-content ui-corner-all" style="width: 278px;" value="'+r.datos.totcant+'">\
        ...
        editurl: "sql_jsest_subcontratistas.php?id_des=<?php echo $id_des; ?>&sestmp="+sestmp,
...
<div class="row">
  ...
  <div class="row">
    ...
    <div class="col-sm-3">
      ...
      <input class="form-control" id="ade2" type="text" value="<?php echo $row2['ade2']; ?>" disabled="disabled">
    </div>
    <div class="col-sm-3">
  ...
  <div class="row">
    ...
    <div class="col-sm-3">
      ...
      <input class="form-control" id="iaa" type="text" value="<?php echo $row6['amorant']; ?>" disabled="disabled">
    </div>
    <div class="col-sm-3">
  ...
  <div class="row">
    <div class="col-sm-3">
      ...
      <input class="form-control" id="tota" type="text" value="<?php echo $row5['totamor']; ?>" disabled="disabled">
    </div>
    <div class="col-sm-3">
      <label>Por amortizar:</label>
      <input class="form-control" id="poramo" type="text" value="<?php echo $poramor; ?>" disabled="disabled">
    </div>
    <div class="col-sm-3">
      <label>&nbsp;</label>
      <input class="btn btn-primary btnMenu" id="btnpdfsub" type="button" value="PDF" onclick="pdf_est_sub('sub')" style="cursor:pointer;">
    </div>
  </div>
  ...
  <div class="row">
    ...
    <div class="col-sm-3">
      ...
      <input class="form-control" id="iaebajo" type="text" value="<?php echo $row2['amortizado_estimacion']; ?>" disabled="disabled">
    </div>
    <div class="col-sm-3">
    ...
    <div class="col-sm-3">
      <label>Fondo de garantia <?php echo $row['fgp']; ?>%:</label>
  ...
  <div class="row">
    ...
    <div class="col-sm-3">
      <label>Factura:</label>
      <input class="form-control" type="text" id="modiFac" value="<?php echo $row2['factura']; ?>">
    </div>
  </div>
  <div class="row">
    <div class="col-sm-3">
      <input class="btn btn-primary btnMenu" type="button" value="Guardar" onclick="modiFac('<?php echo $row2['idest']; ?>','sub');">
    </div>
  ...
  <div class="row">
    <?php if ($row['estatus']==0){ ?>
      <div class="col-sm-3 col-sm-offset-6">
        <input class="best btn btn-primary btnMenu" style="cursor:pointer;" type="button" value="Autorizar" onclick="autorizarest('sub',1);"> 
      </div>
      <div class="col-sm-3">
        <input class="best btn btn-danger btnMenu" style="cursor:pointer;" type="button" value="Cancelar" onclick="autorizarest('sub',2);">
      </div>
    <?php } ?>
