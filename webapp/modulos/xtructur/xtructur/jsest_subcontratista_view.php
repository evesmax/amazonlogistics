...
include('conexiondb.php');
  ...
  $SQL = "SELECT a.id, a.nombre FROM constru_agrupador a where a.id_obra='$id_obra';";
  $result = $mysqli->query($SQL);
  if($result->num_rows>0) {
    while($row = $result->fetch_array() ) {
      $agrupadores[]=$row;
  ...
  $SQL = "SELECT if(sum(amortizado_estimacion) is null,0,sum(amortizado_estimacion)) as amoa from constru_estimaciones_bit_subcontratista where id_obra='$id_obra' and estatus=1 and id_subcontratista='$id_des';";
  $result = $mysqli->query($SQL);
  if($result->num_rows>0) {
    $row = $result->fetch_array();
    $amoa=$row['amoa'];
...
$cad_agrupador='';
  ...
  $SQL = "SELECT imp_cont,ade1,ade2,ade3,anticipo,por_fondo_garantia,por_retencion FROM constru_info_sp  where id_alta='$id_des' LIMIT 1;";
  $result = $mysqli->query($SQL);
  if($result->num_rows>0) {
    $row = $result->fetch_array();
  }else{
...
//$row[clave],$row[descripcion],$row[unidtext],$row[vol_tope],$row[precio],$row[total]
            ...
            dataEvents:[{ 
                type: 'change', fn: function(e) {
                  ...
                  $.ajax({
                    ...
                    type: 'POST',
                    dataType: 'JSON',
                    data: {opcion:'desc_subcontratista_est',id_codigo:id_insumo,id_des:'<?php echo $id_des; ?>',ar:ar},
                    success: function(r){
                      if(r.success==1){
  ...
  <td class="CaptionTD"><font color="#cecece">Vol. Tope area</font></td>\
  <td class="DataTD">&nbsp;\
...
<input id="totcant" disabled="disabled" type="text" role="textbox" class="FormElement ui-widget-content ui-corner-all" style="width: 278px;" value="'+r.datos.totcant+'">\
        ...
        editurl: "sql_jsest_subcontratistas.php?ar=<?php echo $ar; ?>&sema=<?php echo $sema; ?>&id_des=<?php echo $id_des; ?>&sestmp="+sestmp,
...
<div class="row">
  <div class="col-sm-3"  style="margin-top: -6px;">
    <label>Retencion <input id="rep" disabled="disabled" value="<?php echo $row['por_retencion']; ?>"  style="width:40px;">%:</label>
