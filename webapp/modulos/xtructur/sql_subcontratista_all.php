...
if($filtro_proveedor!=0){
    $cadenafiltro.=" AND a.id_subcontratista = '".$filtro_proveedor."' ";
}


session_start();
if (in_array($_SESSION['accelog_nombre_instancia'], $cookie_xtructur['instancias_acontia'])){
 	$adjuntaXML=" <a style=\"margin-top:4px\" onclick=\"adjuntarxml(',a.id,',',0,');\"  class=\"btn btn-default btn-xs\"><span class=\"glyphicon glyphicon-upload\"></span> Adjuntar factura\'s</a>";
}else{
	$adjuntaXML="";
}
...
if($correo==1){
$SQL = "SELECT
concat(b.razon_social_sp) as Subcontratista, b.id,
CASE a.estatus 
WHEN 0 THEN concat('Estimacion-',a.id,' Fecha: ',substr(fecha,1,10),' ','<input type=\"button\" value=\"Cancelar\" style=\"cursor:pointer\"  data-toggle=\"modal\" data-target=\"#mailmodal\"  data-eid=',a.id, ' > ',' <input type=\"button\" value=\"Autorizar\" style=\"cursor:pointer\" onclick=\"autorizarestAll(\'sub\',',a.id,',',1,',',0,');\" >')
WHEN 2 THEN concat('Estimacion-',a.id,' Fecha: ',substr(fecha,1,10),' ','<font color=\"#ff0000\">Estimacion Cancelada</font>')
WHEN 1 THEN concat('Estimacion-',a.id,' Fecha: ',substr(fecha,1,10),' ','<font color=\"#070\">Estimacion Autorizada</font> ".$adjuntaXML." ')
END as Estimacion,
...
WHERE a.id_obra='$id_obra'".$cadenafiltro."  AND c.borrado=0  AND a.borrado=0 AND alt.id_tipo_alta=4 AND (alt.estatus='Alta' OR alt.estatus='Incapacitado') ORDER BY  $sidx $sord LIMIT $start,$limit";
$result = $mysqli->query($SQL);
...
concat(b.razon_social_sp) as Subcontratista, b.id,
CASE a.estatus 
WHEN 0 THEN concat('Estimacion-',a.id,' Fecha: ',substr(fecha,1,10),' ','<input type=\"button\" value=\"Cancelar\" style=\"cursor:pointer\" onclick=\"autorizarestAll(\'sub\',',a.id,',',2,',','0',');\" > ',' <input type=\"button\" value=\"Autorizar\" style=\"cursor:pointer\" onclick=\"autorizarestAll(\'sub\',',a.id,',',1,',',0,');\" >')
WHEN 2 THEN concat('Estimacion-',a.id,' Fecha: ',substr(fecha,1,10),' ','<font color=\"#ff0000\">Estimacion Cancelada</font>')
WHEN 1 THEN concat('Estimacion-',a.id,' Fecha: ',substr(fecha,1,10),' ','<font color=\"#070\">Estimacion Autorizada</font> ".$adjuntaXML." ')
END as Estimacion,
d.codigo, d.descripcion, d.unidtext, d.pu_subcontrato, e.vol_tope*d.pu_subcontrato as total, c.vol_anterior,
...
while($row = $result->fetch_array()) {
	$acumulado=$row['vol_anterior']+$row['vol_estimacion'];
    $poreje=$row['vol_tope']-$acumulado;
    $impfin=$row['pu_subcontrato']*$row['vol_estimacion'];
    $responce->rows[$i]['id']=$i;
    $responce->rows[$i]['cell']=array($row['Subcontratista'],$row['Estimacion'],$row['codigo'],$row['descripcion'],$row['unidtext'],$row['vol_tope'],$row['pu_subcontrato'],$row['total'],$row['vol_anterior'],$row['vol_estimacion'],$acumulado,$poreje,$impfin);
    $i++;
}        
