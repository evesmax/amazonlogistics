...
if(!isset($_GET['filtro_material'])){ $filtro_material=0; }else{ $filtro_material=$_GET['filtro_material']; }
//Fin filtros nuevos
...
if($filtro_material!='0'){
    $cadenafiltro.=" AND wc.descripcion = '".$filtro_material."' ";
}
...
if (in_array($_SESSION['accelog_nombre_instancia'], $cookie_xtructur['instancias_acontia'])){
 	$adjuntaXML=" <a style=\"margin-top:4px\" onclick=\"adjuntarxml(',a.id,',',a.id_oc,');\"  class=\"btn btn-default btn-xs\"><span class=\"glyphicon glyphicon-upload\"></span> Adjuntar factura\'s</a>";
}else{
	$adjuntaXML="";
}
...
WHEN 1 THEN concat('Estimacion-',a.id,' OC-',a.id_oc, ' Fecha: ',substr(fecha,1,10),' ','<font color=\"#070\">Estimacion Autorizada</font> ".$adjuntaXML." ')
END as Estimacion,
...
concat(b.razon_social_sp) as Proveedor,
CASE a.estatus 
WHEN 0 THEN concat('Estimacion-',a.id,' OC-',a.id_oc, ' Fecha: ',substr(fecha,1,10),' ','<input type=\"button\" value=\"Cancelar\" style=\"cursor:pointer\" onclick=\"autorizarestAll(\'prov\',',a.id,',',2,',','0',');\" > ',' <input type=\"button\" value=\"Autorizar\" style=\"cursor:pointer\" onclick=\"autorizarestAll(\'prov\',',a.id,',',1,',',0,');\" >')
WHEN 2 THEN concat('Estimacion-',a.id,' OC-',a.id_oc, ' Fecha: ',substr(fecha,1,10),' ','<font color=\"#ff0000\">Estimacion Cancelada</font>')
WHEN 1 THEN concat('Estimacion-',a.id,' OC-',a.id_oc, ' Fecha: ',substr(fecha,1,10),' ','<font color=\"#070\">Estimacion Autorizada</font> ".$adjuntaXML." ')
END as Estimacion,
wc.clave, wc.descripcion, wc.unidtext, wb.cantidad Rcantidad, 
