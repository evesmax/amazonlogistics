...
$SQL = "SELECT
concat('Suncontratista: ',b.razon_social_sp,' EST-',a.id, ' SEM: ',a.xxano) as Subcontratista, a.id
...
WHERE a.id_obra='$idses_obra'  AND c.borrado=0  AND a.borrado=0 AND alt.id_tipo_alta=4 AND (alt.estatus='Alta' OR alt.estatus='Incapacitado') group by a.id  ORDER BY  Subcontratista asc, a.id asc";
      ...
      $SQL = "SELECT a.*, concat('Sub-',a.id,' -  ',b.razon_social_sp) nombre FROM constru_altas a inner join constru_info_sp b on b.id_alta=a.id where a.id_obra='$idses_obra' and a.borrado=0 AND a.id_tipo_alta=4 AND (a.estatus='Alta' OR a.estatus='Incapacitado');";
  $result = $mysqli->query($SQL);
  if($result->num_rows>0) {
    while($row = $result->fetch_array() ) {
      $proveedores[]=$row;
...
<script> 
        $(document).ready(function () {
            $("#jq_arbol").jqGrid({
               ...
               // d.codigo, d.descripcion, d.unidtext, d.pu_destajo, e.vol_tope*d.pu_destajo as total, c.vol_anterior, c.vol_est
                    ...
                    { label: 'Vol tope', name: 'VolTope',  width: 35, sortable:false,sorttype:"float", formatter:"number", formatoptions: { decimalPlaces: 4 } },
                    { label: 'PU destajo', name: 'precio',  width: 35, sortable:false,sorttype:"float", formatter:"number", formatoptions: { decimalPlaces: 4 } },
                    { label: 'Total', name: 'Total',  width: 35, sortable:false,sorttype:"float", formatter:"number", formatoptions: { decimalPlaces: 4 } },
                    { label: 'Vol. Anterior', name: 'Vol. Anterior',  width: 35, sortable:false,sorttype:"float", formatter:"number", formatoptions: { decimalPlaces: 4 } },
                    { label: 'Vol. Estimacion', name: 'Vol. Estimacion',  width: 35, sortable:false,sorttype:"float", formatter:"number", formatoptions: { decimalPlaces: 4 } },
                    { label: 'Acumulado', name: 'Acumulado',  width: 35, sortable:false,sorttype:"float", formatter:"number", formatoptions: { decimalPlaces: 4 } },
                    { label: 'Por ejecutar', name: 'Por ejecutar',  width: 35, sortable:false,sorttype:"float", formatter:"number", formatoptions: { decimalPlaces: 4 } },
                    { 
        ...
        grouping: true,
                groupingView: {
                    groupField: ["Subcontratista", "Estimacion"],
                    groupColumnShow: [false, false],
                    groupText: [
          //  "Agrupador: <b>{0}</b> - ${importe}",
                    "Subcontratista: <b>{0}</b>",
                    "Estimacion: <b>{0}</b>"
          ],
                    groupOrder: ["asc", "asc"],
            ...
            jQuery("#jq_arbol").jqGrid('navGrid',"#jqp_arbol",
                ...
                buttonicon: "ui-icon-export",
                  onClickButton: function() {
                        $("#jq_arbol").jqGrid('exportarExcelCliente',{nombre:"subcon",formato:"excel"});
...
<div class="panel panel-default" >
      ...
      <div class="panel-title">Autorizacion Estimacion Subcontratistas</div>
...
<input type="button" id='enviarb' value="Enviar" style="cursor:pointer" onclick="autorizarestAll('sub',-1,2,1);">
...
<div class="panel panel-default" >
      ...
      <select class="form-control" id="estimacion_num">
        ...
        if($vercompras!=0){
          foreach ($vercompras as $k => $v) { ?>
            <option value="<?php echo $v['id']; ?>">OC-<?php echo $v['Subcontratista'];?></option>
          <?php } ?>
        <?php }else{ ?>
          <option value="0">No hay Estimaciones</option>
        <?php } ?>
    ...
    <div class="col-sm-2 col-xs-4">
      ...
      <button onclick="pdf_est_sub('sub');" id="btnpdfcomp" class="btn btn-primary btn-xm"><span class="glyphicon glyphicon-download"></span> PDF</button>
    </div>
</div>
...
Todos
  ...
  &nbsp;
    ...
    <button onclick="graficar_ret('est_sub','<?php echo $idses_obra; ?>');" class="btn btn-primary btn-xm"> Visualizar grafica</button>
