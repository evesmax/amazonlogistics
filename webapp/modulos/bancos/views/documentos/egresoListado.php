...
<!-- <label for="buscar">Buscar:</label> <input type="text" id="buscar" value=""/> -->  <br>
  	<table  cellspacing="2" cellpadding="2" width="100%" class="table table-striped table-bordered" id="table">
  		<thead>
  			<tr style="">
	  			...
	  			<th style="width:70px !important;"  class="nmcatalogbusquedatit">Moneda</th>
  				<th style="width:30px !important;"  class="nmcatalogbusquedatit">Proceso</th>
  	...
  	<?php if($egreso){
  				while($in = $egreso->fetch_assoc()){
  					...
  					<tr class="out" onmouseout="this.className='out'" onmouseover="this.className='over'" >
  						...
  						<td style="width:70px !important;"  class=" nmcatalogbusquedacont_1 " align="right"><a href="javascript: <?php echo $_REQUEST['opc']."(".$in['id'].")";?>" ><b style="color:red"><?php echo number_format($in['importe'],2,'.',',');?></b></a></td>
  				  		<td style="width:70px !important;"  class=" nmcatalogbusquedacont_1 "><a href="javascript: <?php echo $_REQUEST['opc']."(".$in['id'].")";?>" ><?php echo $in['description'];?></a></td>
  				  		<td style="width:30px !important;"  class=" nmcatalogbusquedacont_1 "><a href="javascript: <?php echo $_REQUEST['opc']."(".$in['id'].")";?>" ><?php echo $proceso;?></a></td>
  		...
  		<?php 	}
  			...
  			<tr>
  				<td colspan="8" align="center">No tiene Egresos</td>
  			</tr>
  	<?php } ?>
  		</tbody>
  	</table>
  	...
  	<script>
  		function editar(id){
  			window.location="index.php?c=Cheques&f=verEgresos&editar="+id;
  		}
