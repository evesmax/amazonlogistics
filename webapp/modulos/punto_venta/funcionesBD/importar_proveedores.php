	...
	if(!in_array($ext,$allowed) ) 
	{
		...
		if(isset($_FILES["myfile"]))
		{
			...
			else
			{
				...
				$nombreCol= array("ESTATUS","CODIGO","RAZON SOCIAL","RFC","NOMBRE COMERCIAL","PAGINA WEB","DIRECCION","EXTERIOR","COLONIA","C.P.","PAÍS","ESTADO","MUNICIPIO","CIUDAD","NOMBRE","CORREO","TELÉFONO","DÍAS DE CRÉDITO","LÍMITE DE CRÉDITO","CLASIFICADOR","MONEDA");
				...
				{ 
					...
					if(!preg_match($checkRFC, $rowData[0][3])){
						if($rowData[0][3]!=""){
							$caracterInvalido=true;
							$errores.= "<br>(RFC formato invalido)<br>[Fila: ".$row."] [RFC: ".$rowData[0][3]."]";
						}
					...
					for($i=0; $i<22; $i++) {
						...
						if($i==1 || $i==2 || $i==3 || $i==12) {
							...
							if($rowData[0][$i] == "" || $rowData[0][$i] == " ") {
								...
								$camposF = '';
								if($i==1){$camposF .= 'Codigo ';}
								if($i==2){$camposF .= 'Razon Social ';}
								if($i==3){$camposF .= 'RFC ';}
								if($i==12){$camposF .= 'Pais ';}
				...
				else if($hayCamposObligatoriosVacios == true)
				{
					...
					<div class="row">
						<div class="col-md-12" style="text-align: center;">
							<b>Fanta un campo obligatorio: <?php echo $camposF; ?></b>
							<br>
							<b>Revise la ayuda incluida en la plantilla</b>
							<br><br>
							<!--<input type='button' value='Volver a la interface de carga' id='btn_volver' onclick='goBack();' class='btn btn-primary btnMenu'>-->
							<input type='button' value='Volver a la interface de carga' id='btn_volver' onclick='goBack();' class='btn btn-primary'>
						</div>
				...
				else
				{
					...
					<div class="row">
						<div class="col-md-12">
							<?php if ($opt == 1) { $titulo = "Seleccione los proveedores a importar."; } else { $titulo = "Seleccione los proveedores a modificar."; } ?>
							...
							<div class="table-responsive">
								<table cellpadding='0' cellspacing='0' width='100%' style='min-width: 100%; font-size: 11px; overflow: auto; max-height: 350px; border: 1px solid #98ac31; padding: 10px;'>
									<tr>
										...
										<th class='nmcatalogbusquedatit' width=5% style='border-bottom: 1px solid #98ac31; '>Estatus</th>
										...
										<th class='nmcatalogbusquedatit' width=10% style='border-bottom: 1px solid #98ac31;'>Pagina Web</th>
										...
										<th class='nmcatalogbusquedatit' width=10% style='border-bottom: 1px solid #98ac31;'>Clave Clasificador</th>
					...
					-->
						...
						<br><br>
							<input type='button' value='Volver a la interface de carga' id='btn_volver' onclick='goBack();' class='btn btn-primary'>
							<input type='button' value='Importar proveedores' id='btn_importar' onclick='registrarProveedores("<?php echo $output_dir. $_FILES["myfile"]["name"]; ?>","<?php echo $opt; ?>");' class='btn btn-success'>
							<br><br>
...
function registraProveedores($connection)
{
	...
	for ($row = 10; $row <= $highestRow; $row++)
	{
		...
		if(in_array($row, $check)) 
		{
			...
			$colonia = $rowData[0][9];
			$cp = $rowData[0][10];
			$idestado = $rowData[0][12];
			$idmunicipio = $rowData[0][13];
			$nombre = $rowData[0][15];
			$email = $rowData[0][16];
			$telefono = $rowData[0][17];
			$limite_credito = $rowData[0][19];
			//$clasificador = $rowData[0][20];
			$clasificador = clasificando(trim($rowData[0][20]), $connection);
			$moneda = $rowData[0][21];
			...
			if($opt == 1){//carga
				...
				$sql = 'INSERT INTO mrp_proveedor (status, codigo, razon_social, rfc, nombre_comercial, web, calle, no_ext, no_int, colonia, cp, idpais, idestado, idmunicipio, ciudad, nombre, email, telefono, diascredito, limite_credito, clasificacion, moneda) 
				values("'.$status.'","'.$rowData[0][1].'","'.$rowData[0][2].'","'.$rowData[0][3].'","'.$rowData[0][4].'","'.$rowData[0][5].'","'.$rowData[0][6].'","'.$rowData[0][7].'","'.$rowData[0][8].'","'.$rowData[0][9].'","'.$rowData[0][10].'",1,"'.$rowData[0][12].'","'.$rowData[0][13].'","'.$rowData[0][14].'","'.$rowData[0][15].'","'.$rowData[0][16].'","'.$rowData[0][17].'","'.$rowData[0][18].'","'.$rowData[0][19].'","'.$clasificador.'","'.$rowData[0][21].'");';
				$connection->query($sql);
				/*
					$result = $connection->query('INSERT INTO mrp_proveedor (codigo,razon_social, rfc, domicilio, telefono, email, web,diascredito, idpais, idestado, idmunicipio,curp,nombre_comercial,moneda,clasificacion,limite_credito,calle,no_ext,no_int,cp) 
						values ("'.$rowData[0][0].'", "'.$rowData[0][3].'", "'.$rowData[0][1].'", "'.$rowData[0][5].' #'.$rowData[0][6].' '.$rowData[0][7].'", "'.$rowData[0][11].'", "'.$rowData[0][12].'", "'.$rowData[0][13].'", "'.$rowData[0][14].'","'.$rowData[0][9].'","'.$rowData[0][10].'","'.$rowData[0][2].'","'.$rowData[0][4].'","'.$rowData[0][17].'","'.$rowData[0][16].'","'.$rowData[0][15].'","'.$rowData[0][5].'","'.$rowData[0][6].'","'.$rowData[0][7].'","'.$rowData[0][8].'");');  
				*/
