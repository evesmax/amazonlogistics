...
switch (@$_POST["funcion"])
{
	...
	case 'guardarFacturacion': echo guardarFacturacion($_POST["UUID"],$_POST["noCertificadoSAT"],$_POST["selloCFD"],$_POST["selloSAT"],$_POST["FechaTimbrado"],$_POST["idComprobante"],$_POST["idFact"],$_POST["idVenta"],$_POST["noCertificado"],$_POST["tipoComp"],$_POST["monto"],$_POST["cliente"],$_POST["trackId"],$_POST["idRefact"],$_POST["azurian"],$_POST["total"]); break;
	
	case 'guardarNotadeCredito': echo guardarNotadeCredito($_POST["UUID"],$_POST["noCertificadoSAT"],$_POST["selloCFD"],$_POST["selloSAT"],$_POST["FechaTimbrado"],$_POST["idComprobante"],$_POST["idFact"],$_POST["idVenta"],$_POST["noCertificado"],$_POST["tipoComp"],$_POST["monto"],$_POST["cliente"],$_POST["trackId"],$_POST["idRefact"],$_POST["azurian"],$_POST["total"],$_POST["idfac"]); break;
	...
	case 'envioFactura2': echo envioFactura2($_POST["uid"],$_POST["correo"],$_POST["azurian"]); break; 
	case 'envioFactura3': echo envioFactura3($_POST["uid"],$_POST["correo"],$_POST["azurian"]); break; 
	default:
...
function cancelaFacturacion($id)
{
	...
	mysql_query("UPDATE pvt_respuestaFacturacion set borrado=3  where id='$id'");
...
function existenciasSucursal($idProducto)
{
	...
	<th class="nmcatalogbusquedatit" align="center">Cantidad</th> 
	</tr>';
	$cont=0;
	while($r=mysql_fetch_object($q))
	{
...
function cancelarventa($id)
{
	...
	$queryPagos = "Select idCxcpagos from cxc_pagos cp, cxc c where c.idVenta = ".$id." and cp.idCxc=c.idCxc";
	$resultQueryPagos = mysql_query($queryPagos);
	if(mysql_num_rows($resultQueryPagos) > 0)
	{
		return "<label style='text-align: center; font-size: 15px; float: left; width: 100% ! important; color: red;'>No se puede eliminar la venta, por que tiene abonos en cuentas por cobrar.</label>";
	}
	...
	while($rowp=mysql_fetch_object($q))
	{
		...
		$queryordenproduccion=mysql_query("SELECT cantidad from mrp_detalle_orden_produccion where idProducto=".$rowp->idProducto);


		if(mysql_num_rows($querycompuesto)>0 && mysql_num_rows($queryordenproduccion)<0)
		{	
			$querytipo=mysql_query('SELECT tipo_producto from mrp_producto where idProducto='.$rowp->idProducto);
			$rowtipo=mysql_fetch_array($querytipo);

				if($rowtipo['tipo_producto']==2){
...
////////////////////////////////////////////////////////////////////////////////////////////////	    	
	    	mysql_query("update venta set estatus=0, fecha_modifica='".$fechaactual."' where idVenta=".$id);
	    	mysql_query("update pvt_pendienteFactura set facturado=2 where id_sale=".$id);

	    	mysql_query("Delete from cxc where idVenta=".$id);
	    	return ventas();
	    }
//**************************************************************************************************/
	    function detalleventa($id)
	    {
	    	...
	    	/*agregado*/
	    	$caja='<div class="table-responsive"><table id="table-caja" border="0" width="90%">
	    	<tr>
	    	...
	    	$q=mysql_query("select * from venta where idVenta=".$id);
	    	while($rowp=mysql_fetch_object($q))
	    	{
	    		$datosVenta=$rowp;
	...
	//descuento $
	    	...
	    	/*IMPUESTOS*/
	    	$qi=mysql_query("select pi.idimpuesto,i.nombre as impuesto, pi.valor from producto_impuesto pi inner join impuesto i on i.id=pi.idImpuesto where idProducto=".$id.' order by pi.idimpuesto DESC');
	    	if(mysql_num_rows($qi)>0)
	    	{
	    		while($ri = mysql_fetch_object($qi))
	    		{
	    			...
	    			{	
	    				$impuestos_venta[$ri->impuesto]=((($producto->precioventa - $descuento_p)*$ri->valor)/100+$impuestos_venta[$ri->impuesto])*$producto->cantidad;	
	    			}
	    			else
	    			{
	    				$impuestos_venta[$ri->impuesto]=((($producto->precioventa - $descuento_p)*$ri->valor)/100)*$producto->cantidad;	
	    			}*/
	    			...
	    			if ($ri->impuesto == 'IEPS') {
	    				$producto_impuesto = $ieps = number_format((($producto->preciounitario ) * $producto->cantidad - $producto->montodescuento) *  $ri->valor / 100, 2);
	    				//echo $producto_impuesto;
	    				$impuestos_venta[$ri->impuesto] += str_replace(",", "", $producto_impuesto);
	    			} else {
	    				if ($ieps != 0) {
	    					$producto_impuesto = number_format(((($producto->preciounitario + $ieps)) * $producto->cantidad - $producto->montodescuento) *  $ri->valor / 100, 2);
	    					//echo $producto_impuesto;
	    					$impuestos_venta[$ri->impuesto] += str_replace(",", "", $producto_impuesto);
	    				} else {
	    					$producto_impuesto = number_format((($producto->preciounitario) * $producto->cantidad - $producto->montodescuento) *  $ri->valor / 100, 2);
	    					//echo "(".$producto->montodescuento.")";
	    	...
	    	$subtotal=$producto->subtotal-$producto->montodescuento;
	    	$total+=$subtotal;
	    	...
	    	<td align="center">$'.number_format($producto->subtotal-$producto->montodescuento,2).'</td>
	    	</tr>';

	    	$supertotal+=($producto->subtotal)+($producto->impuestosproductoventa)-$producto->montodescuento;
	    	$i++;	
	}//end foreach
...
function cargaDevolucion($id){
	    	$impuestos_venta=array();
	    	/*agregado*/
	    	$caja='<div id="caja"><table id="table-caja-devolucion" border="0" width="90%">
	    	<tr>
	...
	//descuento $
	    	...
	    	if($i%2==0){ $caja.='<tr class="par" >'; } else { $caja.='<tr  class="impar">';}	
	    	$caja.='
	    	<td align="center"><input class="cantidades" type="text" value="'.$producto->cantidad.'" id="cant_'.$producto->idProducto.'" onkeyup="valida('.$producto->idProducto.')"><input type="text" value="'.$producto->cantidad.'" id="hideCa_'.$producto->idProducto.'">'.$option.'</td>
	    	<td align="center"><span '.$propiedad.'>'.$producto->nombre.'</span><br>'.$materiales.'</td>
	    	<td align="center">$'.number_format($producto->preciounitario,2).'</td>
	    	<td align="center">'.($descuento).'</td>
	    	<td align="center">$'.number_format($producto->impuestosproductoventa,2).'</td>
...
//echo $filtro;
		...
		<div id="caja-dialog-confirmacion" class="modal fade" role="dialog" style="z-index: 1200 !important;">
	        <div class="modal-dialog">
	            <div class="modal-content">
	                <div class="modal-header">
	                    <h4 class="modal-title" id="caja-dialog-confirmacion-titulo"></h4>
	                </div>
	                <div class="modal-body" id="caja-dialog-confirmacion-cuerpo">
	                    
	                </div>
	                <div class="modal-footer">
	                    <div class="row btns-confirmacion" id="caja-dialog-footer-confirmacion-cancelar-venta">
	                        <div class="col-md-6">
	                            <button class="btn btn-danger btnMenu" onclick="javascript:dvCancelarVentaSi();">Si</button>
	                        </div>
	                        <div class="col-md-6">
	                            <button class="btn btn-success btnMenu" onclick="javascript:$('#caja-dialog-confirmacion').modal('hide');">No</button>
	                        </div>
	                    </div>
	                    <div class="row btns-confirmacion" id="caja-dialog-footer-confirmacion-devolver-venta">
	                        <div class="col-md-6">
	                            <!--<button class="btn btn-danger btnMenu" onclick="javascript:dvDevolverSi();">Devolver</button> -->
	                        </div>
	                        <div class="col-md-6">
	                            <button class="btn btn-success btnMenu" onclick="javascript:$('#caja-dialog-confirmacion').modal('hide');">Cancelar</button>
	                        </div>
	                    </div>
	                </div>
	            </div>
		...
		<div id="caja-dialog" class="modal fade" role="dialog">
	        <div class="modal-dialog">
	            <div class="modal-content">
	                <div class="modal-header">
	                    <h4 class="modal-title" id="caja-dialog-titulo"></h4>
	                </div>
	                <div class="modal-body" id="caja-dialog-cuerpo">
	                    
	                </div>
	                <div class="modal-footer">
	                    <div class="row btns" id="caja-dialog-footer-detalle-venta">
	                    	<div class="col-md-3">
	                        </div>
	                        <div class="col-md-3">
	                            <button class="btn btn-danger btnMenu" onclick="javascript:dvCancelarVenta();">Cancelar venta</button>
	                        </div>
	                       <!-- <div class="col-md-3">
	                            <button class="btn btn-warning btnMenu" onclick="javascript:dvDevolver();">Devolver</button> 
	                        </div>-->
	                        <div class="col-md-3">
	                            <button class="btn btn-success btnMenu" onclick="javascript:dvImprimir();">Imprimir</button>
	                        </div>
	                        <div class="col-md-3">
	                            <button class="btn btn-primary btnMenu" onclick="javascript:$('#caja-dialog').modal('hide');">Salir</button>
	                        </div>
	                    </div>
	                </div>
	            </div>
		...
		<table id="orden" align="center"  cellspacing="0" border="0" class="busqueda table">
			<thead>
				<tr class="busqueda_fila2">
					<th class="nmcatalogbusquedatit" align="center" sort="folio">Folio</th>
					<th class="nmcatalogbusquedatit" align="center" sort="fecha">Fecha</th>
					<th class="nmcatalogbusquedatit" align="center" sort="cliente">Cliente</th>
					<th class="nmcatalogbusquedatit" align="center" sort="vendedor">Vendedor</th>
					<?php
					if(simple()){	  
						?>
						<th class="nmcatalogbusquedatit" align="center" sort="sucu">Sucursal</th>
						<?php
					} 
					?>
					<th class="nmcatalogbusquedatit" align="center" sort="estatus">Estatus</th>
					<th class="nmcatalogbusquedatit" align="center" sort="impuestos">Impuestos</th>
					<th class="nmcatalogbusquedatit" align="center" sort="monto">Monto</th>
					<th class="nmcatalogbusquedatit" align="center" sort="envio">Facturacion</th>
					<th class="nmcatalogbusquedatit" align="center" sort="envio">UUID</th>
			...
			<tbody>
				...
				if(mysql_num_rows($q)>0){
					while($r=mysql_fetch_object($q))
					{
						$xer = mysql_query('SELECT folio from pvt_respuestaFacturacion where idSale='.$r->folio);
				    	if(mysql_num_rows($xer)>0){
				    		while($r2f=mysql_fetch_object($xer))
				    		{
				    			$foliFac = $r2f->folio;
				    		}
				    	}else{
						...
						<tr class="<?php echo $color; ?>">
							...
							if($r->envio=='1'){
								$envio ='<span style="color:#D3CE35;">Sin Respuesta</span>';
							}
							if($r->envio=='0'){
								$envio ='';
							}							
							...
							<td align="center"><?php echo $envio; ?></td>
							<td align="center"><?php echo $foliFac; ?></td>
							<td align="center">
								<?php
								$i++;
				...
				<tfoot class="nav">
					<tr style="font-size: 10pt;background:#c4c4c4;color: #333"><td></td><td></td><td></td><td></td><td></td><td></td><td></td>
	...
	function entradasalidas($inicio="",$fin="",$producto="",$movimiento="",$registros=20, $pagina=0)
	{
		...
		$q=mysql_query("select * from 
			(
				...
				'' as proveedor_cliente,
				'Ingreso Producciones' as movimiento,
				'' as origen,
				'' as destino,
				'' as comentario
				from mrp_detalle_orden_produccion opd,mrp_orden_produccion op ,mrp_producto p, mrp_sucursal s, almacen a
				where op.id=opd.idOrdPro and opd.idProducto=p.idProducto  and op.idAlmacen=a.idAlmacen
			...
			UNION
				...
				a.nombre as sucursal,
				'' as proveedor_cliente,
				mi.tipo as movimiento,
				'' as origen,
				'' as destino,
				mi.comentario as comentario
				from mrp_movimientos_inv mi, mrp_producto p, almacen a
				where mi.idProducto=p.idProducto and mi.idAlmacen=a.idAlmacen
			...
			select  distinct
				...
				p.nombre as producto,
				p.idProducto as idProducto, 
				(opd.cantidad * m.cantidad) as cantidad,
				'' as monto,
				a.nombre as sucursal,
				'' as proveedor_cliente,
				'Salida Producciones' as movimiento,
				'' as origen,
				'' as destino,
				'' as comentario
				from mrp_detalle_orden_produccion opd,mrp_orden_produccion op ,mrp_producto p, mrp_sucursal s, almacen a, mrp_producto_material m
				where op.id=opd.idOrdPro  and opd.idProducto=m.idProducto and m.idMaterial=p.idProducto and op.idAlmacen=a.idAlmacen
...
$i=0;
$total=0;
$tabla='<table  id="orden" align="center" border="0" class="table busqueda">
<thead><tr class="busqueda_fila2">
<th class="nmcatalogbusquedatit" sort="folio">Folio</th>
<th class="nmcatalogbusquedatit" sort="fecha">Fecha</th>
<th class="nmcatalogbusquedatit" sort="product">Producto</th>
<th class="nmcatalogbusquedatit" sort="movi">Movimiento</th>
<th class="nmcatalogbusquedatit" sort="prove">Proveedor/Cliente</th>
<th class="nmcatalogbusquedatit" sort="cantidad">Cantidad</th>
<th class="nmcatalogbusquedatit" sort="monto">Monto</th>
<th class="nmcatalogbusquedatit" sort="sucu">Sucursal</th>
<th class="nmcatalogbusquedatit" sort="alma">Almac&eacute;n origen</th>
<th class="nmcatalogbusquedatit" sort="destino">Almac&eacute;n destino</th>
<th class="nmcatalogbusquedatit" sort="comentario">Comentario</th>
</tr></thead><tbody>';
$cont=0;
if(mysql_num_rows($q)>0)
{
	while($r=mysql_fetch_object($q))
	{
		...
		<td align="center">'.$r->comentario.'</td>
		</tr>';
		$i++;
		if($movimiento!="")
		{
 ...
 */ 	
	...
	if($i<=19)
	{
		for($e=$i;$e<=19;$e++)
		{	
			$tabla.='<tr class="'.$color.'" style="height:20px"> <td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td> 	</tr>';
		}
	}

	$div = '<div style="overflow-x: auto; display: flex; height: 32px;">Pagina ';
	for ($i=1; $tpaginas>=$i; $i++) {
		if($actual==$i){
			$div.='<b style="margin-left:0.2em;">'.$i.'</b> ';
		}else{
			$div.='<a style="margin-left:0.2em;cursor:pointer; text-decoration:underline;" onclick="filtramovimientos('.$i.')">'.$i.'</a> ';
		}
	}
	$div.='</div><br>';
...
function productosexistencias($iddepa=0,$idfamilia=0,$idlinea=0)
{
	...
	$cbo='<select id="producto" name="producto" onchange="cargaexistencias(this.value); filtraProveedor(this.value);" class="form-control" style="width:100%!important;">';
...
{
	if($id!=0){$filtro=" where idLinea=".$id." and vendible=1 and estatus=1";}else{$filtro="where estatus=1";}	

	$cbo='<select id="producto" name="producto" onchange="cargaProducto(this.value);" class="" >';
	$cbo.='<option value="">-Seleccione-</option>';
...
{
	$cbo='<select id="departamento" name="departamento" onchange="cargaFamilias(this.value);" class="nminputselect">';
...
{
	$cbo='<select id="departamento" name="departamento" onchange="cargaFamilias2(this.value);cargaLineas2(this.value,0);loadproductos(this.value,0,0);" class="form-control">';
	$query = mysql_query("select idDep id,nombre  from mrp_departamento");
...
function familias2($iddep=0)
{
	...
	$cbo='<select id="familia" name="familia" onchange="cargaLineas2('.$iddep.',this.value); loadproductos('.$iddep.',this.value,0);" class="form-control">';
	$cbo.='<option value="0">-Todos-</option>';
	$query = mysql_query("select idFam id,nombre  from mrp_familia ".$filtro);
	while ($row = mysql_fetch_array($query))
	{
...
function lineas($id=0)
{
	...
	$cbo='<select id="linea" name="linea" onchange="cargaProductos(this.value);" class="nminputselect">';
	$cbo.='<option value="">-Seleccione-</option>';
...
function lineas2($iddep=0,$idfam=0)
{
	$filtro="";
	if($idfam!=0){
		$filtro=" where idFam=".$idfam;
	}elseif($iddep!=0){
		$filtro=" where idFam IN (SELECT idFam from mrp_familia where idDep=$iddep)";
	}


	$cbo='<select id="linea" name="linea" onchange="loadproductos('.$iddep.','.$idfam.',this.value);" class="form-control">';
	$cbo.='<option value="0">-Todos-</option>';
	$query = mysql_query("select idLin id,nombre  from mrp_linea ".$filtro);
	while ($row = mysql_fetch_array($query))
	{
...
function formasPago()
{
	$cbo='<select id="formapago" name="formapago" onchange="seleccionaformadepago(this.value);" class="nminputselect">';
...
function proveedores($idProd=0)
{
	...
	$cbo='<select id="proveedor" name="proveedor" onchange="cargaCosto(this.value,'.$idProd.');" class="form-control">';
...
function Facturar($idFact,$monto,$impuestos,$idVenta,$bloqueo,$laobs)
{
	...
	{	  
		$elpid = $producto->id;
		$ee=mysql_query("SELECT if(deslarga='',nombre,deslarga) as descri FROM mrp_producto WHERE idProducto='$elpid';");
		$ridd=mysql_fetch_array($ee);
		echo $producto->nombre = $ridd['descri'];
		exit();
		$nn='';
		$impuestos_venta='';
		if($producto->tipodescuento=="%")
	...
	$parametros['DatosCFD']['Folio']             = $data['folio'];
		...
		$qrpac=mysql_query("SELECT pac FROM pvt_configura_facturacion WHERE id=1;");
		$respac=mysql_fetch_object($qrpac);
		$pac = $respac->pac;
...
*/
	...
	function guardarFacturacion($UUID,$noCertificadoSAT,$selloCFD,$selloSAT,$FechaTimbrado,$idComprobante,$idFact,$idVenta,$noCertificado,$tipoComp,$monto,$cliente,$trackId,$idRefact,$azurian,$total=0.00){
		...
		{
			mysql_query("UPDATE pvt_contadorFacturas set total=total+1 where id=1");
			mysql_query("UPDATE comun_parametros_licencias SET valor=valor-1 WHERE parametro='Facturas'");
		...
		if($idRefact=='c'){
			$tipoComp='C';
			$q=mysql_query("INSERT into pvt_notadeCredito (total,idfac) values ('".$total."','".$insertedId."')");
		}

		return $insertedId;
	}

/*	function guardarNotadeCredito($UUID,$noCertificadoSAT,$selloCFD,$selloSAT,$FechaTimbrado,$idComprobante,$idFact,$idVenta,$noCertificado,$tipoComp,$monto,$cliente,$trackId,$idRefact,$azurian,$total,$idfac){
		//echo 'totalnc='.$total.' foliof='.$folio;
		guardarFacturacion($UUID,$noCertificadoSAT,$selloCFD,$selloSAT,$FechaTimbrado,$idComprobante,$idFact,$idVenta,$noCertificado,$tipoComp,$monto,$cliente,$trackId,$idRefact,$azurian);
     
		$result = mysql_query("INSERT into pvt_notadeCredito (total,idfac) values ('".$total."','".$insertedId."')");
		//$insertedIdnc=mysql_insert_id();
	...
	function vendedores()
	{	
		$cbo='<select id="vendedor" name="vendedor" class="form-control" style="margin-left: 0 !important;">';
		$cbo.='<option value="">-Seleccione-</option>';
	...
	function iniciocaja()
	{
		...
		//var_dump("select  cc.saldofinalcaja from inicio_caja i inner join corte_caja cc on i.idCortecaja=cc.idCortecaja where i.idSucursal=".$sucursal_id." order by i.fecha desc limit 1");
			...
			$cbo='<select id="sucursal" name="sucursal" onchange="cargasaldocaja(this.value);" class="nminputselect">';
			$query = mysql_query("select idSuc id,nombre  from mrp_sucursal");
			$cbo.='<option value="">-Seleccione-</option>';
			while ($row = mysql_fetch_array($query))
			{
...
function checatimbres()
{
	...
	function envioFactura($uid,$Email,$azurian)
	{
        ...
        //Obteniendo la descripcion de la forma de pago
		//echo $azurian['datosTimbrado']['idVenta'].')';
        ...
        //echo $queryFormaPago;
        	...
        	$q2=mysql_query($queryFormaPago);
			if(mysql_num_rows($q2)>0)
			{
				while($r2=mysql_fetch_object($q2))
				{
					//$saldoencaja="$".number_format($r2->saldofinalcaja,2,".",",");
					if (strlen($r2->referencia) > 0) {
		                $formapago .= $r2->nombre .'('.$r2->claveSat.')'." Ref:" . $r2->referencia . ",";
		                //$formapago .= $pagosValue['nombre'] . ",";
		            } else {
		                $formapago .= $r2->nombre .'('.$r2->claveSat.')'.",";
		            }
      ...
      /*  $resultqueryFormaPago = $this->queryArray($queryFormaPago);
        ...
        //echo $azurian['Basicos']['metodoDePago'];
		...
		$obj->agregarTotal($azurian['Basicos']['subTotal'],$azurian['Basicos']['total'],$azurian['nn']['nn']);
			/*$des = explode("(", $azurian['Basicos']['metodoDePago']);
			$azurian['Basicos']['metodoDePago'] = trim($des[1],')'); */
		$obj->agregarMetodo($azurian['Basicos']['metodoDePago'],'','MXN');	 
	...
	function envioFactura3($uid,$Email,$azurian){
	if($Email!=''){
		...
		function envioFactura2($uid,$Email,$azurian)
	{
      ...
      /*  $resultqueryFormaPago = $this->queryArray($queryFormaPago);
		...
		$obj=new CFDIPDF();
		//$obj->ponerColor('#333333');
		$obj->datosCFD($datosTimbrado['UUID'],$azurian['Basicos']['folio'],$datosTimbrado['noCertificado'],$datosTimbrado['FechaTimbrado'],$datosTimbrado['FechaTimbrado'],$datosTimbrado['noCertificadoSAT'],$azurian['Basicos']['formaDePago'],$azurian['Basicos']['tipoDeComprobante'],$doc,$azurian['Basicos']['Moneda'],$azurian['Basicos']['TipoCambio']);
		$obj->lugarE($azurian['Basicos']['LugarExpedicion']);
		$obj->datosEmisor($azurian['Emisor']['nombre'],$azurian['Emisor']['rfc'],$azurian['FiscalesEmisor']['calle'].$nemi,$azurian['FiscalesEmisor']['localidad'],$azurian['FiscalesEmisor']['colonia'],$azurian['FiscalesEmisor']['municipio'],$azurian['FiscalesEmisor']['estado'],$azurian['FiscalesEmisor']['codigoPostal'],$azurian['FiscalesEmisor']['pais'],$azurian['Regimen']['Regimen']);
		$obj->datosReceptor($azurian['Receptor']['nombre'],$azurian['Receptor']['rfc'],$azurian['DomicilioReceptor']['calle'].$nrec,$azurian['DomicilioReceptor']['localidad'],$azurian['DomicilioReceptor']['colonia'],$azurian['DomicilioReceptor']['municipio'],$azurian['DomicilioReceptor']['estado'],$azurian['DomicilioReceptor']['codigoPostal'],$azurian['DomicilioReceptor']['pais']);
		$obj->agregarConceptos($azurian['Conceptos']['conceptosOri']);
		$obj->agregarTotal($azurian['Basicos']['subTotal'],$azurian['Basicos']['total'],$azurian['nn']['nn'],$azurian['Basicos']['descuento']);
			/*$des = explode("(", $azurian['Basicos']['metodoDePago']);
			$azurian['Basicos']['metodoDePago'] = trim($des[1],')'); */
		$obj->agregarMetodo($azurian['Basicos']['metodoDePago'],$refFormaPago,$azurian['Basicos']['Moneda']);	 
		$obj->agregarSellos($datosTimbrado['csdComplemento'],$datosTimbrado['selloCFD'],$datosTimbrado['selloSAT']);
		$obj->agregarObservaciones($azurian['Observacion']['Observacion']);
		$obj->generar("../../netwarelog/archivos/1/organizaciones/".$azurian['org']['logo']."",0);
		$obj->borrarConcepto();
