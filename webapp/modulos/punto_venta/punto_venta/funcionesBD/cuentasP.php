...
//-------------------------------------------------------------------------------------------------------------------------------------------
	...
	{
		$id = $_POST["id"];
		$result = $connection->query("SELECT fechacargo, fechavencimiento, concepto, monto, saldoabonado, saldoactual, estatus FROM cxp WHERE idCxp = ".$id);

		if($result->num_rows > 0)
		{
			if($row = $result->fetch_array())
				echo $row["fechacargo"].'$$$^^^###///'.$row["fechavencimiento"].'$$$^^^###///'.$row["concepto"].'$$$^^^###///'.$row["monto"].'$$$^^^###///'.$row["saldoabonado"].'$$$^^^###///'.$row["saldoactual"].'$$$^^^###///'.$row["estatus"];	
		}
		else
	...
	function crearCuenta($connection)
	{
		...
		if ($result = $connection->query("INSERT INTO cxp (fechacargo, fechavencimiento, concepto, monto, saldoabonado, saldoactual) VALUES ('".$fecha_cargo."', '".$fecha_vencimiento."', '".$concepto."', ".$monto.", ".$saldo_abonado.", ".$saldo_actual.")"))
		{
			echo 1;
		}
		else 
