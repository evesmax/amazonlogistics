...
function loadproductos($idDepartamento, $idFamilia, $idLinea) {
	console.log('Esta es la función cercana al punto de venta');
	console.log(arguments);
	$.ajax({
		type: 'POST',
		url:'funcionesPv.php',
		data:{funcion:"productosexistencias",iddepa: $idDepartamento, idfamilia:$idFamilia, idlinea:$idLinea},
		success: function(resp){
...
function dvCancelarVentaSi(){
	$.ajax({
		type: 'POST',
		url:'funcionesPv.php',
		data:{funcion:"cancelarventa",id:dvGeneral},
		success: function(resp){
			$('#caja-dialog-confirmacion').modal('hide');
...
function dvCancelarVenta(){
	$('#caja-dialog-confirmacion-titulo').html('Cancelar venta');
	$('#caja-dialog-confirmacion-cuerpo').html('Estas seguro que deseas cancelar esta venta, no se podrá deshacer esta acción?');
	$('#caja-dialog-footer-confirmacion-devolver-venta').hide();
	$('#caja-dialog-footer-confirmacion-cancelar-venta').show();
	$('#caja-dialog-confirmacion').modal('show');
}

function dvDevolverSi(){
	$('.cantidades').each(function(index, el) {
		console.log(el);
	});
	var array = {};
    var contador = 0;
    $("#table-caja-devolucion tr").each(function (index) 
    {   //console.log($("#table-caja-devolucion input:text"));
        x=$("input:text", this);
        //console.log(x);
        if(x.length>0){
            item = {};
...
function dvDevolver(){
	...
	.done(function(resp) {
		...
		$('#caja-dialog-confirmacion-titulo').html('Devolución');
		$('#caja-dialog-confirmacion-cuerpo').empty().append(resp2);
		$('#caja-dialog-footer-confirmacion-cancelar-venta').hide();
		$('#caja-dialog-footer-confirmacion-devolver-venta').show();
		$('#caja-dialog-confirmacion').modal('show');			   				
...
function dvImprimir(){
	window.open("ticket.php?idventa="+dvGeneral);
	$('#caja-dialog').modal('hide');
}
	...
	var lucha = id;
 	$.ajax({
		...
		success: function(resp){   
			...
			$('#caja-dialog-titulo').empty().html('Detalle de venta');
			$('#caja-dialog-cuerpo').empty().append(resp);
			$('#caja-dialog-footer-detalle-venta').show();
			$('#caja-dialog').modal('show');
			dvGeneral = id;

		}});//end ajax	
}
//////////////////////////////////////////////////////////////////
function filtramovimientos(pagina)
{
	$("#preloader").show();
	 $.ajax({
		type: 'POST',
		url:'funcionesPv.php',
		data:{funcion:"entradasalidas",registros:$("#registros").val(),inicio:$("#inicio").val(),fin:$("#fin").val(),producto:$("#producto").val(),movimiento:$("#movimiento").val(),pagina:pagina},
		success: function(resp){ 
			$("#preloader").hide(); 
...
function cargaLineas2($idDepartamento, $idFamilia)
{
	console.log('lineas pero no es el idFamilia');
	console.log(arguments);
	$.ajax({
		type: 'POST',
		url:'funcionesPv.php',
		data:{funcion:"cargaLineas2", iddepa: $idDepartamento, idFamilia:$idFamilia},
		success: function(resp){  $("#span-lineas").html(resp);       }});//end ajax	
}
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
