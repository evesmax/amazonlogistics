 ...
 (function($) {
        ...
        $.fn.jTPS = function ( opt ) {
                ...
                $(this).data('tableSettings', $.extend({
                        perPages:                       ['TODO'],                              // the "show per page" selection
                        perPageText:            'Elementos por pagina:',                                               // text that appears before perPages links
                ...
                var pT = $(this), page = page || 1, perPages = $(this).data('tableSettings').perPages, perPage = perPage || perPages[0],
                        rowCount = $('>tfoot', this).find('tr').length;
                ...
                if ( $(this).data('tableSettings').fixedLayout ) {
                        ...
                        if ( $(this).css('table-layout') != 'fixed' ) {
                                ...
                                // ensure browser-formated widths for each column in the thead and tfoot
                                var tfootCh = $('>tfoot',this)[0].childNodes, tmpp = 0;
                                // loop through tfoot children and find the Nth <TR>
                                for ( var tbi=0, tbcl=tfootCh.length; tbi < tbcl; tbi++ )
                                        if ( tfootCh[ tbi ].nodeName == 'TR' )
                                                maxCellHeight = Math.max( maxCellHeight, tfootCh[ tbi ].offsetHeight );

                                // now set the height attribute and/or style to the first TD cell (not the row)
                                for ( var tbi=0, tbcl=tfootCh.length; tbi < tbcl; tbi++ )
                                        if ( tfootCh[ tbi ].nodeName == 'TR' )
                                                for ( var tdi=0, trCh=tfootCh[ tbi ].childNodes, tdcl=trCh.length; tdi < tdcl; tdi++ )
                ...
                var stubCount=0, cols = Math.max( $('>thead:first tr:last th,>thead:first tr:last td', this).length, parseInt( $('>thead:first tr:last th,>thead:first tr:last td').attr('colspan') || 0 ) ),
                        stubs = ( perPage - ( $('>tfoot>tr', this).length % perPage ) ),
                        stubHeight = $('>tfoot>tr:first>td:first', this).css('height');
                for ( ; stubCount < stubs && stubs != perPage; stubCount++ )
                        $('>tfoot>tr:last', this).after( '<tr class="stubCell"><td colspan="' + cols + '" style="height: ' + stubHeight + ';">&nbsp;</td></tr>' );

                // paginate the result
                if ( rowCount > perPage && perPage != 0 )
                        $('>tfoot>tr:gt(' + (perPage - 1) + ')', this).addClass('hideTR');
                                ...
                                function (tdInd) {
                                        $(this).addClass('sortableHeader').unbind('click').bind('click',
                                                function () {
                                                        ...
                                                        var page = parseInt( $('.hilightPageSelector:first', pT).html() ) || 2;
                                                        $('>tfoot>tr', pT).removeClass('hideTR').filter(':gt(' + ( ( perPage - 1 ) * page ) + ')').addClass('hideTR');
                                                        $('>tfoot>tr:lt(' + ( ( perPage - 1 ) * ( page - 1 ) ) + ')', pT).addClass('hideTR');
                                                        ...
                                                        $('>tfoot>tr>td.sortedColumn', pT).removeClass('sortedColumn');
                                                        $('>tfoot>tr:not(.stubCell)', pT).each( function () { $('>td:eq(' + columnNo + ')', this).addClass('sortedColumn'); } );
                                                        clearSelection();
                                ...
                                $(this).bind('click',
                                        function () {
                                                ...
                                                var stubCount=0, cols = $('>thead th,>thead td', pT).length,
                                                        stubs = ( perPage - ( $('>tfoot>tr', pT).length % perPage ) ),
                                                        stubHeight = $('>tfoot>tr:first>td:first', pT).css('height');
                                                for ( ; stubCount < stubs && stubs != perPage; stubCount++ )
                                                        $('>tfoot>tr:last', pT).after( '<tr class="stubCell"><td colspan="' + cols + '" style="height: ' + stubHeight + ';">&nbsp;</td></tr>' );
                                                ...
                                                // update status bar
                                                var cPos = $('>tfoot>tr:not(.hideTR):first', pT).prevAll().length,
                                                        ePos = $('>tfoot>tr:not(.hideTR):not(.stubCell)', pT).length;
                ...
                var cPos = $('>tfoot>tr:not(.hideTR):first', this).prevAll().length,
                        ePos = $('>tfoot>tr:not(.hideTR):not(.stubCell)', this).length;
                                                ...
                                                function () {
                                                        ...
                                                        if ( $(this).hasClass('hilightPageSelector') ) {
                                                                if ( $(this).parent().queue().length > 0 ) {
                                                                        ...
                                                                        $('>tfoot> tr', pT).removeClass('hideTR').addClass('hideTR');
                                                                        $('>tfoot>tr:gt(' + (beginPos - 2) + '):lt(' + ( perPage ) + ')', pT).andSelf().removeClass('hideTR');
                                                                        // update status bar
                                                                        var cPos = $('>tfoot>tr:not(.hideTR):first', pT).prevAll().length,
                                                                                ePos = $('>tfoot>tr:not(.hideTR):not(.stubCell)', pT).length;
                                                                        $('>.nav .status', pT).html( 'Showing ' + ( cPos + 1 ) + ' - ' + ( cPos + ePos ) + ' of ' + rowCount + '' );
                                                        ...
                                                        var beginPos = $('>tfoot>tr:not(.hideTR):first', pT).prevAll().length,
                                                        ...
                                                        while ( steps-- ) {
                                                                $(this).parent().animate({'opacity':1}, $(pT).data('tableSettings').scrollDelay,
                                                                        function () {
                                                                                ...
                                                                                /* scoll up */
                                                                                if ( beginPos > endPos ) {
                                                                                        $('>tfoot>tr:not(.hideTR):first', pT).prevAll(':lt(' + sStep + ')').removeClass('hideTR');
                                                                                        if ( $('>tfoot>tr:not(.hideTR)', pT).length > perPage )
                                                                                                $('>tfoot>tr:not(.hideTR):last', pT).prevAll(':lt(' + ( sStep - 1 ) + ')').andSelf().addClass('hideTR');
                                                                                        // if scrolling up from less rows than perPage - compensate if < perPage
                                                                                        var currRows =  $('>tfoot>tr:not(.hideTR)', pT).length;
                                                                                        if ( currRows < perPage )
                                                                                                $('>tfoot>tr:not(.hideTR):last', pT).nextAll(':lt(' + ( perPage - currRows ) + ')').removeClass('hideTR');
                                                                                /* scroll down */
                                                                                } else {
                                                                                        var endPoint = $('>tfoot>tr:not(.hideTR):last', pT);
                                                                                        $('>tfoot>tr:not(.hideTR):lt(' + sStep + ')', pT).addClass('hideTR');
                ...
                function sort ( target, tdIndex, desc ) {
                        var fCol = $('>thead th,>thead th', target).get(tdIndex),
                                ...
                                re = /([-]?[0-9\.]+)/g,
                                rows = $('>tfoot>tr:not(.stubCell)', target).get(),
                                procRow = [];
                        ...
                        // now re-order the parent tfoot based off the quick sorted tfoot map
                        $('>tfoot', target).addClass('jtpstemp').before('<tfoot></tfoot>');
                        var nr = procRow.length, tf = $('>tfoot', target)[0];
                        // move the row from old tfoot to new tfoot in order of new tfoot with replaceWith to retain original tfoot row positioning
                        if ( sorted )
                                while ( nr-- )
                        ...
                        $('>tfoot.jtpstemp', target).remove();
                        // redraw stub rows
                        var stubCount=0, cols = $('>thead>tr:last th', target).length,
                                stubs = ( perPage - ( $('>tfoot>tr', target).length % perPage ) ),
                                stubHeight = $('>tfoot>tr:first>td:first', target).css('height');
                        for ( ; stubCount < stubs && stubs != perPage; stubCount++ )
                                $('>tfoot>tr:last', target).after( '<tr class="stubCell"><td colspan="' + cols + '" style="height: ' + stubHeight + ';">&nbsp;</td></tr>' );
                }
                // chainable
                return this;
        };
