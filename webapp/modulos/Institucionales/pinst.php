...
//Genera las consultas y llena el arreglo principal
		...
		$sqldelete="delete from reporte_institucionales where idempleado=$usuario";
                ...
                //Actualiza acmulados @Acumulado
                $idorigen=0;
                $iddestino=0;
                $valores="";
                $acum=0;
                $sqlacum="SELECT idrep FROM reporte_institucionales
                        WHERE c1='@Acumulado' and idempleado=$usuario ORDER BY idrep";
                
                $resultado = $conexion->consultar($sqlacum);
                while($rs = $conexion->siguiente($resultado)){
                    $iddestino=$rs{"idrep"};
                    $idorigen=$iddestino-1;
                    
                    $sqlwhere="Select * from reporte_institucionales where idrep=$idorigen";
                    //echo $sqlwhere;
                    $resultadowhere = $conexion->consultar($sqlwhere);
                    while($rswhere = $conexion->siguiente($resultadowhere)){
                        $valores="";
                        $coma="";
                        $acum=0;
                        for ($f=1; $f<=$nperiodos-1;$f++){
                            $acum=$rswhere{"c$f"}+$acum;
                            $valores.=$coma."c$f='".$acum."' ";
                            $coma=",";
                        }
                        $sqlupdate="Update reporte_institucionales Set $valores where idrep=$iddestino";
                        //echo $sqlupdate."<br>";
                        $conexion->consultar($sqlupdate);
                    }
                    $conexion->cerrar_consulta($resultadowhere);
                }
                $conexion->cerrar_consulta($resultado);
...
function regresavalor($sqlvalor,$idcierre,$conexionh){
        ...
        if($sqlvalor<>"Acumulado"){
            //Altera y completa Sql
            $filtro=" And idcierre=$idcierre ";
                    
					$encontrado=strpos($sqlvalor,"@FiltroAcumular");
                    if ($encontrado !== FALSE){
                        $filtroacum="And idcierre<=$idcierre";
                        $sqlvalor=str_replace('@FiltroAcumular',$filtroacum,$sqlvalor);
                    }
